...
- **[Current synthesis re-triage](docs/reports/2026-09-09-synthesis-after-native-contextual-integration.md)**
  — close contextual list construction and exact replay, deliver missing
  Church behaviors, then expand query forms and provider evidence;
...
- **Current synthesis progress.** The bounded local `Type 0` contextual route
  ...
  **2,308 tests in 15 suites**, and **350 signatures per Haskell engine** pass.
  ...
  The [new tree/frontend re-triage](docs/reports/2026-09-09-synthesis-after-native-contextual-integration.md)
  ...
  **350 signatures per Haskell engine**, and **13/13 Exference extended behaviors**.
  ...
  acceptance remains open. The [current delivery order](docs/reports/2026-09-09-synthesis-after-native-contextual-integration.md)
  prioritizes contextual list construction and exact replay, then missing Church
  behaviors, remaining query forms and broader provider evidence. Deadline and
  native contextual integration are complete; the tree investigation remains bounded.
...
- `--length-contract ABSOLUTE-PATH` swaps in a passive contract for one
  command;
- the common built-in-`List` case is one line in Lean notation:
  ...
  Here `--where` requests filtering, every translated source-arrow whose type
  is an exact unary `List` is observed in source order, and the checked result
  selects the scalar profile or canonical `Prod`-of-two-lists profile. The
  clause still reuses the already activated startup execution policy; it cannot
  discover or authorize Z3. Unsupported or ambiguous targets fail closed;
- the advanced inline form keeps every choice explicit for custom observation
  roles or for compatibility with the low-level Length surface:
...
The standalone Djex REPL supports the same workflow independently, using
Haskell notation and its own policy command:
...
Behavioral synthesis is a first-class use case of both repositories, not a
Leant-only frontend over a Djex library. See the
[host-native shorthand and Djex parity reference](docs/length-ranking.md#host-native-shorthand-and-djex-repl-parity).
...
The [implementation re-triage](docs/reports/2026-09-09-synthesis-after-native-contextual-integration.md)
