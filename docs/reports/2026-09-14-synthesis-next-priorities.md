...
The final contextual-binder snapshot has now passed **all 63 public cells** (42 exact kernel replays and 21 actual-False controls), **all 746 native tests**, and **100 private Exference tests**. The `context-universe-public-v14` controller is terminal `passed`, with unchanged captured source and runtime hashes. These are final-snapshot results, replacing reliance on the earlier prototype for these gates. The additional 27 contextual-constructor and 15 strict-binder regression cells are running under `context-universe-regressions-v7`; native publication still depends on their completion. The nine native files and Djex dependency update are prepared locally, not yet published. The completed Haskell release has its own archived evidence; this native status is an interim observation until its complete release archive is produced.

Two priorities change. First, contextual polymorphic type selection moves ahead of a complete class/provider universe redesign: it directly addresses the original impredicative-synthesis requirement and has an explicit refusal in the current implementation. Second, the prepared Lean Djinn `length` comparison gets one bounded diagnostic slot immediately after release closure. It has a checked Exference implementation for comparison and should not wait behind another large metadata change. A failed diagnostic does not earn an open-ended search project.

## Execution order

| Order | Deliverable | Why now and how to finish |
| --- | --- | --- |
| 0 | Close contextual binder integration. | Finish the 42 affected regression cells, package final evidence, publish the tested native source and pin, and update release links. Do not expand the frozen validation source during these checks. |
| 1; one bounded experiment | Diagnose Lean Djinn `length`. | Run the prepared direct-versus-wrapped constructor-field carrier comparison at its fixed limits. The proposed loss is a `Nat` accumulator hidden behind an `Int` constructor field; this remains a hypothesis. If the comparison identifies the first missing construction step, make a focused general repair and require Djinn's own public result and exact replay. Otherwise record the negative result and proceed to source work. |
| 2; first source acceptance | Contextual selection of a polymorphic type. | Construct the smallest Lean-valid public query that forces a quantified type argument instead of permitting a monomorphic substitute. Preserve its binder scope, universe formation and selected evidence through projection and replay. The current `ContextUnsupportedSelectedPolytype` refusal makes this a concrete capability boundary. First establish the witness and baseline; do not merely remove the guard. |
| 3; staged source acceptance | Universe-polymorphic nominal types, classes and providers. | Retain declaration signatures and selected level vectors, then introduce the smallest working nominal/class case before global provider discovery. Two selections of one declaration must coexist without capturing universe parameters or merging different dictionaries. A shared sort-accounting prerequisite may precede item 2; a complete provider framework need not. |
| 4; independent source acceptance | Exference explicit Haskell kinds. | The public execution guard remains. Follow an original kinded query through opening, freshening, substitution and provider checking to the first lost kind. Reuse sound shared transport, but require Exference's own ordinary and named-`where` results with GHC replay. Bound investigation at a concrete transformation boundary instead of allowing it to hold up unrelated work. |
| 5 | Integer `at`. | Baseline one native engine before assuming unindexed native cases share the historical Haskell misses. Separate arithmetic availability from construction of index state. Then close both Haskell engines and all three native modes with negative, zero, in-range and out-of-range observations and supplied defaults. |
| 6; diagnostic only | Extrema. | Existing representative misses and the failed fixed-carrier experiment justify a witness trace, not another speculative heuristic. Find the first absent, pruned or unaffordable step of a checked implementation. Try one general repair only after that causal result. |

This is a serial execution order for one heavy build/runtime owner, not a request for parallel full suites. The single `length` experiment is deliberately smaller than the following source milestones. If it yields no actionable result, source work resumes without raising limits or adding more carrier variants.

## Split the universe work at useful acceptance boundaries
