...
\fancyfoot[L]{\small\sffamily\color{Muted}Updated 15 August 2026}
...
\begin{titlepage}
  ...
  \vfill
  \begin{tabular}{@{}ll@{}}
    \textsf{Date:} & 10 August 2026; updated 15 August 2026 \\
  ...
  \vspace{4mm}
  {\small\color{Muted}
  \emph{Status, 15 August 2026:} the level-1 layer proposed here has since been
  ...
  extension, exact derived-domain ranking through strict relational Natural
  quotient consequences, and sealed descriptor-bound Z3 launch, reachable from
  \code{:synth} behind the explicit
  ...
  exact boundaries.  The successor proposal,
  \emph{Precise Behavioral Code Synthesis in Leant and Djex with Z3}
  (\code{docs/Z3\_Behavioral\_Synthesis\_Proposal/}, 15 August 2026), starts
  from that implemented layer and lays out those next increments.

  \emph{Status, 19 August 2026:} the opt-in semantic post-filter proposed as
  level 1 has also shipped: \code{:synth --behavior-mode filter} performs
  command-authorized hard Length filtering in which only an independently
  replayed counterexample rejects a candidate and every other assessment
  retains it, supported by command-local counterexample banks and progressive
  same-run filter batching; the default rank lane still reorders without
  pruning.  Levels 2 and 3 (typed sketch completion and semantic branch
...
\section{Implementation addendum: exact root-quotient domains}
\label{sec:quotient-addendum}

The current implemented checkpoint advances Leant's closed live Length-ranking
configuration through scalar v19 and product v20.  These versions retain the complete
v17/v18 sealed descriptor launch, owner-thread-scoped usable-work deadline,
cooperative checkpoints, deferred worker opening, non-vacuous preferences,
counterexample simplification, and scalar-v5 or pair-v5 contract grammar.  They replace
only the applicable-domain strategy with
\code{strict-relational-positive-affine-quotient-v1}.

\begin{lstlisting}[language=HaskellExt,basicstyle=\ttfamily\scriptsize,
  caption={Public Leant root-quotient surface}]
...
Write $q_d(E)$ for Natural floor quotient $E\mathbin{\mathrm{quot}}d$, with
$d>0$.  Djex adds exactly four proof-only implications for positive-affine operands:

\begin{align*}
q_d(A) \leq B &\Longrightarrow A \leq dB+(d-1), \\
A \leq q_d(B) &\Longrightarrow dA \leq B, \\
\neg(q_d(A) \leq B) &\Longrightarrow d(B+1) \leq A, \\
\neg(A \leq q_d(B)) &\Longrightarrow B+1 \leq dA.
\end{align*}

A top-level equality contributes the two directed non-strict rules in source order.
The inherited synchronous rule-once closure can therefore propagate a later literal
ceiling through either orientation.  For example, $x\leq q_3(y)$ together with
$y\leq8$ derives source-ordered maxima $[2,8]$; $q_3(x)=y$ together with
$x\leq1$ derives $y\leq0$.

\begin{warningbox}[title=Deliberately narrow authority]
Exactly one relation side may contain a quotient, and that quotient must be at the
operand root.  A nested quotient, a quotient embedded below a sum, scale, monus,
minimum, maximum, modulo, or conditional, quotients at both relation roots, an
unsupported dividend or opposite operand, and an unsupported whole formula grant no
coverage rule or partial bound.  Quotient-free clauses delegate to the strict
predecessor exactly.  Unsupported clauses remain in the actual precondition replay if
other clauses establish a complete box.
\end{warningbox}

Validation retains the predecessor's precedence: reject input width before demanding
the precondition or evaluation limits; scan clauses and close rules; report the first
compact missing input; validate maxima and Cartesian cardinality; then replay
assignments canonically.  Only complete query-owned replay releases an establishment
receipt.  The runtime route remains newest-first four-entry MRU replay, derived-domain
replay, canonical origin replay, live Z3 query, and optional post-unsat explicit box.
An inapplicable quotient pass is a pure miss; a counterexample crosses the existing
query-owned simplification and MRU seams.

\begin{small}
\begin{longtable}{@{}L{0.12\textwidth}L{0.32\textwidth}L{0.27\textwidth}L{0.20\textwidth}@{}}
\toprule
\textbf{Versions} & \textbf{Applicable-domain rule} & \textbf{Usable work} &
\textbf{Launch} \\
\midrule
\endhead
...
The scalar-only compatibility decoder still rejects v4--v20.  The generalized decoder
reaches v19/v20 only after the exact v1--v18 cascade returns its closed unsupported-
version sentinel; v21 is unsupported.  Complete scalar-v19 and pair-v20 JSON documents,
including the inherited descriptor execution member
\code{executableLaunch = descriptor-bound-executable-v1}, scoped-v2 budget, deferred
opening, and contract grammars, are recorded in
\filepath{README.md} and
\filepath{docs/reports/2026-08-15-strict-relational-positive-affine-quotient-length-ranking.md}.

\begin{soundbox}[title=Evidence and identity remain separate]
The four rewrites emit no SMT-LIB command and consume no solver status.  They do not
alter normalized contract bytes, checked problem identity, sealed query fingerprints or
wire bytes, protocol, process, ready-worker, scalar-run, or pair-run identity.  Djex
adds only nominal scalar and product quotient-establishment receipt tags; Leant adds
the matching nominal assessments and renderers, with no canonical evidence bytes of
its own.  The receipt remains model/provider-relative finite-spine evidence: it is not
a Lean theorem, source-language totality or effect claim, solver-soundness claim, or
pruning authority.
\end{soundbox}

The descriptor guarantee remains main-image-only.  It binds the configured digest to
the sealed staged executable bytes, not to an interpreter, dynamic loader, shared
library, file capability, solver semantics, or solver result.  An all-pure deferred
batch still opens no executable descriptor and no worker.

At the quiescent v19/v20 characterization checkpoint,
\filepath{test-unit/Spec.hs} contains 23,334 lines and 443 exact
\code{testCase} occurrences.  The focused quotient-consequence group contains seven
passing cases under the strict test build.  These counts describe this dated snapshot,
not a stable API contract.

The corresponding Djex extraction report is
\filepath{lib/Djex/docs/reports/2026-08-15-strict-relational-positive-affine-quotient-length-applicable-domain.md}.
This checkpoint does not declare Boolean/DNF extraction, or any other behavioral
extension, to be definitively next.
