...
| --- | --- | --- | --- |
| Accepted milestone | `maybeEither` composition in both languages. | The canonical and native release checks pass. The release includes the tested code, indexed receipts and matching documentation; Leant pins the corresponding Djex revision. | Preserve the frozen validation snapshots and check code parity, dependency identity and both remote refs during publication. |
...
| 2 | Lean Djinn `length`: one independent miss. | The fresh retry on the `maybeEither` build still misses at the original 90-second deadline, after 23 falsifications and zero inconclusive observations. Trace its admitted plans and reconstruction/verification losses before selecting a change; do not repeat the unchanged retry. | Exact original-type synthesis, independent Lean replay and an actual False control. Provider admission and Both-mode success do not discharge Djinn acceptance. |
