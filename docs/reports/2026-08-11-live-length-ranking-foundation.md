...
plan key, closed solver status, and optional decoded value artifact. During a
live run, the exact bounded status-frame bytes remain singly owned by the causal
transcript and unchanged query-run identity rather than being copied into that
decoded branch.
