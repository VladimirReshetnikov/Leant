...
The default **unbounded** Haskell one-shot search timed out at its separate 90-second process guard. At 4,096 choices, candidate cutoffs of both 1 and 200 return a term promptly; the captured comparison therefore does not implicate the candidate cutoff alone. This archive alone does not resolve that limitation. The subsequent [contextual proof-prefix repair](2026-09-14-contextual-proof-cutoff-acceptance.md) identifies whole-list checking before the cutoff and validates both original default requests, with exact GHC replay and controls. Its implementation preserves defaults and limits and retains this failed attempt as historical evidence.

This backend archive records Haskell acceptance. The subsequent [native dictionary-selection release](2026-09-14-dictionary-selection-acceptance.md) now integrates this exact Djex revision with Leant's source-evidence repair and passes 98 focused tests, all 766 native tests and 69 affected public cells. It retains its own source/runtime/replay archive, including the original dictionary example across individual engines and Both. Exference's explicit Haskell source-kind execution guard, integer indexing and the original two-universe query remain open under the [current plan](2026-09-14-synthesis-next-priorities.md). No Church behavior-ledger cells are closed by this milestone.
