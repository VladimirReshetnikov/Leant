...
The first line is now implemented: the Length clause is unquoted and ends at
the standalone delimiter, and quote characters are rejected rather than
interpreted as a string. The displayed scalar profile has the explicit
\code{list-binary-product-exact-cases} sibling for product results. Both
construct the same passive contract vocabulary used by files and receive
execution authority only from the activated startup policy. The \code{--law}
and \code{--example} forms remain proposed frontends. They must not implement
independent semantics. Names such as \code{state-bind} would resolve to
versioned built-in contract templates, and the expanded contract fingerprint
would be shown in diagnostics.
