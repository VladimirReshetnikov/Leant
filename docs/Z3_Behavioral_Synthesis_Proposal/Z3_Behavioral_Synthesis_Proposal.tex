...
\Leant and \Djex already solve the type-directed half of program synthesis unusually well: they combine terminating logical proof search, ranked heuristic search, exact source-to-engine provenance, typed candidate graphs, provider instantiation evidence, and mandatory re-elaboration by the Lean backend. They also already contain a serious first behavioral layer. The finite-spine \emph{Length} domain can associate a verified candidate with an exact inventory and contract, symbolically interpret a checked candidate graph, lower the resulting counterexample problem to bounded canonical \QFLIA, run a capability-probed \Zthree worker, and accept model input only after independent replay. At the revisions inspected for this proposal the \Leant integration was deliberately a post-verification ranking stage: it could reorder candidates but not prune them, and raw solver status carried no authority. Since those snapshots, explicit replay-authorized filtering and a fixed-profile inline Length where-clause frontend have shipped; the snapshot-relative proposal below and the later status notes keep those milestones distinct.
...
Handoff & \code{Leant.Synth.Length.Handoff}: the sole conversion of one \code{Verified} receipt plus one passive contract selection (file-backed or resolved from the later inline frontend) into a checked \Djex Length problem. It refuses mismatches instead of trusting caller-supplied graphs or names. \\
...
\paragraph{Implemented since the pinned revisions (20 August 2026).} The fixed
inline Length frontend now reaches that filter runtime through the exact
unquoted form
\code{:synth -{}-behavior-mode filter -{}-length-model
list-scalar-exact-cases|list-binary-product-exact-cases
-{}-length-inputs arg0[,argN...] -{}-where CLAUSE -{}- TYPE}. It requires
explicit filter authority, is exclusive with a contract file, parses one
bounded ASCII relation, counts only translated physical source-arrow slots,
and expands one fixed \code{List} exact-case profile with explicit roles and no
provider laws. Main authorizes and parses, emits the established premise-scope
report, then translates; it resolves the profile after translation/retry and
only then opens one nominal context for all lanes. This remains a passive
contract frontend over the existing checked handoff, not a general Lean
semantics or execution-authority inference.
...
\subsection{Ranking, not pruning, at the inspected snapshot}
...
That architectural step has since shipped exactly as a separate selection
boundary. The historical permutation path remains the ranking mode; explicit
filter mode alone may omit an occurrence, and then only with an independently
replayed counterexample. The later inline where-clause frontend constructs one
fixed passive contract source for that selection path and does not weaken the
handoff or partition seals.
...
A bare type or contract file must not silently remove candidates. The planned
host-native \code{--where} form is itself an explicit request for behavioral
filtering, so it may default the omitted mode to \code{filter}; modes that
launch CEGIS, sketch, proof, or CHC work remain explicit. A clause still grants
...
:synth --behavior-mode filter --length-model list-scalar-exact-cases --length-inputs arg0 --where len(result)=len(arg0) -- List a -> List a
:synth --law state-bind -- TYPE
:synth --law option-bind -- TYPE
:synth --example "[1,2] => [2,1]" -- List Nat -> List Nat
\end{lstlisting}

The first two lines are the next host-native shorthand milestone, not current
syntax. The first belongs to Leant and uses Lean's \code{List.length},
relations, and projections. The second belongs to Djex's standalone Haskell
REPL and uses ordinary Haskell application and comparison notation. The third
line is the currently implemented Leant compatibility form: its Length clause
is unquoted and ends at the standalone delimiter, and quote characters are
rejected rather than interpreted as a string. Its scalar profile has the
explicit \code{list-binary-product-exact-cases} sibling for product results.

For a simple, unambiguous built-in-list target, the shorthand defaults
\code{--where} to filter mode, chooses the host's standard list spine,
requires the scalar or pair result domain to agree with both expression and
checked target, and observes every eligible list argument in source order---not
only referenced arguments. It reuses a safe activated execution policy; an
unconfigured session needs one short activation command first. Ambiguous or
custom shapes fail closed and require the explicit form. Neither frontend
executes its host expression: both lower into the same bounded passive contract
vocabulary and therefore share normalization, fingerprints, replay, limits,
and rejection authority.

Djex REPL parity is part of this immediate surface checkpoint, before new
\code{--law}, \code{--example}, typed-sketch, or additional-domain grammar.
This is a product requirement: Djex's standalone Haskell applications must not
depend on Leant as their only interactive behavioral frontend. Both REPLs
should expose the examples and resolved defaults in their synthesis help.
The \code{--law} and \code{--example} forms remain proposed frontends and must
not implement independent semantics. Names such as \code{state-bind} would
resolve to versioned built-in contract templates, and the expanded contract
fingerprint would be shown in diagnostics.
...
The table deliberately remains a map of commit \code{b701ee5}. After that
snapshot, \path{src/Leant/Synth/Length/Command.hs} gained the separate
fixed-order inline parser,
\path{src/Leant/Synth/Length/Where.hs} gained the two fixed list profiles and
physical-role resolver over Djex's bounded where parser, and
\path{src/Main.hs} activated them with structure--goal--authorization--bounded
parse--scope-report--translation/retry--physical-arrow
resolution--one-context ordering.
Those additions still feed the same Handoff and nominal ranking/selection
