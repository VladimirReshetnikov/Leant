...
\hypersetup{
  ...
  pdfauthor={Technical proposal prepared for Vladimir Reshetnikov},
...
\end{tcolorbox}
\vfill
{\large Technical proposal prepared for Vladimir Reshetnikov\par}
\vspace{0.12in}
...
\item \textbf{The implementation should remain additive.} Existing Length configuration files, scalar and pair contracts, startup behavior, and ranking output can remain byte-for-byte compatible. A new generic behavioral layer can wrap the current Length implementation, while new hard-filter and CEGIS commands are opt-in and separately fingerprinted.
\end{enumerate}
...
\code{prepare\allowbreak Checked\allowbreak Length\allowbreak Problem} accepts a passive contract and one callback-accepted \code{Verified} \code{Detailed\allowbreak Verification\allowbreak Variant}. It requires the typed rendering route and exact semantic sidecar, checks that source and search fragments were not retargeted, requires the expected premise and request shape, rerenders the original variant, resolves the exact family and providers, seals a session with one interpretation policy, seals the contract in that session, and finally asks \Djex to seal the whole typed candidate into a candidate-specific problem. The product-domain analogue additionally requires a canonical Lean \code{Prod} result whose two fields are the configured spine.
...
\item Preserving every current textual configuration shape while simultaneously declaring a stable public contract language. Existing formats can remain compatible; new formats should remain experimental and versioned until the semantics settles.
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
\item out-of-bounds \code{seq.nth} is underspecified, so every index observation must carry and preserve an explicit bounds guard;
...
The official Z3 documentation repository contains the 2025 Synthesiz3 artifact \cite{synthesiz3}, which encodes synthesis specifications of the shape
\[
  \forall \vec{c}.\;\exists y.\;\forall \vec{u}.\;F(\vec{c},y,\vec{u})
\]
using an extended SMT-LIB command and a set of ``uncomputable'' symbols. This work is relevant because it demonstrates synthesis-oriented reasoning on top of Z3, especially when specifications contain symbols that may be used logically but not in the synthesized term.
...
\begin{description}[leftmargin=2.2em,style=nextline]
\item[Bounded normal forms.] Generate beta-normal, eta-long structural alternatives for the target. Best for plumbing terms and logic.
\item[Exference frontier extraction.] Reuse Exference's rated providers and partial search states to create a goal-relevant grammar. Best for library composition.
\item[Template families.] Domain-specific skeletons such as folds, maps, affine arithmetic, bit tricks, case splits, or state-threading forms. Best for deep behavior with a small combinatorial shell.
\end{description}
...
This rejects constant empty, singleton, drop and duplicate-length-changing candidates. It retains identity, reverse, rotations, arbitrary permutations, element replacement and some duplicate/delete combinations of equal net length.
...
The callback $g$ becomes an uninterpreted function $G:E_\alpha\to E_\beta$. A candidate which ignores $g$ cannot generally inhabit the polymorphic result type without another provider, but a candidate which applies $g$ to the wrong element or reverses the list is distinguishable by a symbolic index and atom model.
...
The existing \code{Leant.Synth.Length.*} hierarchy should not be renamed immediately. \code{Behavior.Handoff} can dispatch the Size/Shape-v1 selection to the current Length handoff. Once the generic orchestration proves stable, internal factoring can remove duplication without changing public file versions.
...
The adapter should preserve existing scalar/product nominal distinctions, exact configuration versions, fingerprint bytes, executable policy, refusal constructors and ranking behavior. New selection/CEGIS modes consume the adapter through new code paths; ordinary ranking remains unchanged.
...
Presburger arithmetic & \code{omega}, simp, norm\_num & Natural/integer affine obligations; unfold only named candidate/provider definitions. \\
Bit-vector equality & \code{bv\_decide}, simp & Requires exact width/source semantics. \\
...
The proof runner should retain Leant's present crash/restart and exact environment replay behavior. Proof-state suggestions are not authority; only successful declaration submission produces a receipt.
...
\item explicit expansion or lemmas for monus, min/max, quotient and modulo profiles not directly handled by \code{omega};
...
\code{:behavior-check} performs acquisition, target correspondence and domain sealing without running synthesis or Z3. It reports exact modeled arguments, source families, provider laws, theory profile, proof strategy and all refusals. This prevents expensive debugging through a full synthesis command.
...
\begin{itemize}[leftmargin=2em]
\item all existing Length fingerprints, file versions, refusals and ranking transcripts remain unchanged;
...
These examples are schematic. Production queries should use the typed internal command IR, canonical renderer, exact symbol-role table, and bounded response schema.
...
A model such as \code{input\_0 = 1} is decoded to natural one and replayed by the exact checked Length problem. The \code{define-fun} layout shown here is illustrative; the current canonical query plan may inline terms or use a different symbol policy.
...
A production query should request only values required by its model decoder. Often the domain can replay using equality/disequality observations without decoding the full function interpretation. The response schema must not depend on Z3's arbitrary names for uninterpreted elements.
...
Z3 selects alternative zero. Djex maps selector zero to the exact prepared graph alternative. No source term is reconstructed from \code{eval\_root}.
...
A core \code{(a1 a2)} yields the learned clause $\neg a_1\lor\neg a_2$. The decoder verifies that every returned symbol is one of the exact assumptions issued for this query and canonicalizes the set before creating a nogood receipt.
...
The production protocol must bind objective order and priority mode. An iterative SAT implementation can instead first solve for \code{use\_case = false}, then provider count, then binary-search or linearly tighten node count.
...
In practice a fixedpoint-specific command plan may use \code{declare-rel}, \code{rule}, and \code{query}. The domain owns translation from any returned invariant to a checked arithmetic predicate before Lean proof generation.
...
Spacer safe/unsat observation & CHC bad state not reachable under solver/encoding & Lean induction theorem, termination, exact source semantics & Propose invariant; rank & ``CHC solver found no bad state'' \\
...
\path{src/Leant/Synth/Length/Configuration*.hs} & Versioned execution, replay, applicable-domain and launcher policies. \\
\end{longtable}
...
\begin{itemize}[leftmargin=2em]
\item Is the passive source grammar versioned and bounded before normalization?
...
\bibitem{synthesiz3}
P. Hozzova and N. Bjorner.
\newblock \emph{Synthesiz3 This: an SMT-Based Approach for Synthesis with Uncomputable Symbols}, artifact for an FMCAD 2025 submission.
\newblock \url{https://github.com/Z3Prover/doc/tree/master/synthesiz3}.
