...
\item the engine selection \term{rsSynthEngine} (\term{:set synth-engine}) and Exference's step budget \term{rsSynthSteps} (\term{:set synth-steps}); the queue bound, the candidate window, the verification frontier, and the five-result display quota are compiled-in constants, not settings;
...
\item \term{exferenceMaximumQueueSize}: Leant fixes \term{Just 1024} (Djex's default is 8192) to keep memory interactive; hitting it is reported as \term{queue limit pruned N};
...
The numbers are fixed in \path{src/Leant/Synth/Engine.hs}: a lane hands at most \term{synthMaxTried} $= 12$ fresh groups to Lean (24 in \term{both} mode, so that neither engine's standalone frontier is lost), and \term{synthMaxShown} $= 5$ accepted groups are displayed and bound; the excluded-middle classical lane uses a six-group frontier.  \term{verifyCandidateGroups} in \path{src/Leant/Synth/Verification.hs} is the pure driver and \term{synthVerify} in \path{src/Main.hs} supplies the backend callback.  This avoids eagerly compiling a potentially large candidate stream and gives users five usable answers rather than five attempts.
...
Each setting given without a value prints its current state.  Two environment variables complete the picture: \term{LEANT_SYNTH_TIMEOUT} (seconds, default 20, \term{0} unbounded; it seeds \term{synth-timeout} at startup) and \term{LEANT_SYNTH_DEBUG}.  There is no setting for the number of results (five accepted groups), the candidate window (60), the verification frontier (12 or 24), Exference's queue bound (1024), or the provider cap (80); those are constants of the pinned revision.
...
\item[Candidate window] The at most 60 distinct engine candidates one \term{:synth} lane retains before verification (\term{candidateWindow}); at most five verified ones are shown and bound (\term{synthMaxShown}).
