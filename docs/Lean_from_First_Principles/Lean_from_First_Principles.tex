...
wall-clock deadline & 20\,s (\term{LEANT_SYNTH_TIMEOUT}) & \term{synthTimeoutSeconds} \\
\bottomrule
\end{tabularx}
\end{center}
...
Leant runs several search lanes in a fixed order, all under one command-wide wall-clock deadline (20\,s by default, \term{LEANT_SYNTH_TIMEOUT=N}, \term{0} waits forever):
...
Each setting given without a value prints its current state.  Two environment variables complete the picture: \term{LEANT_SYNTH_TIMEOUT} (seconds, default 20, \term{0} unbounded) and \term{LEANT_SYNTH_DEBUG}.  There is no setting for the number of results (five accepted groups), the candidate window (60), the verification frontier (12 or 24), Exference's queue bound (1024), or the provider cap (80); those are constants of the pinned revision.
