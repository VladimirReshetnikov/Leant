...
\term{finite-spine Length request rejected before IO: ...} & filter mode or a contract path was requested without an activated startup policy & anything about the goal & start Leant with \texttt{-\kern0pt-length-ranking-config} (POSIX only at this commit) \\
\bottomrule
\end{longtable}
...
\item[Length contract] A passive JSON law over the list-spine lengths of a candidate's inputs and result, assessed by the optional Z3-backed stage; \emph{rank} mode reorders, \emph{filter} mode may omit only replayed counterexamples.
