...
\lpath{src/Leant/Synth/Length/Configuration/File.hs} & Large closed versioned JSON grammar. & A versioned Lean decoder with exact object-field rejection and resource bounds. Consider separating stable semantic schema versions from experimental command-line selectors so history is not encoded as one ever-growing decoder function. \\
\lpath{src/Main.hs} & REPL UI, session state, proving, synthesis command orchestration, recovery, and presentation. & Split between \code{Leant.Controller}, \code{Leant.Worker}, and an optional \code{Leant.Command} executable. Native \code{by djex} / \code{#djex} clients call worker-local library APIs and need none of the standalone line editor. \\
\bottomrule
\end{longtable}
...
\begin{itemize}
  \item a tactic such as \code{by djex};
...
\bibitem{djex}
Vladimir Reshetnikov, Lennart Augustsson, Lennart Spitzner, and contributors.
\emph{Djex}.
...
\emph{Elaborators}.
\url{https://lean-lang.org/doc/reference/latest/Elaboration-and-Compilation/Elaborators/}
