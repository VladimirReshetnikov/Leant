...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque reusable execution/replay policy with independent optional origin-probe, finite input-box, complete applicable-domain, and non-vacuous positive-ordering policies; total assembly from validated Djex authorities; sealed digest-expectation presence bridge; and policy-plus-request-contract ranking/assessment entrances for both the scalar and SpinePair domains. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for startup versions 1--16: literal v1--v14 dispatch plus scalar v15 and binary-product v16 strict-relational roots. The later pairs select literal-ceiling, runtime-unscoped usable-work, relational, scoped usable-work, or strict-relational policy without revising older schemas. Its disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
...
\filepath{src/Leant/Synth/Length/Integration.hs} & Opaque disabled/configured mode, startup admission--load--activation setup, retained activation disposition, and one command-local disabled or policy-plus-lazy-contract assessment request. \\
...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, and post-callback integration; default identity, configured sealed ordering, same-receipt counterexample and bounded-positive notes, and sanitized warning-on-fallback presentation. \\
\bottomrule
\end{longtable}
...
The current startup configuration extends through version 16. Versions 7 and 8 introduce the advanced scalar/product root and literal-ceiling positive-affine validation; v9/v10 add the runtime-unscoped v1 shared usable-work object; v11/v12 instead select relational positive-affine validation without that object; v13/v14 combine the relational rule with the owner-thread-affine, cooperatively checkpointed scoped-v2 budget; and v15/v16 preserve the complete v13/v14 root while replacing only the applicable-domain strategy with \code{strict-relational-positive-affine-v1}. That final rule accepts every ordinary relational clause plus exactly one immediate normalized top-level natural complement, \code{not (L <= R)}, as \code{R + 1 <= L}; it is neither general negation normalization nor a claim about source evaluation strictness. Each later decoder is reached only after the complete older chain returns its closed unsupported-version sentinel. Versions 1--6 retain their unchanged eager paths; v7--v16 retain deferred opening, so an all-pure batch opens none and the first live miss opens at most one fresh lexical scope. V13--v16 place preparation, pure ranking, live work, and Leant-owned result forcing beneath one absolute scoped deadline with cooperative phase checkpoints. Applicable-domain notes expose derived maxima, checked/applicable counts, basis, and explicit vacuity; strict simplification metadata stays attached to the exact occurrence; session strategy and deadline observations themselves add no evidence note.
