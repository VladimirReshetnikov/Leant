...
\def\DjexBase{https://github.com/VladimirReshetnikov/Djex/blob/fcc5421bf985dcf39e1e33497b6ce756d4fb07dc/}
\def\LeantBase{https://github.com/VladimirReshetnikov/Leant/blob/07f90778cc2a97d353dc23a486f2ea8c4f82234b/}
...
\textbf{Djex snapshot} & \commitid{fcc5421bf985dcf39e1e33497b6ce756d4fb07dc} \\
\textbf{Leant snapshot} & \commitid{07f90778cc2a97d353dc23a486f2ea8c4f82234b} \\
...
The historical body of this walkthrough predates the typed behavioral-assessment layer. This addendum records the newer layer against the source snapshots named on the title page. Leant remains experimental: configuration numbers, JSON shapes, names, tags, diagnostics, and output are current-tree regression descriptions rather than a public stability or backward-compatibility promise. In particular, ``v33/v34'' below means only the present startup JSON decoder selectors for scalar and nominal product profiles; it is not SemVer, a release number, a negotiated protocol, or a promise to retain those selectors. Paths in this addendum are printed as current-tree file paths rather than through historical line-number references.
...
\begin{enumerate}
  ...
  \item \textbf{The contract is explicit and solver-neutral.} \filepath{src/Leant/Synth/Length/Contract.hs} contains passive source vocabulary: exact Lean names for a unary zero/step spine, pre- and postconditions, optional provider laws, target roles, and a closed candidate-case policy. Leant does not infer that contract from the goal, a declaration name, or an implementation. For each provider law the caller supplies argument roles and a Length transfer expression; the exact closed provider scheme and private identity come from retained translation provenance. Successful sealing establishes consistency of the named objects with the checked handoff. It does \emph{not} establish that a caller-asserted provider law is true of the Lean implementation. The lower-level policy entrance accepts a request-owned contract. Every activated CLI startup version loads one explicit scalar or pair contract and rechecks later candidates against that fixed process-wide assertion; versions 1--3 use the established scalar compatibility grammar, version 4 selects the pair grammar, versions 5 and 6 are their respective positive-ordering successors, and later profiles retain the scalar-v5 or product-v6 contract grammar while changing only checked ranking or execution policy. After activation, \texttt{:synth }\dashdash\texttt{length-contract ABSOLUTE-PATH }\dashdash\texttt{ TYPE} may instead load one bounded contract-only version 1 through version 6 document for that command. Versions 1--5 are scalar. Version 1 retains the compatibility grammar; version 2 adds only \code{["modulo", positiveLiteral, expression]} to conditions and provider transfers. Version 3 retains modulo and requires an ordered \code{targetArgumentRoles} array containing one \code{observed-spine} or \code{unobserved-target} entry for each physical target argument. Version 4 additionally requires \code{candidateCasePolicy = exact-spine-zero-step-v1}. Version 5 retains roles and modulo, requires a case policy chosen from exactly \code{cases-rejected} and \code{exact-spine-zero-step-v1}, and adds \code{["quotient", positiveLiteral, expression]}; quotient therefore grants no case authority by itself. Version 6 is the separately typed canonical-\code{Prod} contract, requires the closed binary-product result shape, and retains version 5's arithmetic, roles, and case-policy vocabulary. No role, case policy, result shape, or domain is inferred. Observed roles receive compact \code{LengthInput} indices; provider transfers retain physical provider-argument indices, so a map-shaped target can forward physical argument 1 to compact input 0 while carrying argument 0 opaquely. An unobserved role means only that the Length interpreter treats that target argument as opaque and non-inspectable; it proves nothing about inhabitance, evaluation, purity, totality, parametricity, strictness, or effects. The exact case policy likewise grants bounded model interpretation, not source termination, equivalence, proof, or pruning authority. The command reuses the already activated policy and cannot replace executable, pin, solver, artifact, replay, or ordering authority. The opaque Integration request retains the strict policy beside the lazy contract and stays on the command stack through ordinary, retry, provider, and classical lanes rather than entering REPL state, history, snapshots, or a cache. No second generic configuration aggregate or configured runner owns a duplicate pairing.
  ...
  After the occurrence seal, \filepath{src/Leant/Synth/Length/Presentation.hs} traverses each whole materialized ranked receipt and derives one opaque candidate-text-plus-note value. Main uses that one ordered list for Lean bindings, splices, and \code{itN} output; it never zips a detached evidence list back to candidates by spelling or index. Independently replayed counterexamples and independently completed bounded-positive input-box receipts produce subordinate notes in both the scalar and canonical-\code{Prod} domains. A counterexample note calls the receipt replayed and model-relative and renders a bounded summary of observed input and result spine lengths. A bounded-positive note instead calls the finite input box independently checked and bounded/model-relative, renders its inclusive input maxima and checked and applicable assignment counts, and states explicitly when zero applicable assignments make the result vacuous within that box. Both note forms reduce a provider-backed basis to the number of assumed laws. Neither projects the receipt's private provider-name list, attributes the positive result to raw \code{unsat}, or makes a Lean, proof, correctness, or Z3-attestation claim.

  \item \textbf{Assessment may reorder but never prune.} The package-private implementation in \filepath{src/Leant/Synth/Length/Ranking/Internal.hs} first productively admits at most 64 candidates and completes all pure candidate/query preparation before opening a worker. Under the historical/default ordering policy, every non-counterexample---including an independently completed bounded-positive receipt---retains its relative position in one neutral stable partition, while independently replayed counterexamples follow in a stable demoted partition. The separately enabled \code{enableLengthRankingNonVacuousInputBoxPreference} policy, selected by startup versions 5 through 34, instead applies one stable trichotomy after successful assessment: bounded-positive receipts whose applicable-assignment count is greater than zero; all neutral candidates, including unassessed candidates, heuristic statuses, and zero-applicable vacuous bounded positives; then independently replayed counterexamples. A sibling \code{enableLengthRankingNonVacuousApplicableDomainPreference} policy applies the analogous stable preference to independently completed applicable-domain receipts. Direct-v1 validation and that preference remain programmatic through startup version 6; advanced v7--v34 require the same preference with their nominal literal-ceiling, relational, strict-relational, strict-relational root-quotient, root-extrema, root-monus, Boolean finite-union, atomic-branching, or recursive piecewise-affine successor validator. Composing both preferences yields applicable-domain positive, explicit-box positive, neutral or vacuous, then counterexample order. Contract-only files select no ranking policy. Original order is retained inside every partition and every callback receipt remains present. Any returned session, query, fingerprint-association, or evidence-replay failure discards partial live assessments and restores literal original order: eligible candidates become \code{Unassessed}, while actual pure preparation refusals retain their bounded refusal class.
  ...
  \item \textbf{Advanced v7/v8 ranking remains additive.} Startup versions 7 and 8 are the scalar and nominal binary-product successors. They retain the v5/v6 contract grammars and require positive-affine applicable-domain validation, its non-vacuous preference, componentwise-lexicographic counterexample simplification, and deferred live-session opening. Contract-only documents remain capped at version 6 and carry no ranking or execution policy. Versions 1--6 remain eager and schema compatible. V7/v8 complete admission and preparation, then run each candidate's MRU $\rightarrow$ positive-affine-domain $\rightarrow$ origin prefix before IO. An all-pure batch opens no process; the first live miss opens one lexical session, executes that head once without repeating its pure prefix, and processes the suffix in the same session. Every counterexample from those pure sources, live replay, or the post-\code{unsat} box crosses the same query-owned simplification seam, and only its final vector enters the domain-local MRU. Stable preference is non-vacuous applicable-domain evidence, non-vacuous explicit-box evidence, neutral and vacuous assessments, then counterexamples. Any indexed failure atomically restores original order and clears earlier evidence and occurrence-attached metadata; a pre-open failure launches nothing.
\end{enumerate}
...
For advanced v7/v8, the same invariant has two positive partitions: a non-vacuous positive-affine applicable-domain receipt precedes a non-vacuous explicit-box receipt; neutral and vacuous assessments follow; counterexamples remain last. Each counterexample has first crossed the query-owned simplification seam, and the final vector is the only vector offered to the domain-local MRU. Deferred lifecycle state is not evidence and cannot enter presentation.
\end{invariantbox}
...
When the replay bank yields no hit---at most four misses---an independently enabled origin policy, selected by startup version 3 and later, may probe the canonical all-zero input vector owned by that exact query. A hit enters the same validated receipt and MRU path; an ordinary miss simply proceeds to live execution and carries no positive or negative authority of its own.

Applicable-domain validation is the second positive axis beside the finite input box: an independent, query-owned exhaustive traversal that establishes the precondition-applicable input domain, sealed in Djex under nominal direct-v1, literal-ceiling positive-affine, relational positive-affine, strict-relational positive-affine, strict-relational positive-affine root-quotient, cumulative root-extrema, cumulative root-monus, Boolean finite-union, atomic-branching, or recursive piecewise-affine receipt schemas for the scalar or binary-product domain. Its receipts feed the separately selected non-vacuous applicable-domain preference---programmatic through startup version 6 and required by v7--v34. Contract-only files never select that preference, and, like the input box, the external solver status itself never becomes the evidence.
...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque reusable execution/replay policy with independent optional origin-probe, finite input-box, complete applicable-domain, and non-vacuous positive-ordering policies; total assembly from validated Djex authorities; the cumulative root-extrema, root-monus, Boolean finite-union, atomic-branching, and recursive piecewise-affine applicable-domain builders; sealed digest-expectation and four-way executable-launch classifiers; literal legacy, descriptor-bound, descriptor-bound effective-ID executable-access, and descriptor-bound execve-check executable-access constructors; and policy-plus-request-contract ranking/assessment entrances for both the scalar and SpinePair domains. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for current startup selectors 1--34: the v1--v32 cascade through scalar v31 and product v32 atomic branching, followed by scalar v33 and product v34 recursive piecewise-affine roots. The latest pair retains the complete v31/v32 schema and changes only version and applicable-domain strategy; v35 is the current unsupported sentinel. These are experimental decoder discriminators, not a compatibility promise. Its disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
...
\filepath{src/Leant/Synth/Length/Configuration/File/Acquire.hs} & Startup-configuration facade over shared acquisition; no discovery or activation. \\
\filepath{src/Leant/Synth/Length/Contract/File.hs} & Exact contract-only versioned root: unchanged v1, v2 positive-literal Natural modulo, v3 required ordered target roles, v4 explicit exact zero/step cases, v5 explicit policy plus positive-literal Natural quotient, and v6 the separately typed canonical-\code{Prod} binary-product form, with shared limits and no execution-policy fields. \\
...
\filepath{src/Leant/Synth/Length/Presentation.hs} & Opaque same-ranked-receipt candidate text and optional bounded, provider-name-free counterexample, independently checked input-box, or applicable-domain note, including the nominal scalar and SpinePair root-extrema, root-monus, Boolean finite-union, atomic-branching, and recursive piecewise-affine renderers; no detached evidence reassociation. \\
...
With that policy activated, \clicode{:synth --length-contract ABSOLUTE-PATH -- TYPE} admits and reads a separate contract-only version 1 through version 6 document with format \code{leant-finite-list-spine-length-contract} once before goal translation. The mandatory standalone delimiter leaves \code{TYPE} opaque. Versions 1--5 are scalar: version 1 reuses the exact bounded compatibility grammar; version 2 adds positive-literal Natural modulo while retaining the same limits and root; version 3 retains modulo and requires the ordered target-role vector described above; version 4 retains roles and modulo and requires the exact zero/step candidate-case policy; and version 5 requires roles and an explicit choice between case rejection and that exact policy and adds positive-literal Natural quotient. Version 6 is the separately typed canonical-\code{Prod} form: it requires the closed binary-product result shape and retains version 5's arithmetic, roles, and case-policy vocabulary. Contract-only versions 1--4 and scalar startup versions 1--3 reject quotient. None of the contract-only documents contains execution or evaluation fields. The startup configuration itself currently admits integer selectors 1 through 34. Those numbers choose exact decoder branches in this tree; they do not promise compatibility or cumulative feature levels. Versions 1--6 are the eager early series: version 1 is the baseline scalar path; version 2 additionally requires an explicit \code{inputBoxValidation} finite input-box policy; version 3 additionally requires the \code{counterexampleProbe} field selecting the origin-probe mode; version 4 selects the pair path while repeating version 3's operational fields; and versions 5 and 6 are respectively the scalar and pair successors that additionally require \code{boundedPositiveOrdering} to select the non-vacuous-positive preference. Versions 7 through 34 are fourteen scalar/product advanced pairs: literal-ceiling positive-affine, its runtime-unscoped shared-budget sibling, relational without a shared budget, relational with the scoped-v2 owner, strict-relational with that same scoped owner, descriptor-bound sealed-main-image launch with strict-relational validation, strict-relational root-quotient validation under that descriptor/scoped profile, effective-ID executable-access descriptor launch with the quotient/scoped profile otherwise unchanged, cumulative root-extrema validation, cumulative root-monus validation, descriptor-bound execve-check executable-access launch under the monus/scoped profile otherwise unchanged, bounded Boolean finite-union validation under that same execve-check/scoped profile, its atomic-branching successor, and the recursive piecewise-affine successor under the identical profile. They retain the v5/v6 contract grammars and the applicable advanced root selections described below. That startup selection fixes the process-wide policy and default contract; a contract-only file replaces only the contract for one command. The decoded command contract is associated with the already activated policy in one opaque request and reused by every verification batch and retry lane for that command. It is never written to \code{ReplState}, \code{ParsedGoal}, history, snapshots, or a cache; the next command returns to the startup-fixed contract unless it names another file.

When configured, \code{Leant.Synth.Length.Integration} gives the exact callback-owned \code{VerificationBatch} to the existing occurrence-sealed adapter. Only candidates with direct or exact-duplicate-recovered typed Exference authority whose retained graph and provenance match the startup contract are eligible; candidates with neither authority and mismatched candidates remain \code{Unassessed}. The default \code{djinn} engine supplies no typed graph, so actual eligibility requires selecting \code{exference} or \code{both}. Under eager startup versions 1--6, each batch with an eligible query opens a fresh lexical worker scope. Under deferred v7--v34, an all-pure batch opens none and the first actual live miss opens the single fresh scope used by the remaining suffix. V9/v10 place admitted preparation and ranking beneath the runtime-unscoped shared owner; v13--v34 use the owner-thread-affine scoped-v2 lease and cooperative phase checkpoints. Structured adapter or live failures retain closed diagnostics and atomically preserve callback order; synchronous and asynchronous exceptions continue to propagate. Presentation uses the proposed order only after the exact occurrence permutation seal succeeds. A successfully replayed counterexample receives the bounded model-relative note described above; an independently completed finite input box or applicable domain receives its bounded-positive note, including checked and applicable counts and an explicit vacuity statement when none was applicable. Disabled, rejected, unassessed, heuristic-only, and atomic-fallback paths add no semantic note. The CLI caps at eight inputs, 256 provider laws, and 4,096-bit evaluation values; large naturals become bit-length summaries, and neither note proves nor prunes.

There is still no configuration discovery, inferred contract, cached or long-lived Z3 worker, pruning, automatic trust in \code{sat}/\code{unsat}/\code{unknown}, or general constraint language. The one-command choice is an explicit file, not a grammar embedded in Lean goal text or a policy override. Broader behavioral domains should be added as new checked domain adapters with their own semantics and replay authorities, not by treating \code{finite-list-spine-length/v1} as generic.

The current startup configuration admits experimental selectors through version 34. Versions 7 and 8 introduce the advanced scalar/product root and literal-ceiling positive-affine validation; v9/v10 add the runtime-unscoped v1 shared usable-work object; v11/v12 instead select relational positive-affine validation without that object; v13/v14 combine the relational rule with the owner-thread-affine, cooperatively checkpointed scoped-v2 budget; and v15/v16 preserve the complete v13/v14 root while replacing only the applicable-domain strategy with \code{strict-relational-positive-affine-v1}. The strict rule accepts every ordinary relational clause plus exactly one immediate normalized top-level natural complement, \code{not (L <= R)}, as \code{R + 1 <= L}; it is neither general negation normalization nor a claim about source evaluation strictness. V17/v18 preserve the complete v15/v16 behavioral and scoped-budget profile while adding only \code{descriptor-bound-executable-v1}. V19/v20 retain that launch/scoped profile and replace only the applicable-domain strategy with \code{strict-relational-positive-affine-quotient-v1}. V21/v22 retain the complete v19/v20 policy and replace only the launch literal with \code{descriptor-bound-effective-id-executable-access-v1}. The opened source must pass the two point-in-time effective-filesystem-credential VFS access checks, while the sealed memfd remains fixed \code{0500} main-image-byte transport; there is no older-launch fallback and no claim over full source exec security, loaders, libraries, or solver semantics.

V23/v24 retain that complete effective-ID launch, scoped-v2, deferred-opening, non-vacuous preference, simplification, and scalar-v5/pair-v5 contract profile. They replace only the applicable-domain strategy with \code{strict-relational-positive-affine-quotient-root-extrema-v1}, selected programmatically by \nolinkurl{enableLengthRankingStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidation}. For positive-affine operands (A), (B), and (C), the cumulative extractor adds exactly four necessary rule pairs:

\begin{itemize}
  \item \code{max(A,B) <= C} gives \code{A <= C} and \code{B <= C};
  \item \code{C <= min(A,B)} gives \code{C <= A} and \code{C <= B};
  \item \code{not (min(A,B) <= C)} gives \code{C + 1 <= A} and \code{C + 1 <= B}; and
  \item \code{not (C <= max(A,B))} gives \code{A + 1 <= C} and \code{B + 1 <= C}.
\end{itemize}

Equality is necessary-only in both orientations: maximum equality emits only the first pair and minimum equality only the second, never a converse. Exactly one immediate binary extremum may occur at one relation operand root. Nested or effectively n-ary extrema, extrema at both roots, mixed roots, embedded extrema, wrong or disjunctive orientations, negated equality, unsupported operands, and unsupported whole formulas yield no partial rule; both rules are emitted atomically or the clause is ignored. Root-extrema-free clauses delegate literally to the quotient predecessor. Accepted clauses and extrema operands retain normalized canonical order through the immutable-snapshot, merge-after-pass rule-once closure. Contradiction precedes missing coverage; otherwise the first compact missing input wins. The original normalized formula remains the replay authority.

Only complete query-owned replay releases \nolinkurl{StrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainEstablished} or its \code{LengthSpinePair} sibling. Presentation uses \nolinkurl{renderLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationNote} or its \code{LengthSpinePair} sibling. Versions 1--22 remain literal in this tree, and v23/v24 are reached only after their complete decoder cascade returns its unsupported-version sentinel; v25/v26 are handled by the later root-monus decoder, v27/v28 by the still-later execve-check launch decoder, v29/v30 by the Boolean finite-union successor, v31/v32 by atomic branching, and v33/v34 by recursive piecewise-affine branching. Versions 1--6 retain eager opening, while v7--v34 retain deferred opening. V13--v34 keep preparation, pure ranking, live work, and Leant-owned deep result forcing beneath one absolute scoped deadline with cooperative checkpoints. An all-pure batch still captures and checks that deadline clock but performs no executable or access-check IO. Existing access, staging, live, replay, forcing, and finalization failures atomically restore original order and create no partial receipt or fallback. The new receipt tags are evidence and presentation distinctions only: problem, query, wire, fingerprint, association, executable, and scoped-owner identities remain unchanged.

V25/v26 retain the complete v23/v24 effective-ID launch, scoped-v2, deferred-opening, non-vacuous preferences, simplification, and scalar-v5/pair-v5 contract profile. They replace only the applicable-domain strategy with \code{strict-relational-positive-affine-quotient-root-extrema-monus-v1}, selected by \nolinkurl{enableLengthRankingStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidation}. Let \code{M = A monus B = max(A-B,0)} and let the opposite exact positive-affine summary be \code{C = c + sum(k_i*x_i)}. The cumulative scanner adds only these five normalized cases:

\begin{itemize}
  \item \code{M <= C} is exactly \code{A <= B+C};
  \item \code{C <= M} gives the exact \code{B+C <= A} when \code{c>0}; an identically-zero \code{C} emits nothing, while a may-zero affine \code{C} is an unsupported union and the whole clause is ignored;
  \item \code{not (M <= C)} is exactly \code{B+C+1 <= A};
  \item \code{not (C <= M)} atomically emits \code{1 <= C} and then \code{A+1 <= B+C}; and
  \item \code{M=C} or \code{C=M} always emits the necessary \code{A <= B+C}, appending \code{B+C <= A} only when \code{c>0}.
\end{itemize}

The reverse non-strict law is unconditionally \code{C <= M iff C=0 or B+C<=A}; no cross-clause positivity, weak \code{C<=A} replacement, or componentwise widening hides that union. Equality's first rule is exact \code{A<=B} for an identically-zero opposite but necessary-only for a may-zero affine opposite. The scanner summarizes all three operands before retaining any rule. Exactly one retained immediate root monus may occur on one relation side. Both-root, nested, embedded, mixed monus/quotient/extrema, unsupported-child, negated-equality, and nested-Boolean shapes grant no partial rule. Normalization may fold literal monus, \code{A monus 0}, or \code{A monus A} before the scanner. The inherited canonical-order, immutable-snapshot, rule-once closure still reports contradiction before the first compact missing input and replays the original normalized formula exhaustively.

Only complete query-owned replay releases \nolinkurl{StrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainEstablished} or its nominal \code{LengthSpinePair} sibling. Presentation uses \nolinkurl{renderLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationNote} or its \code{LengthSpinePair} sibling, capped at 384 characters and carrying no solver or global authority. The generalized decoder reaches v25/v26 only after v1--v24 report their closed unsupported-version sentinel; v27/v28 are handled only by the later execve-check launch decoder, v29/v30 by the Boolean finite-union successor, v31/v32 by atomic branching, and v33/v34 by recursive piecewise-affine branching. Failure preserves the inherited atomic original-order fallback, and the additive scalar/product receipt tags revise no contract, problem, query, wire, fingerprint, association, executable, worker, run, or scoped-owner identity.

V27/v28 retain the complete v25/v26 root-monus, scoped-v2, deferred-opening, preference, simplification, and scalar-v5/pair-v5 contract profile. They replace only \code{execution.executableLaunch} with \code{descriptor-bound-execve-check-executable-access-v1}, selected programmatically by \nolinkurl{mkLengthRankingPolicyWithDescriptorBoundExecveCheckExecutableAccessLaunch}. The public version constants are \nolinkurl{lengthRankingConfigurationFileDescriptorBoundExecveCheckExecutableAccessVersion} (27) and \nolinkurl{lengthRankingConfigurationFileSpinePairDescriptorBoundExecveCheckExecutableAccessVersion} (28). The opened source descriptor must pass \code{faccessat2} and \code{AT\_EXECVE\_CHECK} once before copying and again after the deterministic hook immediately before child allocation. The staged main image uses \code{MFD\_EXEC}, fixed mode \code{0500}, and verified write/grow/shrink/future-write/exec/final seals; it then passes a third execve-check immediately before child allocation. A stock Linux 5.15 runtime fails closed as launch unavailable at the first source execve-check, while a compatible Linux 6.14-or-newer kernel may admit the profile. There is no pathname, older-launch, flag, or seal fallback. Configuration loading, activation, and an all-pure deferred batch perform no executable or access-check IO. The new launch policy, raw-process, ready-worker, and fresh/shared/scoped scalar and product run identities are domain-separated, while the v25/v26 contract, problem, query, wire, association, behavioral receipt, and presentation identities remain literal. The checks are point-in-time executable-access observations, not reservations or claims over format/interpreter dependencies, source metadata transfer, binfmt/bprm/credential transition, loaders, libraries, solver semantics, or reported results. Current later selectors route v29/v30 through the Boolean finite-union decoder, v31/v32 through atomic branching, and v33/v34 through recursive piecewise-affine branching.

V29/v30 retain that complete execve-check launch, scoped-v2, deferred-opening, preference, simplification, and scalar-v5/pair-v5 contract profile. They replace only the applicable-domain strategy with \code{strict-relational-positive-affine-quotient-root-extrema-monus-boolean-finite-union-v1}, selected programmatically by \nolinkurl{enableLengthRankingStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainValidation}. Their public constants are \nolinkurl{lengthRankingConfigurationFileStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionVersion} (29) and its \code{LengthSpinePair} sibling (30). The closed object demands exactly strategy, maximum inputs, generated branches, rules per branch, closure inspections per branch, retained boxes, assignment visits, and unique assignments; their numeric caps are 8, 256, 64, 4096, 256, 262144, and 65536. The generalized decoder reaches these roots only after v1--v28 return the closed unsupported-version sentinel; v31/v32 are handled by the later atomic-branching decoder and v33/v34 by its recursive piecewise-affine successor. V35 is the current unsupported sentinel.

Proof-polarity DNF applies De Morgan traversal to checked conjunction and negation and splits negated equality exactly into two strict branches. Branch canonicalization removes duplicates, literal complements, and strict literal-set supersets before each branch delegates to the unchanged root-monus scanner and bounded rule-once closure. Completely bounded branches become a canonical antichain of zero-origin boxes. Componentwise-contained boxes disappear, but incomparable \code{[1,3]} and \code{[3,1]} remain separate: their 16 raw visits deduplicate to 12 global assignments and never widen to \code{[3,3]}. One set-backed lexicographic replay evaluates the original checked formula once per unique assignment. Empty union and nullary truth remain distinct.

Width, branch, rule, closure, retained-box, value, visit, and unique-assignment caps are ordinary candidate admission misses. Assignment-evaluation and internal-enumeration failures and association mismatch remain indexed atomic failures; every failure route restores literal original order. Non-vacuous positive receipts participate in the existing applicable-domain preference, vacuous receipts remain neutral, counterexamples pass through simplification, and only the final counterexample vector enters the four-entry MRU bank. Complete replay releases the scalar \nolinkurl{StrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainEstablished} or nominal pair sibling and their exact Boolean finite-union renderers. Notes are capped at 384 characters and report box, raw-visit, unique, and applicable counts plus a bounded box prefix without claiming a hull, proof, solver status, or pruning authority.

Every configured batch still captures and checks the inherited scoped deadline clock. An all-pure batch performs no executable, access-check, staging, or worker IO; a later live miss retains v27/v28 literally. The new receipt tags are evidence and presentation distinctions only. Contract, problem, query, wire, fingerprint, association, execution-policy, process, ready-worker, query-run, and scoped-owner identities do not change.

V31/v32 retain the complete v29/v30 root, eight-field applicable-domain object, execve-check launch, scoped-v2 owner, deferred lifecycle, preferences, simplification, and scalar-v5/pair-v5 contract. They replace only the strategy with \nolinkurl{strict-relational-positive-affine-quotient-root-extrema-monus-boolean-finite-union-atomic-branching-v1}, selected by \nolinkurl{enableLengthRankingStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionAtomicBranchingApplicableDomainValidation}. Their public constants are \nolinkurl{lengthRankingConfigurationFileStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionAtomicBranchingVersion} (31) and its \code{LengthSpinePair} sibling (32). The numeric caps remain 8, 256, 64, 4096, 256, 262144, and 65536; only the generated-branch limit now counts the complete formula-DNF by per-atom-alternative Cartesian product before canonical cleanup. The generalized decoder reaches these roots only after v1--v30 return their closed unsupported-version sentinel; v33/v34 are handled by the recursive piecewise-affine successor, and v35 is the current unsupported sentinel.

For normalized positive-affine root children \code{A} and \code{B} and opposite affine operand \code{C}, the exact first-child--then-second-child extremum alternatives are:

\begin{itemize}
  \item \code{C <= max(A,B)} gives \code{[C<=A] | [C<=B]};
  \item \code{min(A,B) <= C} gives \code{[A<=C] | [B<=C]};
  \item \code{not(max(A,B)<=C)} gives \code{[C+1<=A] | [C+1<=B]};
  \item \code{not(C<=min(A,B))} gives \code{[A+1<=C] | [B+1<=C]};
  \item \code{max(A,B)=C} gives \code{[A<=C,B<=C,C<=A] | [A<=C,B<=C,C<=B]}; and
  \item \code{min(A,B)=C} gives \code{[C<=A,C<=B,A<=C] | [C<=A,C<=B,B<=C]}.
\end{itemize}

The root may occur on either equality side, each within-alternative rule sequence stays literal, and the already exact predecessor extrema orientations remain singleton alternatives. For \code{M=A monus B=max(A-B,0)} and nonconstant affine \code{C} which may be zero, \code{C<=M} gives the zero-first alternatives \code{[C<=0] | [B+C<=A]}; either equality orientation gives \code{[A<=B+C,C<=0] | [A<=B+C,B+C<=A]}. The common equality consequence remains first and the zero branch is not simplified to \code{A<=B}. Constant-positive and identically-zero opposites retain predecessor behavior. Both-root, nested, embedded, mixed, effectively n-ary, conditional, unsupported-child, and unsupported opposite-operand shapes are ignored atomically after all required operands are summarized.

Only after raw-product admission does Djex canonicalize sets of original checked literals, then re-expand each surviving set in set order into explicit ignored, contradictory, or ordered-rule alternatives. It manufactures no replacement formula and deduplicates no proof-rule set. The existing rule ceiling remains 64: extremum equality observes three rules against a cap of two, monus equality observes two against a cap of one, and the first default excess observes 65. Closure, box antichain construction, and one global lexicographic replay of the original formula are literal predecessors. Thus \code{min(x,y)<=1} with \code{x<=3,y<=3} retains boxes \code{[[1,3],[3,1]]}, 16 visits, and 12 unique/applicable assignments rather than hull \code{[3,3]}; the characterized monus relation retains \code{[[0,4],[3,3]]}, 21 visits, 17 unique assignments, and 11 applicable assignments, with five applicable under equality.

Complete replay releases \nolinkurl{StrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionAtomicBranchingApplicableDomainEstablished} or its nominal \code{LengthSpinePair} sibling and the two matching atomic-branching renderers. Leant reuses the existing Boolean finite-union ranking failures and admission classifiers. Non-vacuous preference, vacuous neutrality, simplification/MRU, scoped deadline checks, deferred all-pure zero-worker behavior, deep forcing, and original-order atomic fallback are unchanged. The fresh receipt tags distinguish evidence and presentation only: they revise no contract, problem, query, SMT-LIB, fingerprint, association, executable, process, worker, fresh/shared/scoped run, or scoped-owner identity and grant no hull, solver proof, global theorem, or pruning authority. The precise Leant boundary is recorded in \filepath{docs/reports/2026-08-15-atomic-branching-length-ranking.md}; Djex's laws and precedence are in \filepath{lib/Djex/docs/reports/2026-08-15-atomic-branching-length-applicable-domain.md}.

V33/v34 are the current scalar and nominal product selectors for recursive piecewise-affine branching. They retain the complete v31/v32 root, eight-field applicable-domain object, execve-check launch, scoped-v2 owner, deferred lifecycle, preferences, simplification, limits, and scalar-v5/pair-v5 contract. Only the integer version and applicable-domain strategy change, to \nolinkurl{strict-relational-positive-affine-quotient-root-extrema-monus-boolean-finite-union-atomic-branching-recursive-piecewise-affine-v1}. The public builder is \nolinkurl{enableLengthRankingStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionAtomicBranchingRecursivePiecewiseAffineApplicableDomainValidation}. As with all startup numbers in this experimental tree, v33/v34 are decoder discriminators rather than compatibility commitments.

Each signed relation leaf first uses the complete atomic predecessor. Recursive interpretation is attempted only for an exact singleton ignored result that still contains minimum, maximum, or monus somewhere inside a relation operand. The fallback recursively expands compact inputs, literals, normalized sums, retained positive scales, binary extrema, and binary monus into ordered guarded signed-affine values. Any required quotient, modulo, conditional, result reference, out-of-range variable, retained zero scale, or other unsupported descendant rejects the complete fallback atom.

For selected child values \code{L} and \code{R}, the exact disjoint cases are:
...
The first choice owns equality. Descendant guards precede the current selector, left-child cases precede right-child cases, and the relation appends its at-most, immediate strict-complement, or two equality rules last. Signed coefficients created by the positive monus branch are transferred exactly across the inequality into the established natural positive-sided rule representation. No checked formula is manufactured and no selector or relation rule is deduplicated. This admits nested, embedded, both-root, mixed, and normalized effectively n-ary extrema/monus shapes inside the closed grammar without claiming recursive quotient, modulo, conditional, result-reference, or general nonlinear reasoning.

The generated-branch cap observes the complete formula-DNF by recursive-alternative Cartesian product before formula cleanup, guard contradiction, proof-rule collection, closure, or box cleanup. Original checked literal sets are then canonicalized and re-expanded in set order. The existing immutable-snapshot rule-once closure, zero-origin box antichain, raw visits, globally deduplicated lexicographic assignments, and exact original-formula replay remain authoritative. Incomparable boxes are never replaced by a componentwise hull.

The scalar characterization of \code{max(x,y) <= 3 monus min(x,y)} with \code{x<=3,y<=3} retains \code{[[2,3],[3,2]]}, two boxes, 24 visits, 15 unique assignments, and 10 applicable assignments; the v31 control retains \code{[[3,3]]} with 1/16/16/10. The Leant scalar candidate deliberately carries one assumed provider law, so its rendered basis is conditional on that law. The product characterization builds \code{u=min(x,y)+(x monus y)} and \code{v=min(x,y)+(y monus x)} beneath \code{max(u,v)<=2}; v34 retains provider-independent \code{[[2,2]]} with 1/9/9/9, while v32 retains \code{[[3,3]]} with 1/16/16/9. Its 32 raw alternatives distinguish generated-branch caps 31 and 32 before contradictory cases disappear.

Complete replay releases \nolinkurl{StrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionAtomicBranchingRecursivePiecewiseAffineApplicableDomainEstablished} or its nominal \code{LengthSpinePair} sibling. The matching 384-character renderers report model/provider-relative basis, boxes, raw visits, unique and applicable assignments, explicit vacuity, and a bounded maxima prefix. Leant reuses the Boolean finite-union limits, errors, admission classifications, and indexed failure routes. Non-vacuous preference, simplification/MRU, scoped forcing, deferred zero-worker pure completion, live execve-check fallback, and atomic original-order recovery are unchanged. The exact Leant boundary is in \filepath{docs/reports/2026-08-15-recursive-piecewise-affine-length-ranking.md}; Djex's recursive grammar, selector order, signed transfer, raw accounting, no-hull replay, and receipt authority are in \filepath{lib/Djex/docs/reports/2026-08-15-recursive-piecewise-affine-length-applicable-domain.md}.
...
\begin{itemize}
  \item Djex pinned tree: \href{https://github.com/VladimirReshetnikov/Djex/tree/fcc5421bf985dcf39e1e33497b6ce756d4fb07dc}{\nolinkurl{github.com/VladimirReshetnikov/Djex/tree/fcc5421...}}
  \item Leant pinned tree: \href{https://github.com/VladimirReshetnikov/Leant/tree/07f90778cc2a97d353dc23a486f2ea8c4f82234b}{\nolinkurl{github.com/VladimirReshetnikov/Leant/tree/07f9077...}}
