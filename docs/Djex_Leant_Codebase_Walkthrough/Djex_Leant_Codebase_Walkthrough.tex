...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque reusable execution/replay policy with independent optional origin-probe, finite input-box, complete applicable-domain, and non-vacuous positive-ordering policies; total assembly from validated Djex authorities; the cumulative root-extrema and root-monus applicable-domain builders; sealed digest-expectation and four-way executable-launch classifiers; literal legacy, descriptor-bound, descriptor-bound effective-ID executable-access, and descriptor-bound execve-check executable-access constructors; and policy-plus-request-contract ranking/assessment entrances for both the scalar and SpinePair domains. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for startup versions 1--28: literal v1--v26 compatibility dispatch plus scalar v27 and binary-product v28 execve-check launch roots. The version pairs select literal-ceiling, runtime-unscoped usable-work, relational, scoped usable-work, strict-relational, root-quotient, root-extrema, root-monus, sealed-main-image, two-point source-access, or execve-check policy without revising older schemas; v29 remains unsupported. Its disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
