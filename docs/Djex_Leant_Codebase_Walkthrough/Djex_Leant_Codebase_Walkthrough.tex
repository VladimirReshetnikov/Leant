...
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/SMTLib/Z3/Process.hs} & Opaque raw Z3 owner with the established pathname-snapshot launcher, sealed-main-image branch, and effective-ID executable-access sibling: two raw descriptor-based \code{faccessat2} observations, bounded one-pass source hash/copy, optional pin admission, fixed staged \code{0500}, verified anonymous-file seals, descriptor-only execution, signal/process-group ownership, and fail-closed cleanup without pathname or strategy retry. \\
...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque reusable execution/replay policy with independent optional origin-probe, finite input-box, complete applicable-domain, and non-vacuous positive-ordering policies; total assembly from validated Djex authorities; the cumulative root-extrema and root-monus applicable-domain builders; sealed digest-expectation and three-way executable-launch classifiers; literal legacy, descriptor-bound, and descriptor-bound effective-ID executable-access constructors; and policy-plus-request-contract ranking/assessment entrances for both the scalar and SpinePair domains. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for startup versions 1--26: literal v1--v24 compatibility dispatch plus scalar v25 and binary-product v26 root-monus applicable-domain roots. The version pairs select literal-ceiling, runtime-unscoped usable-work, relational, scoped usable-work, strict-relational, root-quotient, root-extrema, root-monus, sealed-main-image, or two-point source-access policy without revising older schemas; v27 remains unsupported. Its disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, and post-callback integration; default identity, configured sealed ordering, three-way descriptor-launch selection notices without premature access or pin-match claims, same-receipt counterexample and bounded-positive notes, and sanitized warning-on-fallback presentation. \\
\bottomrule
\end{longtable}
