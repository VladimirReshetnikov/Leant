...
\begin{enumerate}
  ...
  Leant's \filepath{src/Leant/Synth/Engine.hs} first builds one prepared semantic-origin record. Search, renderer inputs, retained Exference authority, and later exact-origin rerendering project that same record; no post-search field-for-field preparation copy exists. Each provider binding is the sole prepared owner of its private declaration inputs, renderer metadata, and ordered instantiation assignments. Preparation derives the provider declaration list and one lazy strict-map projection shared by both renderer closures; later exact-origin rerendering and inspection derive that same provider-map projection at their use edges. Djinn and Exference derive the aggregate assignment list in binding order, while inspection projects the same compatibility view. The prepared semantic origin does not cache an aggregate assignment list or provider renderer map beside the bindings. The engine retains the typed candidate in a \code{TypedCandidateSemanticSidecar} together with the exact Exference inventory/session, prepared origin, source-name table, session policy, request, and converted Exference assignments. It does not cache a converted source goal beside the prepared source goal and name table: the package-private inspection view derives its historical projection lazily, and the checked handoff performs a total conversion at its use edge. It keeps no parallel graph-key cache: the exact graph remains owned by the opaque checked candidate, and Djex fingerprints it only while sealing an actual behavioral problem. Candidate grouping and renderer filtering preserve a group sidecar only while the group still denotes the same engine candidate. During combined-engine exact-text deduplication, an earlier compatibility-only spelling may instead retain a private variant-scoped witness to the first bounded later Exference occurrence with identical bytes. That witness keeps the Exference sidecar and source renderer ordinal separate from the display group's route, display ordinal, ordering, sidecar, and sibling variants. At the verification boundary, the display route, ordinal, and text are flattened into the callback candidate, which retains that spelling's exact typed origin once rather than keeping the scheduling-only variant and a parallel origin field. A wrapper such as the classical prove-mode transformation explicitly drops both direct and recovered authority.
