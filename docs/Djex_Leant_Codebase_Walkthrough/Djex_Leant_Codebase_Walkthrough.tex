...
The historical body of this walkthrough predates the typed behavioral-assessment layer. This addendum records the newer layer against the source snapshots named on the title page. Leant remains experimental: JSON shapes, names, tags, diagnostics, and output are current-tree descriptions rather than a public stability or backward-compatibility promise. The startup configuration and command-local contract file now each have one versionless schema whose explicit \code{rankingDomain} is \code{scalar} or \code{binary-product}; older numbered roots survive only in Git history and dated non-normative reports. Paths in this addendum are printed as current-tree file paths rather than through historical line-number references.
  ...
  The headline is intentionally modest: the repositories now have a checked insertion seam for one finite-list-spine Length model, a scoped Z3 execution boundary, an explicit startup policy opt-in, an optional one-command contract choice, and an explicit per-command choice between stable ranking and replay-authorized hard filtering in \filepath{src/Main.hs}. Without the startup opt-in the ordinary no-option and explicit-rank commands remain the exact non-strict identity path, while a filter request or requested command contract is refused before file IO.

  \textbf{What one command actually does.} Main introduces one nominal rank-2 context before translation and reuses its filter bank across the complete command. It now consumes Engine's opaque candidate cursor directly: one ordinary filter run may assess two ordered standalone or combined batches from the same lazy engine outcome, while excluded middle remains one half-width batch. Under the default session limits those widths are a 60-group cursor and 12-group, 24-group, and six-group batches; every one of them is a \code{:set} away from being something else (Appendix~\ref{app:limits}), so the numbers quoted throughout this addendum describe an unmodified session. Only no verification or complete behavioral rejection reaches the single successor; survivor and preserve-all stop, and Main never probes a third tail. Djex supplies the candidate-independent bounded replay-input bank and exact query-owned fresh replay. Established configured-rank and direct Selection compatibility paths retain their raw four-vector MRU; rank and disabled commands retain one engine batch. The scheduler keeps one lazy reverse outcome accumulation and finalizes it once.
...
\filepath{src/Leant/Synth/Length/Command.hs} & Exact fixed-order \code{:synth [-{}-behavior-mode rank|filter] [-{}-length-contract PATH] -{}- TYPE} parser, with ordinary no-option goals left delimiter-free and opaque. A separate entrance parses the inline \code{-{}-length-model}/\code{-{}-length-inputs}/\code{-{}-where} form into an opaque plan, now consumed by \filepath{src/Main.hs}'s filter-only inline runner: fixed option order, no repetition, mutual exclusion with the contract-file option, and the same activated-startup-policy and delimiter rules as the file forms. \\
\filepath{src/Leant/Synth/Length/Where.hs} & Leant profiles over Djex's bounded where-clause syntax: two fixed exact-case \code{List} models, ASCII input-index admission, and a resolver that builds the complete source-ordered role vector from the Lean-translated physical arity before elaboration expands the built-in profile (exact zero/step cases, no provider laws); every stage remains passive data and no step infers authority from the clause or the target. \\
...
\begin{enumerate}
  \item introduce one nominal rank-2 assessment context for the command;
  \item translate the goal inside that context, including any universe-narrowing retry;
