...
\begin{enumerate}
  ...
  Leant's \filepath{src/Leant/Synth/Engine.hs} first builds one prepared semantic-origin record. Search, renderer inputs, retained Exference authority, and later exact-origin rerendering project that same record; no post-search field-for-field preparation copy exists. The engine retains the typed candidate in a \code{TypedCandidateSemanticSidecar} together with the exact Exference inventory/session, prepared origin, source-name table, converted source goal, session policy, request, and provider assignments. It keeps no parallel graph-key cache: the exact graph remains owned by the opaque checked candidate, and Djex fingerprints it only while sealing an actual behavioral problem. Candidate grouping and renderer filtering preserve a group sidecar only while the group still denotes the same engine candidate. During combined-engine exact-text deduplication, an earlier compatibility-only spelling may instead retain a private variant-scoped witness to the first bounded later Exference occurrence with identical bytes. That witness keeps the Exference sidecar and source renderer ordinal separate from the display group's route, display ordinal, ordering, sidecar, and sibling variants. At the verification boundary, the display route, ordinal, and text are flattened into the callback candidate, which retains that spelling's exact typed origin once rather than keeping the scheduling-only variant and a parallel origin field. A wrapper such as the classical prove-mode transformation explicitly drops both direct and recovered authority.
...
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/Semantic/Length/SMTLib/Session/Process.hs} & Profile-only raw Z3 launch and bounded executable observation, FIFO pipes, cancellation/deadlines, and staged cleanup; no complete Length-policy copy. \\
...
\midrule
\endhead
\filepath{src/Leant/Synth/Engine.hs} & One prepared semantic origin shared by search and rendering, Exference run authority, typed semantic sidecars, opaque verification provenance, and exact-origin rerender mechanics. \\
...
\filepath{src/Leant/Synth/Length/PostVerification/Internal.hs} & Package-private rank-2 adapter which seals occurrence handles before projecting the ranking report. \\
