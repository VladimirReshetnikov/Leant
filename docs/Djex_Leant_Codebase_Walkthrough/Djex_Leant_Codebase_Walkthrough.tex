...
Djinn now exposes \code{DjinnTypedCandidate} and \code{DjinnTypedResult}, with typed counterparts of its ordinary, provider-candidate, assignment, and kinded-assignment runners in \filepath{djinn/src-core/Language/Haskell/Djex/Djinn.hs} and \filepath{djinn/src-core/Djinn/Core.hs}. The legacy runners are one-way compatibility projections of those typed paths. Final candidate keys are allocated only after proof checking, deduplication, and any configured final ordering step, so future typed occurrence identities need not depend on discarded proof-plan ordinals.
...
\filepath{src/Leant/Synth/Length/Ranking/Internal.hs} & Package-private preparation, scoped serial execution, evidence replay, handle-preserving no-prune ordering, and atomic fallback. \\
