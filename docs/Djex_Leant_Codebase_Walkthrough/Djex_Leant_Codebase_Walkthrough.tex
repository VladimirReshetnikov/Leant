...
The headline is intentionally modest: the repositories now have a checked insertion seam for one finite-list-spine Length model, a scoped Z3 execution boundary, an explicit startup policy opt-in, an optional one-command contract choice, and an explicit per-command choice between stable ranking and replay-authorized hard filtering in \filepath{src/Main.hs}. Without the startup opt-in the ordinary no-option and explicit-rank commands remain the exact non-strict identity path, while a filter request or requested command contract is refused before file IO. The Level-1 filter can refill survivors from the remainder of one already bounded synthesis lane, and Main now retains a private four-way outcome for that lane. Djex also exposes candidate-independent bounded replay-input bank types, but Leant does not use them. Neither prerequisite is the proposal's persistent or cross-lane counterexample-guided loop. There is no claim here of general behavioral constraints, general program equivalence, a Lean-to-SMT semantics, solver-certified proofs, typed-sketch synthesis, or automatic Z3 integration.
...
The pinned Djex tree separately exposes nominal candidate-independent scalar and binary-product bank scopes and bounded immutable input stores. Scope construction binds the checked session inventory and provider-law basis, solver-neutral interpretation policy, exact contract, and alpha-canonical target while excluding the candidate and execution identity. A scope match authorizes only an attempted fresh replay. No Leant module imports this store, no command owns one, and no automatic insertion or replay-to-receipt bridge exists; the active MRU in the preceding paragraph remains fresh for every assessment batch.
...
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/SMTLib.hs} & Transient typed QF\_LIA planning, canonical bounded bytes, structurally complete query fingerprint, retained problem-scope projection, minimal replay authority, and model-binding validation. \\
...
Complete replay releases \code{ApplicableDomainEstablished} or its nominal \nolinkurl{LengthSpinePairApplicableDomainEstablished} sibling, carrying \code{ValidatedLengthApplicableDomain} or \nolinkurl{ValidatedLengthSpinePairApplicableDomain}. \nolinkurl{renderLengthApplicableDomainValidationNote} and \nolinkurl{renderLengthSpinePairApplicableDomainValidationNote} report model/provider-relative basis, boxes, raw visits, unique and applicable assignments, explicit vacuity, and a bounded maxima prefix. They grant no hull, solver proof, global theorem, or pruning authority. The current file-schema resets are recorded in \filepath{docs/reports/2026-08-15-versionless-length-ranking-configuration.md} and \filepath{docs/reports/2026-08-15-versionless-length-contract.md}; the current guarded behavior is in \filepath{docs/reports/2026-08-15-guarded-conditional-length-ranking.md}, the connected selection path is recorded in \filepath{docs/reports/2026-08-15-command-authorized-length-filtering.md}, its lane-local refill is recorded in \filepath{docs/reports/2026-08-15-lane-local-length-survivor-refill.md}, and the private outcome seam is recorded in \filepath{docs/reports/2026-08-16-explicit-synthesis-lane-outcomes.md}, while the earlier recursive and structural-seal reports remain non-normative implementation history at their landing checkpoints. Djex's exact guarded semantics and private identity boundary are in \filepath{lib/Djex/docs/reports/2026-08-15-guarded-conditional-length-applicable-domain.md}; its unused candidate-independent bank foundation is in \filepath{lib/Djex/docs/reports/2026-08-16-nominal-length-counterexample-bank-foundation.md}.
