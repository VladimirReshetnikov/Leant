...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque reusable execution/replay policy, total assembly from validated Djex authorities, sealed digest-expectation presence bridge, and policy-plus-request-contract ranking/assessment entrances. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded version-1 JSON decoder whose disabled owner retains one strict validated policy beside one lazy fixed compatibility contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
...
\filepath{src/Leant/Synth/Length/Contract/File.hs} & Exact contract-only versioned root: unchanged v1, v2 positive-literal Natural modulo, v3 required ordered target roles, v4 explicit exact zero/step cases, and v5 explicit policy plus positive-literal Natural quotient, with shared limits and no execution-policy fields. \\
...
\filepath{src/Leant/Synth/Length/Ranking.hs} & Association-free ranking facade: complete reports, sanitized failures and refusals, and no associated-plan projector. \\
\filepath{src/Leant/Synth/Length/Ranking/Internal.hs} & Package-private preparation orchestration, single-owner receipt-association/query state, scoped serial execution, query-owned evidence replay, handle-preserving no-prune ordering, and atomic fallback. \\
...
\begin{itemize}
  \item \code{:reset} starts a replacement backend without replaying the old history. It preserves user preferences such as timeout, color, engine, and search settings while clearing declarations/import state to the baseline.
...
\lpath{src/Leant/Session/Snapshot.hs} manages a snapshot as a small artifact family:
