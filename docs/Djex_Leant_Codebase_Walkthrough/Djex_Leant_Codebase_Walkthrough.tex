...
\begin{enumerate}
  ...
  \item \textbf{The contract is explicit and solver-neutral.} \filepath{src/Leant/Synth/Length/Contract.hs} contains passive source vocabulary: exact Lean names for a unary zero/step spine, pre- and postconditions, and optional provider laws. Leant does not infer that contract from the goal, a declaration name, or an implementation. For each provider law the caller supplies argument roles and a Length transfer expression; the exact closed provider scheme and private identity come from retained translation provenance. Successful sealing establishes consistency of the named objects with the checked handoff. It does \emph{not} establish that a caller-asserted provider law is true of the Lean implementation. The lower-level policy entrance still accepts a request-owned contract. The version-1 CLI compatibility path instead loads one explicit contract at startup and rechecks every later candidate against that fixed process-wide assertion. Its file boundary retains one strict reusable policy beside that lazy contract; after pin activation, the private integration mode keeps those values together and calls the same policy-plus-contract assessment entrance. No second generic configuration aggregate or configured runner owns a duplicate pairing.
...
\filepath{src/Leant/Synth/Length/Integration.hs} & Opaque disabled/configured mode, admission--load--activation setup, retained closed activation disposition for startup presentation, private ownership of the activated policy/fixed-contract pair, sanitized fallback, and exact disabled identity. \\
