...
\def\DjexBase{https://github.com/VladimirReshetnikov/Djex/blob/f957549e36a63cd6003f5edef4ab8a867221813f/}
\def\LeantBase{https://github.com/VladimirReshetnikov/Leant/blob/a18f3d0a99bb1b988f48747ecaadbe28b756e139/}
...
\textbf{Djex snapshot} & \commitid{f957549e36a63cd6003f5edef4ab8a867221813f} \\
\textbf{Leant snapshot} & \commitid{a18f3d0a99bb1b988f48747ecaadbe28b756e139} \\
\textbf{Relationship} & Leant's \filepath{lib/Djex} submodule is pinned to the same Djex revision. \\
\textbf{Prepared} & August 16, 2026 \\
\end{tabularx}
\end{tcolorbox}
\vfill
\vspace{0.25in}
...
Ranking keeps every verified occurrence and uses the established positive/neutral/counterexample order. It retains the historical five-success verification frontier, raw MRU, and one engine batch. Filtering instead constructs one total scalar or nominal pair partition for each selected cursor batch: at most 12 groups for a standalone engine, 24 for \code{both}, or 6 for the excluded-middle route. An ordinary filter run may select one same-width successor after no verification or complete all-rejection; excluded middle never does. It retains preparation refusals, unassessed candidates, every raw solver status, bounded-positive candidates, and applicable-domain establishments. Only an independently replayed counterexample becomes rejected. The generic partition seal restores both subsequences to original callback order and supplies association, not evidence. Any selection failure preserves the literal original batch and presents no rejection; Main still caps that terminal preserve-all presentation at five, while completed nominal-bank transitions remain committed. Main takes at most five survivor presentations from the first terminal batch but prints every rejection accumulated before and within that terminal result.
...
The module sets bounded constants for backend candidate windows, combined verification frontiers, attempted groups, and shown successes. At this snapshot, ranking stops verification after five accepted groups and consumes one candidate batch. Filtering may consume two ordered batches from one ordinary lazy engine outcome: 12+12 groups for a standalone engine or 24+24 for a combined engine. Only no verification or complete all-rejection reaches the successor. Excluded middle remains one six-group batch, while double negation uses the ordinary policy and a filter-only Djinn candidate cutoff of 60. Every batch is verified and assessed once through the command context; survivor or preserve-all stops, and the second batch ends without a third tail probe. Only after a terminal outcome does Main take five survivor presentations. This is not a survivor quota within a run or across lanes: the first batch with one survivor is terminal. The batch prefix remains separate from the successful-group quota because a failed group does not consume that quota. The constants are named and centralized; code should not introduce ad hoc \code{take 5} operations that lose progress accounting.
...
advance request is admitted only from 1 through the established
\code{candidateWindow} of 60. Nonpositive and over-limit requests are rejected
...
and clamps a request near the cumulative 60-group cap to the remaining
allowance. It reports natural exhaustion only after an advance before that cap
observes an empty tail. Once group 60 has been returned, the next advance
reports the hard cap without examining group 61. Consequently an exactly
60-element stream is intentionally classified as capped, not naturally
...
When enabled after a retained sound refutation and the Glivenko split admits the fragment, Leant first offers bounded excluded-middle premises, then applies a double-negation transformation when excluded middle has no verified candidate or is entirely behaviorally rejected. Both routes use the command's assessment context. Excluded middle is always one six-group batch. Filter-mode double negation follows the ordinary 12+12 or 24+24 policy and raises the tuned Djinn candidate cutoff to 60; rank and disabled modes retain one 12- or 24-group batch and the cutoff of 12. Filtering keeps the original absolute command deadline through both routes. Rank and disabled modes instead capture independent fresh configured-duration deadlines when EM and NN are actually reached; zero remains unbounded. A survivor or preserve-all result stops; every actual batch outcome is accumulated, but neither route contributes negative evidence because:
...
batch; ranking stops after five acceptances. Filtering traverses a complete
12-group standalone or 24-group combined batch and passes all callback
...
from the same cursor. Excluded middle remains one six-group batch; double
...
terminal filter outcome, at most five survivors are displayed and bound, while
...
\begin{tikzpicture}[node distance=7mm and 7mm]
  ...
  \node[graybox,left=of assess,text width=34mm] (bind) {Display and bind\\up to five survivors};
...
\end{tikzpicture}}
\caption{The candidate funnel. The exact Lean target remains the acceptance gate; optional behavioral filtering partitions each bounded cursor batch, with at most one successor, before the five-survivor presentation cap.}
...
at most five original candidates before retained earlier rejections. Neither
...
Leant combined batch prefix & 24 & Bounds cross-engine semantic groups; filter mode may consume two batches from one cursor, at most 48 groups. \\
Leant standalone batch prefix & 12 & Bounds Djinn- or Exference-only groups; filter mode may consume two batches from one cursor, at most 24 groups. \\
Leant excluded-middle batch prefix & 6 & Keeps the first classical route cheap; it never consumes a successor. \\
Double-negation Djinn candidate cutoff & 12 rank/disabled; 60 filter & Lets filter mode expose a successor while retaining the historical one-batch classical policy elsewhere. \\
Leant ranking accepted-group quota & 5 & Preserves the historical rank verifier frontier. \\
Leant presented survivors & 5 & Caps bindings and interactive output after filtering without truncating rejection rows. \\
