...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque reusable execution/replay policy with independent optional origin-probe, finite input-box, complete applicable-domain, and non-vacuous positive-ordering policies; total assembly from validated Djex authorities; sealed digest-expectation and three-way executable-launch classifiers; literal legacy, descriptor-bound, and descriptor-bound effective-ID executable-access constructors; and policy-plus-request-contract ranking/assessment entrances for both the scalar and SpinePair domains. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for startup versions 1--22: literal v1--v20 compatibility dispatch plus scalar v21 and binary-product v22 effective-ID executable-access descriptor roots. The version pairs select literal-ceiling, runtime-unscoped usable-work, relational, scoped usable-work, strict-relational, root-quotient, sealed-main-image, or two-point source-access launch policy without revising older schemas; v23 remains unsupported. Its disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
...
\filepath{src/Leant/Synth/Length/Presentation.hs} & Opaque same-ranked-receipt candidate text and optional bounded, provider-name-free counterexample or independently checked input-box note; no detached evidence reassociation. \\
...
The current startup configuration extends through version 22. Versions 7 and 8 introduce the advanced scalar/product root and literal-ceiling positive-affine validation; v9/v10 add the runtime-unscoped v1 shared usable-work object; v11/v12 instead select relational positive-affine validation without that object; v13/v14 combine the relational rule with the owner-thread-affine, cooperatively checkpointed scoped-v2 budget; and v15/v16 preserve the complete v13/v14 root while replacing only the applicable-domain strategy with \code{strict-relational-positive-affine-v1}. The strict rule accepts every ordinary relational clause plus exactly one immediate normalized top-level natural complement, \code{not (L <= R)}, as \code{R + 1 <= L}; it is neither general negation normalization nor a claim about source evaluation strictness. V17/v18 preserve the complete v15/v16 behavioral and scoped-budget profile while adding only \code{descriptor-bound-executable-v1}: the optional digest is checked against the sealed staged main-image bytes installed through descriptor execution, with no pathname retry and no authority over loaders, libraries, or solver semantics. V19/v20 retain that launch/scoped profile and replace only the applicable-domain strategy with \code{strict-relational-positive-affine-quotient-v1}; exactly one positive-literal quotient may occur at one top-level relation operand root, equality contributes both source-ordered directions, and unsupported nested, embedded, two-root, or whole-formula shapes grant no partial coverage rule. V21/v22 retain the complete v19/v20 policy and replace only the launch literal with \code{descriptor-bound-effective-id-executable-access-v1}. The opened source must pass the two point-in-time effective-filesystem-credential VFS access checks described above, while the sealed memfd remains fixed \code{0500} main-image-byte transport; there is no older-launch fallback and no claim over full source exec security, loaders, libraries, or solver semantics. Each later decoder is reached only after the complete older chain returns its closed unsupported-version sentinel; version 23 is the next unsupported sentinel. Versions 1--6 retain their unchanged eager paths; v7--v22 retain deferred opening, so an all-pure batch performs no executable or access-check IO and the first live miss opens at most one fresh lexical scope. V13--v22 place preparation, pure ranking, live work, and Leant-owned result forcing beneath one absolute scoped deadline with cooperative phase checkpoints. Applicable-domain notes expose derived maxima, checked/applicable counts, basis, and explicit vacuity; strict and quotient simplification metadata stays attached to the exact occurrence; launch strategy and deadline observations themselves add no evidence note.
