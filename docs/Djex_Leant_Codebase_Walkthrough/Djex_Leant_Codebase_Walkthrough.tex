...
Applicable-domain validation is the second positive axis beside the finite input box: an independent, query-owned exhaustive traversal that establishes the precondition-applicable input domain, sealed in Djex under a nominal direct-v1 or positive-affine receipt schema for the scalar or binary-product domain. Its receipts feed the separately selected non-vacuous applicable-domain preference---programmatic through startup version 6 and required with positive-affine validation by v7/v8. Contract-only files never select that preference, and, like the input box, the external solver status itself never becomes the evidence.
...
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for startup versions 1--8: unchanged v1--v6 dispatch plus the advanced scalar v7 and binary-product v8 roots. Its disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
...
The current startup configuration extends through version 8. Versions 7 and 8 have the exact root demand order \code{format}, \code{version}, \code{executionAdmission}, \code{execution}, \code{evaluation}, \code{inputBoxValidation}, \code{counterexampleProbe}, \code{boundedPositiveOrdering}, \code{applicableDomainValidation}, \code{applicableDomainOrdering}, \code{counterexampleSimplification}, \code{liveSessionOpening}, then \code{contract}. V7 selects scalar contract grammar v5; v8 selects the nominal pair grammar and exact binary-product result shape. The positive-affine and simplification objects retain independent limits capped at eight inputs and 65,536 assignments, and the exact live-session literal is \code{defer-until-live-query}. The generalized decoder delegates v1--v6 to their unchanged eager paths. Thus the preceding statements that every eligible batch opens a worker describe those eager paths only: a v7/v8 all-pure batch opens none, while a first live miss opens exactly one fresh lexical scope. Positive-affine notes expose derived maxima, checked/applicable counts, basis, and explicit vacuity; strict simplification notes remain attached to the exact occurrence; deferred opening itself adds no evidence note.
