...
\hypersetup{
  ...
  citecolor=DeepGreen,
  urlcolor=MidBlue,
  pdfauthor={Technical walkthrough of the Djex and Leant codebases},
...
\def\DjexBase{https://github.com/VladimirReshetnikov/Djex/blob/e454757d53ec057238c22fc6cd0d935b7d96b3dc/}
\def\LeantBase{https://github.com/VladimirReshetnikov/Leant/blob/ada69d884480b93e74ee4b3daf77be38702ee07d/}
...
\textbf{Djex snapshot} & \commitid{e454757d53ec057238c22fc6cd0d935b7d96b3dc} \\
\textbf{Leant snapshot} & \commitid{ada69d884480b93e74ee4b3daf77be38702ee07d} \\
\textbf{Relationship} & Leant's \filepath{lib/Djex} submodule is pinned to the same Djex revision. \\
\textbf{Prepared} & August 16, 2026; revised August 17, 2026 \\
\textbf{Scope} & This is now the only maintainer walkthrough for either repository. The former Djex-only walkthrough was retired on August 17, 2026; its Djex material is covered by Parts~I--II here. \\
\end{tabularx}
\end{tcolorbox}
\vfill
\vspace{0.25in}
...
\chapter*{Current-tree addendum:\\typed behavioral assessment}
\addcontentsline{toc}{chapter}{Current-tree addendum: typed behavioral assessment}
\markboth{Current-tree addendum: typed behavioral assessment}{}

The historical body of this walkthrough predates the typed behavioral-assessment layer. This addendum records the newer layer against the current source snapshots named on the title page while retaining the historical landing commits that established it. Leant remains experimental: JSON shapes, names, tags, diagnostics, and output are current-tree descriptions rather than a public stability or backward-compatibility promise. The startup configuration and command-local contract file now each have one versionless schema whose explicit \code{rankingDomain} is \code{scalar} or \code{binary-product}; older numbered roots survive only in Git history and dated non-normative reports. Paths in this addendum are printed as current-tree file paths rather than through historical line-number references.


  The headline is intentionally modest: the repositories now have a checked insertion seam for one finite-list-spine Length model, a scoped Z3 execution boundary, an explicit startup policy opt-in, an optional one-command contract choice, an active fixed-profile inline where-clause choice, and an explicit per-command choice between stable ranking and replay-authorized hard filtering in \filepath{src/Main.hs}. The title-page Leant snapshot includes the active inline Main route and preserves the passive parser/profile foundation as its predecessor in history. Without the startup opt-in the ordinary no-option and explicit-rank commands remain the exact non-strict identity path, while a filter, inline, or requested command-contract request is refused before clause parsing or file IO.

  \textbf{What one command actually does.} Established startup and contract-file requests introduce one nominal rank-2 context before translation. The inline route instead validates structure, selects the goal, authorizes literal filter mode, parses the bounded clause, reports inaccessible hypotheses and premise scope, translates and retries, resolves physical source-arrow roles, and only then introduces exactly one nominal context. Either route reuses its filter bank across the complete scheduler. Main consumes Engine's opaque candidate cursor directly: one ordinary filter run may assess two ordered standalone or combined batches from the same lazy engine outcome, while excluded middle remains one half-width batch. Under the default session limits those widths are a 60-group cursor and 12-group, 24-group, and six-group batches; every one of them is a \code{:set} away from being something else (Appendix~\ref{app:limits}), so the numbers quoted throughout this addendum describe an unmodified session. Only no verification or complete behavioral rejection reaches the single successor; survivor and preserve-all stop, and Main never probes a third tail. Djex supplies the candidate-independent bounded replay-input bank and exact query-owned fresh replay. Established configured-rank and direct Selection compatibility paths retain their raw four-vector MRU; rank and disabled commands retain one engine batch. The scheduler keeps one lazy reverse outcome accumulation and finalizes it once.

  \textbf{And what it is not.} This is bounded command-local same-run progression, not persistent state, survivor-quota filling, an engine rerun, batch reassessment, counterexample-directed engine search, or the proposal's complete CEGIS loop. There is no claim here of general behavioral constraints, general program equivalence, a Lean-to-SMT semantics, solver-certified proofs, typed-sketch synthesis, or automatic Z3 integration.
...
\begin{enumerate}
  ...
  \textbf{One prepared record, projected everywhere.}   Leant's \filepath{src/Leant/Synth/Engine.hs} first builds one prepared semantic-origin record. Search, renderer inputs, retained Exference authority, and later exact-origin rerendering project that same record; no post-search field-for-field preparation copy exists. Each provider binding is the sole prepared owner of its private declaration inputs, renderer metadata, and ordered instantiation assignments. Preparation derives the provider declaration list and one lazy strict-map projection shared by both renderer closures; later exact-origin rerendering and inspection derive that same provider-map projection at their use edges. Djinn, Exference, and inspection derive the aggregate assignment list in binding order. One private conversion maps that exact projection through the complete source-name table for the Exference search call, preserving provider order, provider-local order, kinds, and variable identities.

  \textbf{The sidecar retains inputs, not derived copies.} The typed candidate's \code{TypedCandidateSemanticSidecar} retains the exact Exference inventory/session, prepared origin and provider bindings, source-name table, session policy, and request rather than caching either the converted assignment list or a converted source goal. Package-private inspection derives both historical projections lazily; the checked handoff performs its checked source-goal conversion at the use edge. It keeps no parallel graph-key cache: the exact graph remains owned by the opaque checked candidate, and Djex fingerprints it only while sealing an actual behavioral problem.

  \textbf{Deduplication must not let two spellings swap origins.} Candidate grouping and renderer filtering preserve a group sidecar only while the group still denotes the same engine candidate. During combined-engine exact-text deduplication, an earlier compatibility-only spelling may instead retain a private variant-scoped witness to the first bounded later Exference occurrence with identical bytes. That witness keeps the Exference sidecar and source renderer ordinal separate from the display group's route, display ordinal, ordering, sidecar, and sibling variants. At the verification boundary, the display route, ordinal, and text are flattened into the callback candidate, which retains that spelling's exact typed origin once rather than keeping the scheduling-only variant and a parallel origin field. A wrapper such as the classical prove-mode transformation explicitly drops both direct and recovered authority.

  \item \textbf{Lean callback acceptance is recorded, not inflated.} A rendered group carries its original renderer ordinal and semantic origin into \filepath{src/Leant/Synth/Verification.hs}. The bounded verifier tries variants lazily and mints an opaque nominal \code{Verified candidate} only when its callback returns \code{VariantAccepted}. Ranking consumes one engine batch and stops after \code{synth-shown} accepted groups (five by default). Filtering verifies a complete standalone or combined batch --- 12 or 24 groups by default --- and, only after no verification or complete all-rejection, may verify one successor of the same width from that engine cursor. Excluded middle remains one half-width batch, six groups by default; double negation uses the ordinary policy. Every batch is verified and assessed once, the first survivor or preserve-all result stops, and no third tail is probed. In normal execution the callback asks the Lean backend to elaborate the generated verification program and rejects transport failures, fatal responses, diagnostics, and admitted \code{sorry} results. Nevertheless, the \code{Verified} type promises only callback acceptance at this boundary. It is not a Lean kernel proof object, behavioral evidence, or a solver certificate.
  ...
  \item \textbf{The contract is explicit and solver-neutral.} \filepath{src/Leant/Synth/Length/Contract.hs} contains passive source vocabulary: exact Lean names for a unary zero/step spine, pre- and postconditions, optional provider laws, target roles, and a closed candidate-case policy. Leant does not infer that contract from the goal, a declaration name, or an implementation. For each provider law the caller supplies argument roles and a Length transfer expression; the exact closed provider scheme and private identity come from retained translation provenance. Successful sealing establishes consistency of the named objects with the checked handoff. It does \emph{not} establish that a caller-asserted provider law is true of the Lean implementation.

  \textbf{Startup contract versus command contract.} Every activated startup file loads one explicit scalar or pair contract and rechecks later candidates against that fixed process-wide assertion. After activation, \texttt{:synth }\dashdash\texttt{length-contract ABSOLUTE-PATH }\dashdash\texttt{ TYPE} may instead load one command-local document whose exact root is \code{\{format, rankingDomain, contract\}}; an optional preceding \texttt{--behavior-mode rank|filter} selects what the command does with that contract. \code{rankingDomain = scalar} selects the full scalar grammar and \code{rankingDomain = binary-product} selects the separately typed canonical-\code{Prod} grammar; it is the sole discriminator and neither nested contract admits \code{resultShape}. Both nested grammars require exact \code{targetArgumentRoles} and \code{candidateCasePolicy}, and both admit current modulo and positive-literal quotient expressions. The bounded JSON input is parsed once, with no version dispatcher, migration, or alternate-domain retry.

  \textbf{Active inline contract source.} The fixed-order form is\par
  \smallskip\noindent
  \code{:synth} \code{-{}-behavior-mode filter} \code{-{}-length-model}
  \nolinkurl{list-scalar-exact-cases|list-binary-product-exact-cases}
  \code{-{}-length-inputs} \nolinkurl{arg0[,argN...]}
  \code{-{}-where CLAUSE -{}- TYPE}. The bar presents
  two alternative literal model tokens. The clause is unquoted and ends at the
  standalone delimiter; quotes are rejected clause characters. The input token
  is a nonempty strictly increasing subset of physical \code{arg0} through
  \code{arg7}; gaps and unused declarations are accepted. Only translated
  source-arrow slots count, never universal, instance, or contextual binders.
  The fixed expansion supplies \code{List}/\code{List.nil}/\code{List.cons},
  exact zero/step cases, true precondition, the explicit observed/unobserved
  role vector, and an empty provider-law set. It is mutually exclusive with a
  contract file and reuses the activated policy. Neither clause nor target
  infers any of those semantic or execution choices.

  \textbf{What a role does and does not mean.} Observed roles receive compact \code{LengthInput} indices; provider transfers retain physical provider-argument indices, so a map-shaped target can forward physical argument 1 to compact input 0 while carrying argument 0 opaquely. An unobserved role means only that the Length interpreter treats that target argument as opaque and non-inspectable; it proves nothing about inhabitance, evaluation, purity, totality, parametricity, strictness, or effects. The exact case policy likewise grants bounded model interpretation, not source termination, equivalence, proof, or rejection authority by itself. The command reuses the already activated policy and cannot replace executable, pin, solver, artifact, replay, or ordering authority. An inline candidate requiring a provider law absent from its fixed empty set retains the ordinary preparation refusal; that dependency is not rejection evidence. The opaque Integration request retains the strict behavior mode and policy beside the lazy contract and stays on the command stack through ordinary, retry, provider, and classical lanes rather than entering REPL state, history, snapshots, or a cache. No second generic configuration aggregate or configured runner owns a duplicate pairing.

  \item \textbf{Djex seals two versioned Length problems.} The public modules below \filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length} implement two sealed domains: \code{finite-list-spine-length/v1} and its separately typed \code{finite-binary-product-spine-lengths/v1} sibling. The product domain's public vocabulary---\code{LengthSpinePair}, \nolinkurl{ValidatedLengthSpinePairCounterexample}, \nolinkurl{ValidatedLengthSpinePairInputBox}, and \nolinkurl{ValidatedLengthSpinePairApplicableDomain} in \filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/Evaluate.hs}---is nominally distinct from the scalar vocabulary, so evidence sealed under one domain cannot be presented under the other. The scalar domain interprets a declared unary zero/step spine by natural-number length, supports a bounded expression/formula language, opens the exact candidate graph against the checked target, and records the precondition, postcondition, counterexample condition, inventory, encoding, candidate, and problem fingerprints. Its additive exact-case entrance freshly re-seals a complete recursive zero/step graph against the resolved spine schema; the zero branch receives zero, the step tail receives \code{input monus 1}, and the payload remains opaque. All ordinary entrances and every other case shape fail closed. The generic behavioral envelope is the checked problem's single retained owner of the concrete encoding fingerprint; the Length-specific selector derives from it. The versioned candidate key already embeds the exact freshly checked graph fingerprint, so the interpreted candidate receipt does not retain a parallel detachable graph key. This is not a semantics for arbitrary inductives, dependent values, effects, partiality, infinite structures, or arbitrary Haskell/Lean programs.
  ...
  Resource ownership beneath all three native descriptor strategies is now one
  shared transition. A resource-producing deadline worker publishes its one
  terminal result under masking as its final effect; cancellation, deadline
  loss, or a controller exception joins through that same completion and rolls
  back any acquired value. A successful raw \code{DescriptorCreated} then
  crosses one restored checkpoint while raw child-and-stdio rollback is
  installed. The consumer is entered masked and allocates the opaque process
  owner before restored initialization; after that single transfer, process
  cleanup is the only owner. The exec-status \code{Handle} is independently
  closed in a masked finalizer on normal, synchronous-failure, deadline, and
  asynchronous-interruption exits. This package-private hardening changes no
  launch selector, fingerprint, protocol, receipt, or evidence authority; its
  exact transition is recorded in
  \filepath{lib/Djex/docs/reports/2026-08-15-descriptor-spawn-resource-ownership.md}.
  ...
  After an occurrence seal, \filepath{src/Leant/Synth/Length/Presentation.hs} traverses each whole materialized ranked, selected, or rejected receipt and derives an opaque candidate/evidence presentation. Main never zips a detached evidence list back to candidates by spelling or index. Ranking uses its ordered survivor list for Lean bindings, splices, and \code{itN} output. Filtering binds only selected occurrences and prints each rejected occurrence separately with the exact established counterexample or simplification note. Independently replayed counterexamples, independently completed bounded-positive input-box receipts, and current applicable-domain receipts produce subordinate notes in both the scalar and canonical-\code{Prod} domains. A counterexample note calls the receipt replayed and model-relative and renders a bounded summary of observed input and result spine lengths. A bounded-positive note reports the checked bounded region and its checked and applicable assignment counts, stating explicitly when zero applicable assignments make the result vacuous. Applicable-domain presentation uses only \code{renderLengthApplicableDomainValidationNote} or \code{renderLengthSpinePairApplicableDomainValidationNote} and includes the canonical maxima-antichain prefix, box and visit counts, unique and applicable assignments, and basis. All note forms reduce a provider-backed basis to the number of assumed laws. None projects the receipt's private provider-name list, attributes positive evidence to raw \code{unsat}, or makes a Lean, proof, correctness, or Z3-attestation claim.

  \item \textbf{Assessment reports support two separately authorized operations.} The package-private implementation in \filepath{src/Leant/Synth/Length/Ranking/Internal.hs} first productively admits at most 64 candidates and completes all pure candidate/query preparation before opening a worker. Programmatic policies may leave completed positive receipts neutral or enable either non-vacuous preference independently. The current startup policy enables both preferences and the guarded recursive piecewise-affine validator. In default ranking mode, its stable order is non-vacuous applicable-domain evidence, non-vacuous explicit-box evidence, neutral or vacuous assessments, then independently replayed counterexamples. Original order is retained inside every partition and every callback receipt remains present. In explicit filter mode, the scalar or pair Selection adapter maps each report back to its original callback occurrence, retains preparation refusal, \code{Unassessed}, every heuristic status, \code{BoundedPositive}, and \code{ApplicableDomainEstablished}, and rejects only an independently replayed \code{Counterexample}. Contract-only files select no policy or operation. Any returned session, query, fingerprint-association, or evidence-replay failure discards partial live assessments and restores literal original order: eligible candidates become \code{Unassessed}, while actual pure preparation refusals retain their bounded refusal class.
  ...
  \item \textbf{A distinct generative seal protects hard selection.} \filepath{src/Leant/Synth/BehavioralSelection.hs} mints another fresh rank-2 occurrence epoch and validates one bounded total retain/reject decision for every original receipt. Its public facade exports no decision builder; package-internal Length adapters can classify only handles supplied by that epoch and cannot manufacture or reindex one. The seal reconstructs the original \code{Verified} receipts and emits both selected and rejected partitions in original callback order, independent of assessment rank or decision order. The generic wrapper supplies no behavioral evidence. \filepath{src/Leant/Synth/Length/Selection.hs} and its nominal SpinePair sibling supply the replay-only taxonomy above. Every post-verification, ranking, ranking-absence, index, limit, or seal failure returns the complete original batch and exposes no partial rejection.
  \item \textbf{The current startup route fixes one advanced policy.} The versionless startup decoder composes the guarded recursive piecewise-affine applicable-domain validator, both non-vacuous preferences, componentwise-lexicographic simplification, deferred opening, and one scoped usable-work deadline. It completes admission and preparation, then runs each candidate's selected bank $\rightarrow$ guarded domain $\rightarrow$ origin prefix before IO. Rank and direct Selection compatibility paths select the raw four-vector MRU; Integration's context-aware filter path selects its nominal bank. An all-pure batch opens no process; the first live miss opens one lexical session, executes that head once without repeating its pure prefix, and processes the suffix in the same session. Every counterexample from those pure sources, live replay, or the post-\code{unsat} box crosses the same query-owned simplification seam. The raw path inserts only the final vector; the context path promotes an unsimplified hit without reevaluation or freshly records the final receipt with its live, solver-independent, or simplification origin. Any indexed failure atomically restores original candidate order and clears earlier evidence and occurrence-attached metadata, but does not roll back completed context transitions; a pre-open failure launches nothing. Contract-only documents use one current versionless domain-selected schema and carry no ranking or execution policy.
\end{enumerate}

\begin{invariantbox}[title={Current post-verification invariant}]
The only batch eligible for sealed post-verification presentation is the exact callback-accepted batch. In ranking mode, an independently replayed counterexample for the exact retained Exference graph and canonical scalar or pair query moves stably behind neutral candidates; non-vacuous applicable-domain and explicit-box receipts may move ahead under the current policy, but every receipt remains present. No raw solver status moves a candidate: \code{unsat} only schedules independent finite-box traversal. A complete bounded permutation is sealed before presentation.

In explicit filter mode, the same assessed occurrences enter a distinct total-partition seal. Preparation refusal, unassessed input, raw status, bounded-positive evidence, and applicable-domain establishment are retained. Only the exact replayed-counterexample report is rejected. Both output subsequences are restored to original callback order. Any adapter or seal failure preserves the literal original batch and exposes no rejection. Candidate text and evidence are projected from the same whole ranked, selected, or rejected wrapper, so equal spellings cannot exchange evidence.

A replayed counterexample need not come from a live solver response: the query-owned origin probe or an input from the selected raw or nominal bank can yield the same receipt only after independent replay against that occurrence. Each counterexample first crosses the query-owned simplification seam. The raw rank/direct-Selection path retains only its final vector; the context-aware filter path promotes an unsimplified retained sample or records the final receipt under the appropriate coarse origin. Established startup/file requests introduce one filter context before goal translation; the inline route introduces it after translation and physical-arrow resolution. Either reuses exactly one context through every ordinary, provider, and classical lane. The compatibility assessor still creates a fresh context for each call made by other clients. Deferred lifecycle state, behavior mode, assessment contexts, and replay state are not evidence and do not enter persistent REPL state.
\end{invariantbox}

\section*{The binary-product mirror and the newer ranking mechanics}

The canonical-\code{Prod} path mirrors the scalar ranking, post-verification,
and selection stacks under
\filepath{src/Leant/Synth/Length/SpinePair}; each public module has a
package-private \code{Internal} sibling. Since the two ranking and the two
selection siblings ran the same control flow in two vocabularies, that flow
now lives once---in \filepath{src/Leant/Synth/Length/Ranking/Generic.hs} over
the closed \code{LengthRankingDomain} class and in
\filepath{src/Leant/Synth/Length/Selection/Generic.hs} over a
\code{SelectionDomain} record---and each \code{Internal} sibling is the
domain's instance: it keeps its nominal assessment, failure, policy, receipt,
and result types and its own Djex vocabulary, so a product receipt still
cannot be presented as a scalar one. The nominal counterexample-bank states
and contexts are likewise one implementation parameterized by each domain's
limits, bank, scope, sample, receipt, and error types. On the Djex side,
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/Semantic/Length/SMTLib/Protocol/SpinePair.hs}
is deliberately thin: the scalar and product protocol modules share one phase
machine, framing validation, write rendering, and fingerprint construction,
parameterized by the \code{LengthSMTLibProtocolIdentity} record in
\filepath{.../Semantic/Length/SMTLib/Protocol.hs}. The SpinePair wrapper
contributes only what distinguishes the product domain---its schema tags, plan
fingerprint role, and exact product-query projections---while its nominal plan,
receiver, decoded, retention, rejection, and selection types remain distinct
from their scalar siblings, so evidence sealed under one domain cannot be
presented under the other even when the rendered wire bytes coincide.

Before any later live call inside one assessment scope, the internal ranking modules consult one selected replay cursor. Every established rank and direct Selection compatibility entrance constructs the raw bank: at most four exact counterexample input vectors, kept newest-first with exact-vector deduplication. Integration's additive context-aware filter entrance instead supplies a nominal scalar or product bank. Either path freshly evaluates every attempted input against that candidate's own checked query; a hit yields an ordinary validated counterexample receipt and avoids one live query. The stores contain inputs rather than cached verdicts, solver statuses, receipts, or proofs.

The current vendored Djex tree exposes nominal candidate-independent scalar and binary-product bank scopes, bounded immutable input stores, and exact query-owned operations for fresh recording and retained-sample replay. Scope construction binds the checked session inventory and provider-law basis, solver-neutral interpretation policy, exact contract, and alpha-canonical target while excluding the candidate and execution identity. A scope match authorizes only an attempted fresh replay. \filepath{src/Leant/Synth/Length/CounterexampleBank/Internal.hs} wraps that bridge in nominal package-private scalar/product states and command-tagged contexts. Each state owns validated limits and zero or one active bank, initializes lazily, discards the sole bank on scope drift, traverses samples newest first while preserving charged successors and ordered refusals, and requires explicit evaluation-free hit promotion. Command-tagged replay hits cannot be promoted through another context even at the same semantic identity.

Each context serializes one pure transition with an exception-restoring \code{MVar}. It fully evaluates the successor state and forces the outer expected classification to weak head normal form before commit. A synchronous or asynchronous exception restores the old state and propagates; an expected result installs the authoritative successor. The ranking, simplification, and solver loops remain outside that narrow transition boundary. An unsimplified hit promotes its exact retained sample without reevaluation; a simplified hit records the final receipt under simplification origin. Fresh live and solver-independent receipts retain their corresponding cache origins only when no strict reduction is found; every strict reduction records under simplification origin. Recording does not replace the candidate's pre-record final receipt.

When the selected replay source yields no hit, the current startup policy probes the canonical all-zero input vector owned by that exact query; lower-level programmatic policies may enable the same probe independently. A hit enters the same validated receipt and selected-bank path; an ordinary miss simply proceeds to live execution and carries no positive or negative authority of its own. Expected attempt or insertion caps retain authoritative context successors. Later ranking or selection fallback preserves candidate order but does not roll back already completed cache transitions.

Applicable-domain validation is the second positive axis beside the finite input box: one independent, query-owned exhaustive guarded recursive piecewise-affine traversal for the scalar or binary-product domain. Leant exposes only \nolinkurl{enableLengthRankingApplicableDomainValidation}. Djex exposes the short \nolinkurl{validateLengthProblemApplicableDomain} and \nolinkurl{validateLengthSpinePairProblemApplicableDomain} checked-problem validators plus \nolinkurl{validateLengthSMTLibQueryApplicableDomain} and \nolinkurl{validateLengthSpinePairSMTLibQueryApplicableDomain}, together with the short receipt and error families. Direct, affine, relational, strict, quotient, extrema, monus, Boolean-union, and atomic analyses are ordered private fallbacks inside that validator, not public strategy choices. Their former public builders, receipts, assessments, failures, renderers, and schema tags were deleted without aliases or migration because the projects are experimental and promise no stability or backward compatibility. Contract-only files never select ranking policy, and, like the input box, the external solver status itself never becomes the evidence.
...
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/CounterexampleBank.hs} & Nominal scalar/product candidate-independent scope identities and bounded immutable input-vector stores; matching permits an attempted fresh replay and grants no verdict or evidence. \\
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/Problem.hs} & Atomic checked session with session-owned provider inventory, separately revalidated contract, candidate semantics, complete Length problem, and retained candidate-independent bank scope. \\
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/Evaluate.hs} & Bounded concrete natural-number evaluator and the only constructor path for validated Length counterexamples. The scalar and product counterexample, finite input-box, simplification, and applicable-domain families run once over a private per-domain record with a shared three-way replay view; only the two assignment-replay implementations and the contract evaluators stay per-domain (the product side evaluates two result components). \\
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/SMTLib.hs} & Transient typed QF\_LIA planning, canonical bounded bytes, structurally complete query fingerprint, retained problem-scope projection, exact same-scope fresh-record and retained-sample replay bridges, and model-binding validation. Internally the scalar and product boundaries share one plan sealer, one fingerprint builder over a role/schema/logic vocabulary, and one model reader over each domain's error constructors, and the eight query-owned operation pairs (model validation, input replay, the origin probe, bank recording and retained-sample replay, and the validation associations) run once over a private query-surface record, while every public query, fingerprint, and error type stays nominally per-domain. \\
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/Where.hs} & Bounded ASCII where-clause surface syntax for one Length postcondition: two byte- and nesting-bounded parser entrances over one grammar core -- compact \code{len(...)} and Haskell-shaped \code{length}/\code{fst}/\code{snd} spellings, each rejecting the other's operators -- producing the same opaque, source-free parsed value, and a separate elaborator requiring the caller's explicit scalar/binary-product domain and complete source-ordered role vector before lowering observed physical \code{argN} references onto the existing passive contract-source vocabulary; no spine, role, provider, solver, or execution authority is inferred. Leant Main and the standalone Djex REPL are both authority-owning consumers; neither changes this passive Djex boundary. \\
...
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/SMTLib/Z3/Process.hs} & Shared opaque profile-driven subprocess runtime with one terminal masked resource-publication cell, rollback-protected native descriptor-child handoff, exec-status-handle finalization, associated schema-free observation and limit fields, FIFO typed pipe events, cancellation/deadlines, and staged cleanup without domain identity authority. \\
...
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/SMTLib/Z3/Process.hs} & Opaque raw Z3 owner with the established pathname-snapshot launcher, sealed-main-image branch, effective-ID executable-access sibling, and execve-check successor: two raw descriptor-based \code{faccessat2} observations, two source and one staged \code{AT\_EXECVE\_CHECK} observations for the newest strategy, bounded one-pass source hash/copy, optional pin admission, fixed staged \code{0500}, verified anonymous-file seals, descriptor-only execution, one masked raw-child-to-process ownership transfer shared by all three native strategies, signal/process-group ownership, and fail-closed cleanup without pathname or strategy retry. \\
...
\midrule
\endhead
\filepath{src/Leant/Synth/Engine.hs} & One prepared semantic origin shared by search and rendering, provider-binding-owned declaration and renderer projections plus assignment order, Exference run authority, typed semantic sidecars, opaque verification provenance, exact-origin rerender mechanics, and an additive opaque bounded cursor over one lazy detailed outcome. \\
\filepath{src/Leant/Synth/Verification.hs} & Lazy callback verification parameterized by a successful-group quota, with opaque nominal acceptance receipts; unchanged by lane-local refill and the Main-private outcome refactor. \\
...
\filepath{src/Leant/Synth/Length/Command.hs} & Exact fixed-order established \code{:synth [-{}-behavior-mode rank|filter] [-{}-length-contract PATH] -{}- TYPE} parser, with ordinary no-option goals left delimiter-free and opaque, plus a separate inline entrance consumed by Main's filter-only runner. The inline form has fixed option order, admits no repetition, is mutually exclusive with the contract-file form, and obeys the established standalone-delimiter rule; Main gives it precedence and authorizes literal filter mode before entering the bounded clause source. \\
\filepath{src/Leant/Synth/Length/Where.hs} & Leant profiles over Djex's bounded where-clause syntax: two fixed exact-case \code{List} models, ASCII input-index admission, and a resolver that builds the complete source-ordered role vector from Lean-translated \code{SlotArrow}-only physical arity before elaboration expands the exact zero/step profile with no provider laws. The plan, parsed source, and expanded contract remain passive data; Main alone pairs the resolved selection with existing policy authority and one nominal context. \\
\filepath{src/Leant/Synth/Length/Integration.hs} & Opaque disabled/configured mode, startup admission--load--activation setup, retained activation disposition, byte-free executable-launch classification, pre-IO explicit-contract permission, and one command-local mode-plus-policy-plus-lazy-contract request; a rank-2 \code{LengthAssessmentContext} whose filter branches own one nominal bank and rank branches own none; reusable context assessment used around Main's complete command scheduler and every progressive batch plus a fresh-per-call compatibility wrapper for other callers. \\
...
\filepath{src/Leant/Synth/Length/Ranking/Generic.hs} & The ranking control flow written once over the closed \code{LengthRankingDomain} class: package-private preparation orchestration, single-owner receipt-association/query state, scoped serial execution, a raw-four-vector or nominal-context replay cursor threaded through eager/deferred and unbudgeted/v1/scoped runners, the query-owned all-zero origin probe, \code{unsat}-triggered finite input-box validation, one complete guarded recursive piecewise-affine applicable-domain route, exact promotion/record timing, query-owned evidence replay, handle-preserving no-prune ordering, and candidate-report fallback without cache rollback. Each Djex outcome the flow inspects arrives through a small shared view, so the core never names a domain's constructors. \\
\filepath{src/Leant/Synth/Length/Ranking/Internal.hs} & The scalar instance of that core: the nominal assessment, failure, policy, and opaque receipt types as newtypes over the shared structure, the handoff and query refusal classifiers, the scalar bank surface and bridge, and the established public entrances (including the three command-local bank-context entrances) with byte-identical signatures. \\
\filepath{src/Leant/Synth/Length/CounterexampleBank/Internal.hs} & One package-private bank implementation over Djex's query-owned replay bridge, parameterized by each domain's limits, bank, scope, sample, receipt, and error types, with nominal scalar and product states, command-tagged contexts, and domain-named wrappers around it: validated limits, lazy one-scope ownership and reset, newest-first charged traversal, ordered bounded refusals, explicit evaluation-free promotion, fresh-replayed origin-aware recording, immutable snapshots, and serialized exception-restoring commit after full successor-state and classification-WHNF forcing. Each domain contributes only a query-free \code{BankSurface} and a query-owned \code{BankBridge}. \\
...
\filepath{src/Leant/Synth/BehavioralSelection.hs}, \filepath{BehavioralSelection/Internal.hs} & Domain-neutral rank-2 occurrence handles, package-internal retain/reject builders, and bounded total-partition seal; structural association only, with no behavioral authority by itself. \\
...
\filepath{src/Leant/Synth/Length/SpinePair/Ranking.hs}, \filepath{Ranking/Internal.hs}, \filepath{PostVerification.hs}, \filepath{PostVerification/Internal.hs} & Nominal binary-product siblings of the scalar ranking and post-verification stack. \filepath{Ranking/Internal.hs} is the product instance of the shared ranking core---its own leaf types, refusal classifiers, and bank vocabulary over the one runner matrix and cursor---while the post-verification pair remains a distinct nominal twin; product receipts stay distinct types from scalar ones end to end. \\
\filepath{src/Leant/Synth/Length/Selection.hs}, \filepath{Selection/Internal.hs}, \filepath{Selection/Generic.hs}, and SpinePair siblings & Nominal replay-authorized hard-selection adapters over one shared pipeline (\filepath{Selection/Generic.hs}, whose explanation, rejection, and result types take each domain's receipts as parameters and whose \code{SelectionDomain} record carries the per-domain failure naming): unchanged raw compatibility entrances plus additive context-aware entrances; only independently replayed counterexamples reject; every other assessment retains; any adapter/ranking/index/seal failure preserves the exact original batch without rewinding completed bank transitions. \\
\filepath{src/Leant/Synth/Length/Presentation.hs} & Opaque same-ranked/selected/rejected-receipt candidate presentation, exact existing counterexample/simplification rejection notes, retained bounded-positive notes, and no detached evidence reassociation. \\
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, established pre-translation request contexts, and the active inline route whose structure--goal--filter-authority--bounded-parse--scope-report--translation/retry--\code{SlotArrow} resolution order precedes exactly one context; per-command rank/filter dispatch, opaque same-run cursor consumption, filter-only two-batch progression at the session's verification width (12+12 or 24+24 by default), single-batch rank/disabled and half-width excluded-middle policies, mode-sensitive classical deadlines, one verification and assessment per batch, chronological run frontiers and note ownership, private four-way batch classification and lazy reverse accumulation, no-verified/all-rejected continuation with full-frontier deduplication, first-survivor termination without quota filling or a third probe, one plural result finalizer, survivor-only \code{itN} binding, complete accumulated rejection rows, structural/classical and abnormal diagnostic gates, four-way descriptor-launch notices, and sanitized mode-neutral preserve-all warning. \\
\bottomrule
\end{longtable}

\section*{What remains deliberately opt-in}

Without \clicode{--length-ranking-config}, \filepath{src/Main.hs} selects the opaque disabled assessment mode. The ordinary \code{:synth TYPE} path and explicit \code{:synth --behavior-mode rank -- TYPE} path retain the \code{VerificationBatch} lazily, perform no assessment IO, read no file, open no solver, and project callback receipts in their exact historical order. No startup line or per-batch diagnostic is added. An explicit filter request, inline request, or any \clicode{--length-contract} request in this mode fails before clause parsing, path admission, file IO, Lean goal translation, or candidate verification because there is no activated execution policy with which to associate it.
...
With that policy activated, the established option-bearing grammar is \code{:synth [-{}-behavior-mode rank|filter] [-{}-length-contract ABSOLUTE-PATH] -{}- TYPE}. Mode precedes contract and the standalone delimiter leaves \code{TYPE} opaque; omitted mode means ranking. Filter without a contract uses the startup contract. A contract form admits and reads one current contract-only document with format \code{leant-finite-list-spine-length-contract} once before goal translation. The exact root contains \code{format}, \code{rankingDomain}, and \code{contract}; \code{scalar} and \code{binary-product} select nominal nested grammars with the same six fields and no \code{resultShape}. Both require ordered target roles and an explicit case policy and admit modulo and positive-literal quotient. The decoder parses once and has no historical-version migration or fallback. Contract-only files contain no execution, filtering, replay, ordering, simplification, or usable-work policy. A decoded command contract and its separately parsed mode choose only that command's operation and passive contract; the activated policy remains process-fixed, and no request, assessment context, bank, or selection result enters \code{ReplState}, history, snapshots, or a cache.

The active inline grammar is \code{:synth}
\code{-{}-behavior-mode filter} \code{-{}-length-model}
\nolinkurl{list-scalar-exact-cases|list-binary-product-exact-cases}
\code{-{}-length-inputs} \nolinkurl{arg0[,argN...]}
\code{-{}-where CLAUSE -{}- TYPE}. Its unquoted
ASCII relation is parsed through Djex's bounded source-free frontend only after
literal filter authorization. Main then reports inaccessible hypotheses and
premise scope, translates/retries the goal, counts only physical
\code{SlotArrow} entries, resolves result references and the complete role
vector, and opens exactly one scalar or product context for all lanes. The
fixed profile has true precondition, exact list zero/step cases, and no provider
laws. No inline field can replace startup execution policy, and a provider
preparation refusal remains a retained occurrence.
...
Every accepted startup document constructs one fixed operational bundle: descriptor-bound execve-check launch; selected newest-first replay and the all-zero origin probe; independent post-\code{unsat} input-box traversal; the current guarded recursive piecewise-affine applicable-domain traversal; both non-vacuous preferences; componentwise-lexicographic simplification; deferred opening; and one owner-thread-affine, cooperatively checkpointed scoped usable-work deadline. Established rank and direct Selection compatibility paths select the raw four-entry MRU; Integration's filter context selects its nominal bank. Numeric limits remain caller supplied and independently bounded. Construction and activation are pure. An all-pure batch captures and checks the deadline but opens no descriptor; the first live miss opens one lexical session for the remaining suffix. Structured failures preserve the original-order candidate-report fallback without rewinding completed context transitions.

Only candidates with direct or exact-duplicate-recovered typed Exference authority whose retained graph and provenance match the selected startup contract are eligible. The default \code{djinn} engine supplies no typed graph, so actual eligibility requires \code{exference} or \code{both}. A successfully replayed counterexample receives the bounded model-relative note described above; an independently completed finite input box or applicable domain receives its bounded-positive note. Disabled, preparation-refused, unassessed, heuristic-only, and atomic-fallback survivor paths add no semantic note.

Ranking keeps every verified occurrence and uses the established positive/neutral/counterexample order. It retains the historical success frontier of \code{synth-shown} groups, the raw MRU, and one engine batch. Filtering instead constructs one total scalar or nominal pair partition for each selected cursor batch: \code{synth-verify} groups for a standalone engine, twice that for \code{both}, or half that for the excluded-middle route --- 12, 24, and 6 under the defaults. An ordinary filter run may select one same-width successor after no verification or complete all-rejection; excluded middle never does. It retains preparation refusals, unassessed candidates, every raw solver status, bounded-positive candidates, and applicable-domain establishments. Only an independently replayed counterexample becomes rejected. The generic partition seal restores both subsequences to original callback order and supplies association, not evidence. Any selection failure preserves the literal original batch and presents no rejection; Main still caps that terminal preserve-all presentation at \code{synth-shown}, while completed nominal-bank transitions remain committed. Main takes at most \code{synth-shown} survivor presentations from the first terminal batch but prints every rejection accumulated before and within that terminal result.


  Main introduces an established startup/file request's reusable rank-2 context before initial goal translation. The inline request translates and resolves first, then introduces exactly one context. Either carries its context through both same-run batches and every baseline, provider, excluded-middle, and double-negation route. Rank contexts remain bank-free; filter contexts reuse one nominal bank, and every retained sample still requires independent replay against the later query.

  \textbf{One lane outcome per batch.} Main represents each bounded batch with a private lazy lane outcome: one field retains the complete bounded rendered-spelling frontier used for provider deduplication, while a separate trace contains only detailed variants recorded immediately before actual Lean callbacks. A private run receipt restores batch order, concatenates those complete frontiers, preserves cumulative debug accounting, and owns run-note placement. Verification and assessment perform no presentation or splice effects. A pure classifier distinguishes no verified receipt, survivors, all behavioral rejections, and preserve-all assessment failure.

  \textbf{Accumulation and the finalizer.} Main prepends actual batch outcomes to a lazy reverse accumulation; no-verified and all-rejected may consume the filter successor or enter a later route after the run finishes, while survivor and preserve-all stop. The first survivor is terminal even below five. One plural finalizer owns chronological metrics, effective-prefix warnings, one final binding/cache phase, terminal candidate rows, accumulated rejections, and handled notes. No context or bank crosses the command, and there is no third same-run probe, engine rerun, reassessment, quota filling, counterexample-directed engine search, persistence, or \code{ReplState} change.

The current applicable-domain algorithm first runs the complete private atomic predecessor for every signed relational leaf. Recursive interpretation is attempted only for an exact singleton ignored result that still contains minimum, maximum, monus, or a conditional in a relation operand. It expands compact inputs, literals, normalized sums, retained positive scales, binary extrema, binary monus, and fully supported \code{LengthIf} nodes into ordered guarded signed-affine values. A conditional is all-or-nothing: every leaf of both its condition and complement and every descendant in both selected arms must be supported. Quotient, modulo, result reference, out-of-range variable, retained zero scale, or another unsupported descendant rejects the complete recursive fallback atom.
...
\begin{itemize}
  ...
  \item \code{L monus R} gives \code{[L<=R; value 0] | [R+1<=L; value L-R]}; and
  \item \code{if F then T else E} gives positive-DNF guard cases paired with \code{T}, followed by negative-DNF guard cases paired with \code{E}.
\end{itemize}

The first extrema/monus choice owns equality. Conditional true-arm alternatives precede false-arm alternatives; condition-DNF alternatives are outermost inside an arm and condition guards precede selected-arm guards. Descendant guards precede the current selector, left-child cases precede right-child cases, and the relation appends its at-most, immediate strict-complement, or two equality rules last. Signed coefficients created by a positive monus branch transfer exactly across the inequality into the established natural positive-sided rule representation. No checked formula is manufactured and no selector, conditional guard, or relation rule is deduplicated.

The generated-branch cap observes the complete formula-DNF by recursive-alternative Cartesian product before formula cleanup, conditional-guard or selector contradiction, rule collection, closure, or box cleanup. A contradictory conditional guard therefore retains its selected value in the raw product and collapses only when the enclosing relation forms coverage. Original literal sets are canonicalized and re-expanded in set order. The immutable-snapshot rule-once closure, zero-origin box antichain, raw visits, globally deduplicated lexicographic assignments, and exact original-formula replay remain authoritative. Incomparable boxes are never replaced by a hull.

The one-input characterization \code{(if x <= 2 then x else 5) <= 3} has two raw guard alternatives and retains provider-independent \code{[[2]]} with one box, three visits, three unique assignments, and three applicable assignments. A negative equality guard retains its two strict complement alternatives, producing three raw cases. The nested fixture \code{y<=2} and \code{(if x<=1 then max(x,y) else x monus y)<=2} has four raw alternatives and retains \code{[[4,2]]} with 1/15/15/12 box, visit, unique, and applicable counts. A wider independent replay oracle finds no satisfying assignment omitted by the box, while replacing either selected arm with an unsupported quotient or modulo rejects the whole fallback atom.

Complete replay releases \code{ApplicableDomainEstablished} or its nominal \nolinkurl{LengthSpinePairApplicableDomainEstablished} sibling, carrying \code{ValidatedLengthApplicableDomain} or \nolinkurl{ValidatedLengthSpinePairApplicableDomain}. \nolinkurl{renderLengthApplicableDomainValidationNote} and \nolinkurl{renderLengthSpinePairApplicableDomainValidationNote} report model/provider-relative basis, boxes, raw visits, unique and applicable assignments, explicit vacuity, and a bounded maxima prefix. They grant no hull, solver proof, global theorem, or pruning authority. The current file-schema resets are recorded in \filepath{docs/reports/2026-08-15-versionless-length-ranking-configuration.md} and \filepath{docs/reports/2026-08-15-versionless-length-contract.md}; the current guarded behavior is in \filepath{docs/reports/2026-08-15-guarded-conditional-length-ranking.md}, the connected selection path is recorded in \filepath{docs/reports/2026-08-15-command-authorized-length-filtering.md}, its historical lane-local refill is recorded in \filepath{docs/reports/2026-08-15-lane-local-length-survivor-refill.md}, the private outcome seam is recorded in \filepath{docs/reports/2026-08-16-explicit-synthesis-lane-outcomes.md}, the historical pure bank-state foundation is recorded in \filepath{docs/reports/2026-08-16-nominal-length-counterexample-bank-state.md}, its filter-only context-runner successor is recorded in \filepath{docs/reports/2026-08-16-filter-only-length-counterexample-bank-context-runner.md}, Main's command-local successor is recorded in \filepath{docs/reports/2026-08-16-command-local-length-counterexample-bank-scheduler.md}, the Engine observation foundation is recorded in \filepath{docs/reports/2026-08-16-opaque-detailed-synthesis-cursor-foundation.md}, and its progressive Main runtime successor is recorded in \filepath{docs/reports/2026-08-16-progressive-same-run-length-filter-batching.md}, while the earlier reports remain non-normative implementation history at their landing checkpoints. Djex's exact guarded semantics and private identity boundary are in \filepath{lib/Djex/docs/reports/2026-08-15-guarded-conditional-length-applicable-domain.md}; its candidate-independent store and fresh-replay bridge are in \filepath{lib/Djex/docs/reports/2026-08-16-nominal-length-counterexample-bank-foundation.md} and \filepath{lib/Djex/docs/reports/2026-08-16-length-counterexample-bank-query-replay-bridge.md}.
...
\dpath{synthesis/src/Language/Haskell/Synthesis/Selection.hs} separates search generation from candidate selection. A backend may expose a lazy stream with scores and progress; a client chooses a bounded subset, filters it, and preserves the strongest progress observation. Admissibility can be checked purely or, through the monadic sibling selector, by an effectful check that runs once per candidate in encounter order while preserving the pure selector's batch boundaries and laziness. This is particularly important for Leant, which performs its own cross-engine merge and final verification window.
...
\midrule
\endhead
\dpath{src/Language/Haskell/Djex/REPL/Command.hs} & Command grammar, abbreviations, arguments, and command-level diagnostics, including the structured \code{-{}-where CLAUSE -{}- TYPE} query envelope retained with each type query. \\
...
\dpath{src/Language/Haskell/Djex/REPL/LengthWhere.hs} & Conservative built-in-list defaults for REPL Length constraints: observes every structural list argument of the elaborated target, accepts only a scalar list result or a boxed pair of list results, and seals the session and contract through the existing checked Length boundaries; inert until checked runtime activation. \\
...
\subsection{Settings are an enumeration, not a string table}

Djex's own settings are worth studying before Leant's (Section~\ref{sec:leant-settings}), because the two REPLs solve the same problem differently and the difference trips people up.

Djex names every option once, as a constructor of the \code{ReplSetting} type in \dpath{src/Language/Haskell/Djex/REPL/Command.hs}. The type derives \code{Bounded} and \code{Enum}, and \code{replSettingName} is a total function from constructor to spelling. Three consequences follow without any extra bookkeeping:

\begin{itemize}
  \item \code{:show settings} enumerates \code{[minBound .. maxBound]}, so a newly added setting is displayed the day it is added and cannot be forgotten;
  \item tab completion and command execution consult the same list, so they cannot drift apart;
  \item \code{replSettingIsBoolean} decides GHCi-style \code{+NAME}/\code{-NAME} eligibility on the setting type rather than at the parse site, so a newly added non-boolean option cannot accidentally accept \code{on} merely because the invocation used a sign.
\end{itemize}

Beside the long-standing backend, target, selection, rendering, qualification, prompt, search-limit, and axiom options, three settings deserve a note:

\begin{itemize}
  \item \code{:set timeout N} is a wall clock over the \emph{whole} command, not over one engine. It has to be, because the two backends do their work at different moments: Djinn searches when its \code{Either} result is matched, and Exference searches while its candidates are being printed. A timeout wrapped around either engine alone would expire in the wrong place.
  \item \code{:set djinn-strategy depth-first|interleave} selects the LJT search order. The value travels from the REPL through \code{QueryOptions} to the one production \code{SearchMode}; the \code{Strategy} type is re-exported along \code{Core} $\rightarrow$ \code{Djinn/Internal/Request} $\rightarrow$ \code{Djex.Djinn} so that callers can name it without reaching into internals.
  \item \code{:set heuristic NAME VALUE} sets one Exference scoring weight by name. The names come from \code{heuristicFields} and the writers from \code{heuristicAssignments}, which sit beside each other in \dpath{exference/src-core/Language/Haskell/Exference/Core/Internal/Options.hs} precisely so that a weight cannot gain a reader without also gaining a writer. The thirteen names are \code{goalVar}, \code{goalCons}, \code{goalArrow}, \code{goalApp}, \code{stepProvidedGood}, \code{stepProvidedBad}, \code{stepEnvGood}, \code{stepEnvBad}, \code{tempUnusedVarPenalty}, \code{tempMultiVarUsePenalty}, \code{functionGoalTransform}, \code{unusedVar}, and \code{solutionLength}.
\end{itemize}

\begin{pitfallbox}[title={Two REPL grammars that look alike}]
The \code{:set} grammar is whitespace-separated, and the \emph{first} \code{=} in the argument is taken as part of the setting name. \code{:set heuristic goalVar=3.5} is therefore rejected as an unknown setting; write \code{:set heuristic goalVar 3.5}.

A bare \code{:set NAME} is also not a getter in Djex --- it reports that the setting requires a value. Use \code{:show settings} instead. Leant's REPL made the opposite choice and does treat a bare known name as a getter, so a habit formed in one shell misfires in the other.
\end{pitfallbox}
...
At the guarded-conditional documentation checkpoint those sixteen suites pass
1{,}807 tests in total, including 370/370 in the Length suite.
...
\lpath{src/Leant/Synth/Engine.hs} & Fragment-to-Djex translation, Djinn and Exference sessions, provider assignments, library premises, staged engine runs, candidate merging, evidence policy, bounded forcing, and the opaque detailed-outcome cursor. \\
...
\section{Session settings and \code{:set}}\label{sec:leant-settings}

A large part of the synthesis state is reachable from the prompt. \code{:set} is the single door, and understanding how it is dispatched removes most of the surprise from reading \filepath{src/Main.hs}.

The command has four shapes:

\begin{itemize}
  \item \code{:set} alone prints every synthesis setting with its current value. This is the first thing to ask for when reproducing a report, because the defaults are not guaranteed to be in force.
  \item \code{:set NAME} prints one setting. Leant treats a bare known name as a getter; this is worth noting because Djex's own REPL does not --- there a bare \code{:set NAME} is an error and \code{:show settings} is the getter.
  \item \code{:set NAME VALUE} assigns one setting, after validating the value. A rejected value leaves the previous one in place and prints a usage line rather than failing the command.
  \item \code{:set OPT VALUE} for any name Leant does not recognize is forwarded to the Lean backend as \code{set\_option OPT VALUE}. Leant's own settings are therefore \emph{intercepted} from a namespace it shares with Lean, which is why the recognized names all carry a \code{synth-} or \code{backend-} prefix: the prefix keeps the two namespaces from colliding as Lean gains options.
\end{itemize}

The settings divide into three groups by what they own.

\textbf{Search shape.} \code{synth-engine} (\code{djinn}, \code{exference}, or \code{both}), \code{synth-steps} (the Exference step budget, default 4096), \code{synth-classical}, and \code{synth-library} choose which lanes run at all.

\textbf{Search bounds.} \code{synth-shown}, \code{synth-verify}, \code{synth-window}, \code{synth-budget}, and \code{synth-queue} are the five fields of the \code{SynthLimits} record described in Appendix~\ref{app:limits}. \code{synth-timeout} is the wall clock one \code{:synth} may spend before answering ``no answer'' (default 20 seconds; \code{0} waits indefinitely; \code{LEANT\_SYNTH\_TIMEOUT} seeds it at startup). \code{synth-providers}, \code{synth-provider-cap} (default 80), and \code{synth-library-premises} (default 8) bound provider discovery and rated library offers. \code{backend-timeout} bounds one Lean request rather than one synthesis (default 300 seconds, seeded by \clicode{--timeout}).

\textbf{Observation.} \code{synth-debug} prints fragments, providers, per-lane outcomes, and metrics; \code{synth-ratings} lists the rated library functions rather than setting anything.

\begin{invariantbox}[title={Settings are session state, not replay input}]
Every setting lives in \code{ReplState} and none of them enters replay history, the undo stack, or a snapshot. Reconstructing a session therefore reproduces its \emph{declarations}, not its tuning: after \code{:unpickle}, the search bounds are whatever the current session last set them to. This is deliberate --- a snapshot records what the Lean environment contains, and search policy is not part of that environment --- but it means a golden transcript is only reproducible together with the settings that produced it.
\end{invariantbox}

\begin{designbox}
The settings were once compiled-in constants, and the migration deliberately kept \code{defaultSynthLimits} byte-compatible with them so that no existing transcript, golden file, or documented example changed meaning. When adding a new tunable, thread it through the existing policy records rather than adding an argument to an observation function: the characterization tests pin exact call sites, and a new positional argument on \code{runSynthLaneCursor} breaks the pinned one-liners while a new \code{SynthLaneCursorPolicy} field does not.
\end{designbox}
...
The module bounds backend candidate windows, combined verification frontiers, attempted groups, and shown successes. These are no longer compiled-in constants read directly at each use site. They are the five fields of one \code{SynthLimits} record, and \code{defaultSynthLimits} installs exactly the historical values, so an untouched session behaves as it always did while a REPL user may retune any field with \code{:set} (Appendix~\ref{app:limits}). Every number in the rest of this section is therefore the \emph{default}; read it as ``what an unmodified session does''.

With the defaults in force, ranking stops verification after five accepted groups (\code{synth-shown}) and consumes one candidate batch. Filtering may consume two ordered batches from one ordinary lazy engine outcome: 12+12 groups for a standalone engine or 24+24 for a combined engine, since the combined engine doubles \code{synth-verify}. Only no verification or complete all-rejection reaches the successor. Excluded middle remains one six-group batch --- \code{synth-verify} halved --- while double negation uses the ordinary policy and a filter-only Djinn candidate cutoff of 60, which is \code{synth-window} rather than a separate constant. Every batch is verified and assessed once through the command context; survivor or preserve-all stops, and the second batch ends without a third tail probe. Only after a terminal outcome does Main take five survivor presentations. This is not a survivor quota within a run or across lanes: the first batch with one survivor is terminal. The batch prefix remains separate from the successful-group quota because a failed group does not consume that quota.

Two maintenance rules follow from the record. First, the limits are threaded, not fetched: a lane receives its bounds as an argument, and code should not introduce ad hoc \code{take 5} operations that both lose progress accounting and ignore a retuned session. Second, one setting can silently narrow another --- a batch prefix wider than \code{synth-window} is clamped by \code{admissibleCursorBatch} --- so Main emits an explicit note when the admitted batch differs from the requested frontier rather than letting the two settings disagree in silence.

\section{Opaque bounded observation of one detailed outcome}

The Engine module now exports an additive \nolinkurl{DetailedSynthCursor}
without exporting its constructor. \nolinkurl{DetailedCandidateBatch} is opaque in the same
way and can be projected only to its ordered groups and the original run-level
notes. Both representations are positional and lazy, with no record selector,
strict field, \code{Eq}, or \code{Show} instance. The batch constructor remains
inside Engine, so a candidate step cannot carry an empty group slice or an
arbitrary caller-manufactured note association. This is ordinary pure-Haskell
opacity rather than linear ownership: an old cursor may be retained, but every
successor observes the same retained detailed outcome and never launches the
engine again.

\nolinkurl{startDetailedSynthCursor} does not demand the outcome it wraps. An
advance request is admitted only from 1 through the session's candidate window
--- \code{synth-window}, 60 by default. The window is a plain argument to
\nolinkurl{advanceDetailedSynthCursorWith} rather than a cursor field, so a
cursor cannot carry a stale bound past a \code{:set}, and the laziness
regressions that poison a cursor tail still observe the value the caller
supplied. Nonpositive and over-limit requests are rejected
before the cursor is evaluated, and the outer successful admission likewise
does not demand it; only forcing the returned step observes the outcome. An
ordinary step carries one nonempty batch plus a lazy successor. Engine failure,
the exact refutation soundness flag, and no-term completion remain distinct
terminal constructors rather than being flattened into an empty candidate
batch.

The cursor counts the actual groups returned, preserves their source order,
and clamps a request near the cumulative window cap to the remaining
allowance. It reports natural exhaustion only after an advance before that cap
observes an empty tail. Once the window's last group has been returned, the
next advance reports the hard cap without examining the group after it.
Consequently a stream of exactly the window's length --- 60 elements under the
default --- is intentionally classified as capped, not naturally
exhausted, and a bottom or cyclic tail remains productive. Every candidate
batch and each note-bearing completion retains the original notes unchanged;
Main's private scheduler now decides their exactly-once presentation. The
Engine cursor itself still owns no such policy.

\nolinkurl{forceDetailedSynthCursorStep} is the matching deadline force target. For
a candidate step it traverses the selected group-list and variant-list spines
and their constructors far enough to project text, forces each selected route
to weak head normal form, traverses each selected rendered spelling's
\code{String} list spine through \code{length}, and traverses the run-note list
spine and each note's \code{String} spine through \code{length}. It does not
force \code{Char} values or perform text encoding, and leaves semantic sidecars,
recovered exact-origin lookup, the successor, and the unselected stream tail
lazy. Terminal force behavior matches the established
\nolinkurl{forceDetailedOutcome}, and the old helper now shares the same private
group and note forcing functions. Neither pure helper installs a clock or
catches an exception; the IO caller owns that boundary.

The executable consumes this surface through private Main-only orchestration.
\filepath{src/Main.hs} imports the opaque cursor type without its constructor,
the visible step taxonomy, batch accessors, and start/advance/force operations.
It neither names the hidden batch type nor imports either hidden constructor.
\nolinkurl{runSynthLaneCursor} starts one cursor for one retained
engine outcome. Rank and disabled policies stop after one candidate batch;
ordinary filtering may consume one successor after no verification or complete
all-rejection. Every batch is verified and assessed exactly once under the same
command context, and Main never reruns the engine or probes a third tail.

The private run receipt concatenates the complete batch spelling frontiers,
keeps cumulative group counts and debug ordinals, and restores outcomes to
chronological order before adding them to the command accumulation. Ordinary
run notes attach once to the rightmost handled outcome; an all-no-verification
run retains them only for its final diagnostic, while classical handled
outcomes remain note-free. The cursor itself still has no assessment context,
counterexample bank, provider-deduplication policy, survivor quota, persistent
state, or counterexample-directed engine input.
...
\begin{enumerate}
  \item recognize the established or inline command and resolve its explicit or current prove/\code{sorry} goal source;
  \item for startup/file requests, introduce one nominal rank-2 assessment context and translate inside it; for inline requests, authorize literal filter mode, parse the bounded clause, report inaccessible hypotheses and premise scope, translate with any universe-narrowing retry, count only physical source-arrow slots, resolve the fixed profile, and then introduce exactly one context;
  ...
  \item start one cursor for the retained outcome; verify and assess one bounded batch, optionally one filter successor after no-verification or all-rejection, and stop on survivor, preserve-all, or the literal two-batch ceiling;
  \item discover providers, then run one or more widening provider lanes, deduplicating both continuing classes by their complete bounded spelling frontiers;
  \item after a retained sound refutation, optionally run excluded-middle and then double-negation classical lanes under the same context;
  \item finalize the lazy reverse lane accumulation once, then apply the structural or abnormal diagnostic gate.
\end{enumerate}

The exact path short-circuits on the first survivor or preserve-all result, not after filling five slots. It cannot short-circuit to a refutation before later constructive lanes required by policy have run, and the rank-2 context cannot escape the command.
...
\begin{itemize}
  ...
  \item a Djinn refutation can survive as the structural fallback only if no constructive lane has a terminal survivor/preserve-all result and all completeness conditions hold; completed all-rejected outcomes remain behavioral output;
...
Both backend result streams may be lazy. Main forces only the bounded prefix required for candidate grouping, plus progress sentinels where needed, under the command timeout. Its established forcing function evaluates enough structure that exceptions and runaway search occur inside the timeout boundary rather than later during printing.
...
The cursor does not relax that rule. Main uses its step-specific force target to
put each selected filter batch under the same absolute command deadline while
leaving the successor and remaining stream dormant. Merely calling
\nolinkurl{advanceDetailedSynthCursor} is not the deadline boundary, because its
admitted outer result and selected payload deliberately remain lazy.

\section{Classical fallback}

When enabled after a retained sound refutation and the Glivenko split admits the fragment, Leant first offers bounded excluded-middle premises, then applies a double-negation transformation when excluded middle has no verified candidate or is entirely behaviorally rejected. Both routes use the command's assessment context. Excluded middle is always one batch of \code{synth-verify} halved --- six groups by default. Filter-mode double negation follows the ordinary 12+12 or 24+24 policy and raises the tuned Djinn candidate cutoff to \code{synth-window} (60 by default); rank and disabled modes retain one 12- or 24-group batch and a cutoff of \code{synth-verify} (12). The classical routes therefore inherit the session's retuning rather than pinning their own numbers. Filtering keeps the original absolute command deadline through both routes. Rank and disabled modes instead capture independent fresh configured-duration deadlines when EM and NN are actually reached; zero remains unbounded. A survivor or preserve-all result stops; every actual batch outcome is accumulated, but neither route contributes negative evidence because:
...
\midrule
\endhead
Djinn exhausted; complete formula and fragment; no budget; required provider lanes have no terminal survivor or preserve-all result & Structural fallback & All known lossy boundaries are absent and proof search established uninhabitability of the translated problem; completed all-rejected lanes remain separately handled. \\
...
Verified batch has one retained candidate, or assessment preserves all & Candidates & The survivor/preserve-all disposition is terminal even below five presentations and dominates prior fallback refutation. \\
Verified batch is entirely behaviorally rejected & Continue or final all-rejection & Only independently replayed counterexamples authorize this result; it carries its complete frontier and rejection rows but does not masquerade as no verification. \\
\bottomrule
\end{longtable}
...
Main's cursor driver supplies one exact bounded batch before verification
chooses its successful-group quota. Ranking and disabled modes consume one
batch; ranking stops after \code{synth-shown} acceptances, five by default.
Filtering traverses a complete batch of \code{synth-verify} groups for a
standalone engine or twice that for the combined engine --- 12 and 24 under
the defaults --- and passes all callback
acceptances to one behavioral assessment under the command's context. Only a
no-verified or all-rejected disposition may consume one same-width successor
from the same cursor. Excluded middle remains one half-width batch, six groups
by default; double
negation uses the ordinary policy. Each batch is verified and assessed once,
and a continuing second batch ends without a third probe. Failures can be
retained for diagnostics but are not shown as candidates. After the first
terminal filter outcome, at most \code{synth-shown} survivors are displayed and
bound, while
every accumulated rejection is reported. This is not quota filling: one
survivor stops the scheduler.

Each batch outcome retains two histories with different authority. The checked
frontier contains every rendered spelling in the complete bounded batch, even
when callback laziness skips it. The private run receipt concatenates those
frontiers in batch order and remains the provider-stage deduplication input.
The exact callback trace records a detailed variant
immediately before its backend request; it therefore contains failures before
an accepted spelling but excludes that group's later siblings and every group
after the successful-group quota. Provider scheduling never substitutes this
narrow trace for the complete frontier. Candidate counts, debug ordinals, and
outcomes likewise remain chronological across both batches.

Verification and optional Length assessment return one private lazy
\code{SynthLaneOutcome} without emitting result rows or changing generated
bindings and proof splices. The pure disposition is no verified receipt,
survivors, all behaviorally rejected, or assessment preserved. Main prepends
every actual batch outcome to a lazy \code{SynthLaneAccumulation} in reverse
attempt order. No-verification and all-rejection may consume the filter
successor or reach a later engine run; survivor and preserve-all are terminal.

One \code{finalizeSynthLaneAccumulation} owns the command's result effects.
It emits raw chronological metrics, then uses the effective prefix through the
first terminal disposition for warnings, one final binding/cache phase, rows, and
handled notes. Terminal candidate rows precede prior-all-rejected plus terminal
rejection rows. Ordinary run notes are attached once to the rightmost handled
same-run outcome; all-no-verification notes remain only for the final run
diagnostic, and classical handled outcomes carry none. Final all-rejection
clears once; terminal
survivor or preserve-all replaces once.

Normal aggregate all-rejection suppresses an unrelated no-survivor/no-term
diagnostic. A retained sound structural refutation still masks provider
timeout/error, but completed outcomes enter the classical accumulation;
excluded-middle no-verification or all-rejection enters double negation. Its
caller finalizes once and suppresses the refutation claim and constructive hint
for aggregate all-rejection while preserving the unresolved-universe
annotation. Without that fallback, timeout/error finalizes completed outcomes
before the unchanged abnormal diagnostic.
...
\begin{tikzpicture}[node distance=7mm and 7mm]
  ...
  \node[greenbox,below=12mm of verify,text width=34mm] (accepted) {Verified groups\\mode-owned quota};
  \node[redbox,below=12mm of render,text width=34mm] (reject) {Reject spelling; try\\next variant/group};
  \node[goldbox,below=of accepted,text width=34mm] (assess) {Optional Length\\batch assessment};
  \node[graybox,left=of assess,text width=34mm] (bind) {Display and bind\\survivors up to the\\session's shown limit};
  \node[redbox,right=of assess,text width=34mm] (breject) {Report every bounded\\behavioral rejection};
  ...
  \draw[flow] (accepted) -- (assess);
  \draw[flow] (assess) -- (bind);
  \draw[flow] (assess) -- (breject);
\end{tikzpicture}}
\caption{The candidate funnel. The exact Lean target remains the acceptance gate; optional behavioral filtering partitions each bounded cursor batch, with at most one successor, before the presentation cap of \code{synth-shown} survivors (five by default).}
...
An accepted all-rejected behavioral partition is the explicit continuing
all-behaviorally-rejected disposition. It creates no binding or immediate cache
effect, retains its rejection rows, and may consume the one filter successor or,
after that run finishes, enter another provider or classical route. Only final
aggregate all-rejection prints every accumulated rejection and clears the
earlier synthesis-splice cache. Preserve-all assessment failure has
its own terminal disposition, keeps its complete internal batch, and presents
at most \code{synth-shown} original candidates --- five by default --- before
retained earlier rejections. Neither
result signals Lean verification failure; continuation remains bounded to the
existing lane schedule and is not survivor-quota filling.
...
\lpath{test-unit/Spec.hs} is a broad unit suite covering JSON, classifiers, snapshot metadata and publication, replay decisions, fragment parsing and analysis, provider queries and cache behavior, Djex translations, assignment fitting, rendering, candidate grouping, and many regressions around rank-N providers and recursive families. At the progressive same-run batching checkpoint the suite has 23{,}602 lines and 455 exact \code{testCase} tokens, 454 of which register and execute as suite cases. The retained ten-case cursor group covers ordered nonempty batching, unchanged notes, terminal taxonomy, request admission before cursor demand, start and outer-result laziness, natural exhaustion versus the exact 60-group cap, poison and cyclic tails, legacy force parity, selected-payload forcing, opaque representation, and Main's accessor-only use without hidden-constructor reconstruction. The suite's single largest area remains the Length/SpinePair behavioral-assessment layer: the current versionless scalar/product startup decoder, exact \code{:synth} behavior-mode and contract grammar, disabled-before-I/O authorization, guarded recursive piecewise-affine applicable-domain behavior, the checked handoff, ranking, replay-authorized selection, post-verification and total-partition sealing, association-safe presentation, scalar/product dispatch, progressive same-run refill, command-local nominal bank contexts, cross-lane all-rejected continuation, and live execution against the \code{djex-fake-z3} fixture. Rank and direct Selection compatibility tests retain the raw MRU.

The lane-outcome characterization still separates the complete bounded spelling frontier from a deliberately shorter callback trace, pins every private lane-outcome field and four-way pure disposition, and rejects presentation or splice effects in the verification seam. Nine Main characterizations now pin one pre-translation command context, the lane/accumulation boundary, chronological first-terminal folding, exactly-once plural finalization, provider and EM-to-NN continuation after both nonterminal dispositions, private progressive policy/run receipts, the literal at-most-two-batch cursor driver, mode-sensitive classical deadlines, and the normal/structural/abnormal diagnostic gates. They freeze 12+12 and 24+24 filter progression, one-batch rank/disabled and EM routes, NV/AR continuation, S/P termination, no third probe, one verification and assessment per batch, chronological full-frontier and debug accounting, rightmost handled-note ownership, every run-end route, no rerun/reassessment/quota fill, and no \code{ReplState} ownership. The package-private bank runner group retains its seven scalar/product route, cap, sealing, failure, Integration-lifetime, and architecture cases.

The focused bounded cursor surface passes 10/10, the strict bank runner surface
passes 7/7, and explicit Length assessment integration passes 24/24. The
complete strict Leant unit suite passes 454/454. Strict warning-as-error builds
of the test target and complete project are green; \code{cabal check} and
\code{git diff --check} are clean. Against the pinned Lean 4.31 backend, all 26
golden transcripts pass byte-exact for this progressive checkpoint with exit
status zero, no update mode, and no fixture drift. The 26 inputs, 26 golden
files, and runner are unchanged.

\textbf{Inline activation evidence.} The production commit
\commitid{e078ca9168b71f8d8dab5cbe87d115384be855d5} connected the passive
where-clause foundation to Main, and
\commitid{0425ad9be5bcb8f8e3612b711e2fff94ad1c13be} characterized the runtime.
At the initial reconciled documentation baseline
\commitid{260d36d1dc2578ce7e470171c8591137569e455f}, with the same Djex revision
now pinned on the title page, the active inline group
passes 21/21, Integration passes 24/24, and the full strict unit suite passes
475/475. The strict all-target Cabal build and all 26 pinned-backend transcripts pass;
the latter use comparison-only normalization for machine-dependent queue
telemetry, without updating any golden fixture for inline activation. These
figures describe the activation before its final documentation reconciliation.
The title-page snapshots include the runtime, characterization, reconciled
source documentation, and the matching Djex gitlink.
...
\dpath{synthesis/src/Language/Haskell/Synthesis/Selection.hs} & Bounded stream observation and candidate selection, with pure and effectful admission variants. \\
...
Two kinds of bound appear in the table, and it matters which one you are looking at. Most rows are \emph{structural}: the number is compiled in, a validator or plan enumerator depends on it, and changing it is a source change with test consequences. The rows marked \emph{session default} are different. Those are the Leant search bounds collected in the \code{SynthLimits} record (\lpath{src/Leant/Synth/Engine.hs}), and a REPL user retunes them for the rest of the session with \code{:set}. The value in the table is what \code{defaultSynthLimits} installs at startup, so an unmodified session behaves exactly as the surrounding chapters describe; a session that has issued \code{:set synth-verify 30} does not. When you are reading a bug report, ask for \code{:set} with no argument before assuming the defaults were in force.

\begin{longtable}{@{}L{0.24\textwidth}L{0.24\textwidth}L{0.44\textwidth}@{}}
\toprule
\textbf{\code{SynthLimits} field} & \textbf{\code{:set} name} & \textbf{What it bounds} \\
\midrule
\endhead
\code{synthLimitShown} & \code{synth-shown} & Accepted candidate groups shown and bound. \\
\code{synthLimitTried} & \code{synth-verify} & Fresh groups Lean verifies per lane; the \code{both} engine doubles it, excluded middle halves it. \\
\code{synthLimitWindow} & \code{synth-window} & Candidate groups one lane may observe; also the cursor's hard cap and the filter-mode double-negation cutoff. \\
\code{synthLimitBudget} & \code{synth-budget} & Djinn choice-point budget of the ordinary and provider lanes; \code{off} is unbounded. \\
\code{synthLimitQueue} & \code{synth-queue} & Exference queue bound. \\
\bottomrule
\end{longtable}

Three further session settings sit beside them without living in \code{SynthLimits}: \code{synth-timeout} (the per-command wall clock), \code{synth-providers}/\code{synth-provider-cap} (live provider discovery), and \code{synth-library-premises} (rated library offers kept per goal). \code{synth-debug} toggles per-lane tracing. The library and classical lanes keep their own fixed budgets and are not retunable.
...
Exference queue capacity in Leant lane & 1024 \emph{(session default, \code{synth-queue})} & Keeps live provider/library search operational; exact pruning is reported. \\
Leant backend candidate window & 60 \emph{(session default, \code{synth-window})} & Gives renderer/verification room after deduplication and rejection; also the detailed cursor's cumulative hard cap. \\
Leant combined batch prefix & 24 \emph{(session default: twice \code{synth-verify})} & Bounds cross-engine semantic groups; filter mode may consume two batches from one cursor, at most 48 groups. Clamped down to \code{synth-window} when that is smaller. \\
Leant standalone batch prefix & 12 \emph{(session default, \code{synth-verify})} & Bounds Djinn- or Exference-only groups; filter mode may consume two batches from one cursor, at most 24 groups. Clamped down to \code{synth-window} when that is smaller. \\
Leant excluded-middle batch prefix & 6 \emph{(session default: \code{synth-verify} halved)} & Keeps the first classical route cheap; it never consumes a successor. \\
Double-negation Djinn candidate cutoff & 12 rank/disabled; 60 filter \emph{(session defaults: \code{synth-verify} and \code{synth-window})} & Lets filter mode expose a successor while retaining the historical one-batch classical policy elsewhere. \\
Leant ranking accepted-group quota & 5 \emph{(session default, \code{synth-shown})} & Preserves the historical rank verifier frontier. \\
Leant presented survivors & 5 \emph{(session default, \code{synth-shown})} & Caps bindings and interactive output after filtering without truncating rejection rows. \\
...
\begin{itemize}
  \item Djex pinned tree: \href{https://github.com/VladimirReshetnikov/Djex/tree/e454757d53ec057238c22fc6cd0d935b7d96b3dc}{\nolinkurl{github.com/VladimirReshetnikov/Djex/tree/e454757...}}
  \item Leant pinned tree: \href{https://github.com/VladimirReshetnikov/Leant/tree/ada69d884480b93e74ee4b3daf77be38702ee07d}{\nolinkurl{github.com/VladimirReshetnikov/Leant/tree/ada69d8...}}
