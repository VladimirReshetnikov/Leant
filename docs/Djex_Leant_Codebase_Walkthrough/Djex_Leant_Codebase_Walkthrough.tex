...
\begin{enumerate}
  ...
  After the occurrence seal, \filepath{src/Leant/Synth/Length/Presentation.hs} traverses each whole materialized ranked receipt and derives one opaque candidate-text-plus-note value. Main uses that one ordered list for Lean bindings, splices, and \code{itN} output; it never zips a detached evidence list back to candidates by spelling or index. Only \code{Counterexample} produces a subordinate note. The note calls it a replayed, model-relative finite-list-spine Length counterexample, renders a bounded summary of input and result spine lengths, and reduces a provider-backed basis to the number of assumed laws. It never projects the receipt's private provider-name list or makes a Lean, proof, correctness, or Z3-attestation claim.
...
\filepath{src/Leant/Synth/Length/Presentation.hs} & Opaque same-ranked-receipt candidate text and optional bounded, provider-name-free counterexample note; no detached evidence reassociation. \\
\filepath{src/Main.hs} & Explicit startup opt-in and post-callback integration; default identity, configured sealed ordering, same-receipt counterexample notes, and sanitized warning-on-fallback presentation. \\
\bottomrule
\end{longtable}
