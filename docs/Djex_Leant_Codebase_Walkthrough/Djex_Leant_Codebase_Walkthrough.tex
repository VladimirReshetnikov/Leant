...
\def\DjexBase{https://github.com/VladimirReshetnikov/Djex/blob/6493b29f3b19c6be65ee0e57c8ef30d996fe1e1c/}
\def\LeantBase{https://github.com/VladimirReshetnikov/Leant/blob/e5841361208ef13eefea01bde0d65af1e528beb3/}
...
\textbf{Djex snapshot} & \commitid{6493b29f3b19c6be65ee0e57c8ef30d996fe1e1c} \\
\textbf{Leant snapshot} & \commitid{e5841361208ef13eefea01bde0d65af1e528beb3} \\
...
package-private \code{Internal} sibling. On the Djex side,
...
\filepath{src/Leant/Synth/Length/Ranking/Internal.hs} & Package-private preparation orchestration, single-owner receipt-association/query state, scoped serial execution, a raw-four-vector or nominal-context replay cursor threaded through eager/deferred and unbudgeted/v1/scoped runners, the query-owned all-zero origin probe, \code{unsat}-triggered finite input-box validation, one complete guarded recursive piecewise-affine applicable-domain route, exact promotion/record timing, query-owned evidence replay, handle-preserving no-prune ordering, and candidate-report fallback without cache rollback. \\
\filepath{src/Leant/Synth/Length/CounterexampleBank/Internal.hs} & Package-private nominal scalar/product state and command-tagged contexts over Djex's query-owned replay bridge: validated limits, lazy one-scope ownership and reset, newest-first charged traversal, ordered bounded refusals, explicit evaluation-free promotion, fresh-replayed origin-aware recording, immutable snapshots, and serialized exception-restoring commit after full successor-state and classification-WHNF forcing. \\
...
\filepath{src/Leant/Synth/Length/SpinePair/Ranking.hs}, \filepath{Ranking/Internal.hs}, \filepath{PostVerification.hs}, \filepath{PostVerification/Internal.hs} & Nominal binary-product siblings of the scalar ranking and post-verification stack, including the separate raw/context cursor and full runner matrix, distinct end to end from their scalar counterparts. \\
\filepath{src/Leant/Synth/Length/Selection.hs}, \filepath{Selection/Internal.hs}, and SpinePair siblings & Nominal replay-authorized hard-selection adapters: unchanged raw compatibility entrances plus additive context-aware entrances; only independently replayed counterexamples reject; every other assessment retains; any adapter/ranking/index/seal failure preserves the exact original batch without rewinding completed bank transitions. \\
