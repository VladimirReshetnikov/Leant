...
The headline is intentionally modest: the repositories now have a checked insertion seam for one finite-list-spine Length model, a scoped Z3 execution boundary, an explicit startup policy opt-in, an optional one-command contract choice, and an explicit per-command choice between stable ranking and replay-authorized hard filtering in \filepath{src/Main.hs}. Without the startup opt-in the ordinary no-option and explicit-rank commands remain the exact non-strict identity path, while a filter request or requested command contract is refused before file IO. The Level-1 filter can refill survivors from the remainder of one already bounded synthesis lane, and Main now retains a private four-way outcome for that lane. Djex also exposes candidate-independent bounded replay-input banks and exact query-owned fresh replay; Leant wraps them in package-private pure state which has no runtime caller. Neither prerequisite is the proposal's persistent or cross-lane counterexample-guided loop. There is no claim here of general behavioral constraints, general program equivalence, a Lean-to-SMT semantics, solver-certified proofs, typed-sketch synthesis, or automatic Z3 integration.
...
A replayed counterexample need not come from a live solver response: the query-owned origin probe or a vector from the four-entry batch-local MRU can yield the same receipt only after independent replay against that occurrence. Each counterexample first crosses the query-owned simplification seam, and only its final vector enters the MRU. The next batch starts empty. Deferred lifecycle state, behavior mode, and replay state are not evidence and do not enter persistent REPL state.
\end{invariantbox}
...
Before any later live call inside one assessment scope, the internal ranking modules consult a bounded MRU input replay bank: at most four exact counterexample input vectors, kept newest-first with exact-vector deduplication. Each vector is independently validated and replayed against that candidate's own checked query; a hit yields an ordinary validated counterexample receipt, avoids one live query, and promotes the vector to the MRU end. The bank stores only input vectors---no cached verdicts, solver statuses, receipts, or proofs---and it never crosses an assessment batch.

The current vendored Djex tree separately exposes nominal candidate-independent scalar and binary-product bank scopes, bounded immutable input stores, and exact query-owned operations for fresh recording and retained-sample replay. Scope construction binds the checked session inventory and provider-law basis, solver-neutral interpretation policy, exact contract, and alpha-canonical target while excluding the candidate and execution identity. A scope match authorizes only an attempted fresh replay. \filepath{src/Leant/Synth/Length/CounterexampleBank/Internal.hs} wraps that bridge in nominal package-private scalar/product states. Each owns validated limits and zero or one active bank, initializes lazily, discards the sole bank on scope drift, traverses samples newest first while preserving charged successors and ordered refusals, and requires explicit evaluation-free hit promotion. It has no ranking, selection, integration, Main, command-state, or persistence call site; the active MRU in the preceding paragraph remains fresh for every assessment batch.
...
\filepath{src/Leant/Synth/Length/Integration.hs} & Opaque disabled/configured mode, startup admission--load--activation setup, retained activation disposition, byte-free executable-launch classification, pre-IO explicit-contract permission, and one command-local mode-plus-policy-plus-lazy-contract assessment request dispatching nominal ranking or selection; its behavior projector reads only the strict mode and does not force the retained contract. \\
...
\filepath{src/Leant/Synth/Length/Ranking/Internal.hs} & Package-private preparation orchestration, single-owner receipt-association/query state, scoped serial execution, the four-entry newest-first MRU counterexample-input replay bank, the query-owned all-zero origin probe, \code{unsat}-triggered finite input-box validation, one complete guarded recursive piecewise-affine applicable-domain route, query-owned evidence replay, handle-preserving no-prune ordering, and atomic fallback. \\
\filepath{src/Leant/Synth/Length/CounterexampleBank/Internal.hs} & Package-private nominal scalar/product state over Djex's query-owned replay bridge: validated limits, lazy one-scope ownership and reset, newest-first charged traversal, ordered bounded refusals, explicit evaluation-free promotion, and fresh-replayed origin-aware recording; no runtime call site. \\
...
\filepath{src/Leant/Synth/Length/Selection.hs}, \filepath{Selection/Internal.hs}, and SpinePair siblings & Nominal replay-authorized hard-selection adapters: only independently replayed counterexamples reject; every other assessment retains; any adapter/ranking/index/seal failure preserves the exact original batch. \\
...
Ranking keeps every verified occurrence and uses the established positive/neutral/counterexample order. It retains the historical five-success verification frontier. Filtering instead constructs one total scalar or nominal pair partition after verifying one already bounded lane: at most 12 groups for a standalone engine, 24 for \code{both}, or 6 for the excluded-middle route. It retains preparation refusals, unassessed candidates, every raw solver status, bounded-positive candidates, and applicable-domain establishments. Only an independently replayed counterexample becomes rejected. The generic partition seal restores both subsequences to original callback order and supplies association, not evidence. Any selection failure preserves the literal original batch and presents no rejection; Main still caps that preserve-all presentation at five. On success Main takes at most five survivor presentations for binding but prints every rejection from the bounded assessed batch with the exact established counterexample or simplification note.

Main represents that bounded work with private lazy lane outcomes. One field retains the complete bounded rendered-spelling frontier used for provider deduplication; a separate trace contains only detailed variants recorded immediately before actual Lean callbacks. Verification and the one Length assessment perform no presentation or splice effects. A pure classifier distinguishes no verified receipt, survivors, all behavioral rejections, and preserve-all assessment failure, and one finalizer owns observations, warnings, bindings, cache mutation, rows, and handled-lane notes. Only the empty-verified-receipt disposition continues; all other outcomes remain terminal. The four-entry input-vector MRU is still fresh per assessment batch, and every later hit is independently replayed against that later query. The new outcome is private rather than a public scheduler API. No bank or candidate crosses a lane or command, and no engine, verification module, or \code{ReplState} change implements the proposal's full Level-2 CEGIS loop.
...
\lpath{test-unit/Spec.hs} is a broad unit suite covering JSON, classifiers, snapshot metadata and publication, replay decisions, fragment parsing and analysis, provider queries and cache behavior, Djex translations, assignment fitting, rendering, candidate grouping, and many regressions around rank-N providers and recursive families. At the explicit lane-outcome characterization checkpoint the suite has 19{,}532 lines and 418 exact \code{testCase} tokens, 417 of which execute as suite cases; its single largest area is the Length/SpinePair behavioral-assessment layer: the current versionless scalar/product startup decoder, exact \code{:synth} behavior-mode and contract grammar, disabled-before-I/O authorization, guarded recursive piecewise-affine applicable-domain behavior, the checked handoff, ranking, replay-authorized selection, post-verification and total-partition sealing, association-safe presentation, scalar/product dispatch, all-rejected control flow, bounded same-lane survivor refill, and live execution against the \code{djex-fake-z3} fixture. The refill case rejects five early occurrences, retains seven later survivors in original order, and shows that the single assessment batch replays its first counterexample through the existing MRU bank.

The newest characterization separates the complete bounded spelling frontier from a deliberately shorter callback trace, pins every private lane-outcome field and the four-way pure disposition, rejects presentation or splice effects in the verification seam, and assigns metrics, warnings, reverse binding, cache mutation, rows, and handled notes to one finalizer. Scheduler checks prove that only \code{SynthLaneNoVerified} reaches another provider or classical lane and that provider deduplication never consumes the callback trace. Separate tests retain the lazy behavior projector, exact rank-five stop, finite poison and cyclic tails, 24-below-64 combined capacity, post-selection presentation cap, complete rejection projection, preserve-all cap, and command authorization before path IO. The guarded conditional case still decodes nominal scalar and product startup documents, establishes matching provider-independent \code{[[2]]} receipts with 1/3/3/3 counts, pins both presentation notes, and proves the missing executable and event path remain untouched.

The explicit Length-assessment integration group passes 18/18, verification
observability passes 9/9, and the complete Leant unit suite passes 417/417.
Strict warning-as-error builds of the test target and complete project are
green. The golden runner could not start in the validation environment because
no Lean REPL backend executable was installed; it changed no fixture and is
