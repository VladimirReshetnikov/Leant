...
\filepath{src/Leant/Synth/Length/Command.hs} & Exact fixed-order \code{:synth [-{}-behavior-mode rank|filter] [-{}-length-contract PATH] -{}- TYPE} parser, with ordinary no-option goals left delimiter-free and opaque. A separate passive entrance parses the inline \code{-{}-length-model}/\code{-{}-length-inputs}/\code{-{}-where} form into an opaque plan; \filepath{src/Main.hs} does not yet consume it, so a line spelling those options is currently ordinary goal text. \\
\filepath{src/Leant/Synth/Length/Where.hs} & Passive Leant profiles over Djex's bounded where-clause syntax: two fixed exact-case list models, ASCII input-index admission, and a resolver that builds the complete source-ordered role vector from the Lean-translated physical arity before elaboration; every stage is passive data and no step infers authority from the clause or the target. \\
