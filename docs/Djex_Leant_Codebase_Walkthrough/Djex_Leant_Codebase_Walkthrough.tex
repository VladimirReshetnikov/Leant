...
\def\LeantBase{https://github.com/VladimirReshetnikov/Leant/blob/0ab93f0a2380b7a91e6ce6388c7e5a2771aa64ed/}
...
\textbf{Leant snapshot} & \commitid{0ab93f0a2380b7a91e6ce6388c7e5a2771aa64ed} \\
...
The historical body of this walkthrough predates the typed behavioral-assessment layer. This addendum records the newer layer against the source snapshots named on the title page. Leant remains experimental: JSON shapes, names, tags, diagnostics, and output are current-tree descriptions rather than a public stability or backward-compatibility promise. The startup configuration now has one versionless schema whose explicit \code{rankingDomain} is \code{scalar} or \code{binary-product}; older numbered startup roots survive only in Git history and dated non-normative reports. The separate contract-only grammar remains versioned. Paths in this addendum are printed as current-tree file paths rather than through historical line-number references.
...
Applicable-domain validation is the second positive axis beside the finite input box: an independent, query-owned exhaustive traversal that establishes the precondition-applicable input domain, sealed in Djex under nominal direct-v1, literal-ceiling positive-affine, relational positive-affine, strict-relational positive-affine, strict-relational positive-affine root-quotient, cumulative root-extrema, cumulative root-monus, Boolean finite-union, atomic-branching, or recursive piecewise-affine receipt schemas for the scalar or binary-product domain. The current startup route selects the recursive receipt and its non-vacuous preference; lower-level builders retain the other strategies. Contract-only files never select ranking policy, and, like the input box, the external solver status itself never becomes the evidence.
...
With that policy activated, \clicode{:synth --length-contract ABSOLUTE-PATH -- TYPE} admits and reads a separate contract-only version 1 through version 6 document with format \code{leant-finite-list-spine-length-contract} once before goal translation. The mandatory standalone delimiter leaves \code{TYPE} opaque. Versions 1--5 are scalar: version~1 is the baseline scalar grammar; version~2 adds positive-literal Natural modulo; version~3 requires ordered target roles; version~4 requires the exact zero/step case policy; and version~5 requires an explicit case-policy choice and adds positive-literal Natural quotient. Version~6 is the separately typed canonical-\code{Prod} form with a required binary-product result shape. Contract-only files contain no execution, replay, ordering, simplification, or usable-work policy. A decoded command contract replaces only the process-fixed contract for that command and never enters \code{ReplState}, history, snapshots, or a cache.
...
Every accepted startup document constructs one fixed operational bundle: descriptor-bound execve-check launch; the four-entry MRU bank and all-zero origin probe; independent post-\code{unsat} input-box traversal; recursive piecewise-affine Boolean finite-union applicable-domain traversal; both non-vacuous preferences; componentwise-lexicographic simplification; deferred opening; and one owner-thread-affine, cooperatively checkpointed scoped usable-work deadline. Numeric limits remain caller supplied and independently bounded. Construction and activation are pure. An all-pure batch captures and checks the deadline but opens no descriptor; the first live miss opens one lexical session for the remaining suffix. Structured failures preserve the original-order atomic fallback.
...
The current applicable-domain strategy first runs the complete atomic predecessor for every signed relational leaf. Recursive interpretation is attempted only for an exact singleton ignored result that still contains minimum, maximum, or monus in a relation operand. It expands compact inputs, literals, normalized sums, retained positive scales, binary extrema, and binary monus into ordered guarded signed-affine values. Quotient, modulo, conditional, result reference, out-of-range variable, retained zero scale, or another unsupported descendant rejects the complete recursive fallback atom.
...
The scalar characterization of \code{max(x,y) <= 3 monus min(x,y)} with \code{x<=3,y<=3} retains \code{[[2,3],[3,2]]}, two boxes, 24 visits, 15 unique assignments, and 10 applicable assignments; the lower-level atomic predecessor retains \code{[[3,3]]} with 1/16/16/10. The product characterization builds \code{u=min(x,y)+(x monus y)} and \code{v=min(x,y)+(y monus x)} beneath \code{max(u,v)<=2}; the recursive path retains provider-independent \code{[[2,2]]} with 1/9/9/9, while the atomic predecessor retains \code{[[3,3]]} with 1/16/16/9. Its 32 raw alternatives distinguish generated-branch caps 31 and 32 before contradictory cases disappear.

Complete replay releases \nolinkurl{StrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionAtomicBranchingRecursivePiecewiseAffineApplicableDomainEstablished} or its nominal \code{LengthSpinePair} sibling. The matching renderers report model/provider-relative basis, boxes, raw visits, unique and applicable assignments, explicit vacuity, and a bounded maxima prefix. They grant no hull, solver proof, global theorem, or pruning authority. The current schema reset is recorded in \filepath{docs/reports/2026-08-15-versionless-length-ranking-configuration.md}; the semantic boundary is in \filepath{docs/reports/2026-08-15-recursive-piecewise-affine-length-ranking.md} and \filepath{lib/Djex/docs/reports/2026-08-15-recursive-piecewise-affine-length-applicable-domain.md}.
...
\begin{itemize}
  ...
  \item Leant pinned tree: \href{https://github.com/VladimirReshetnikov/Leant/tree/0ab93f0a2380b7a91e6ce6388c7e5a2771aa64ed}{\nolinkurl{github.com/VladimirReshetnikov/Leant/tree/0ab93f0...}}
