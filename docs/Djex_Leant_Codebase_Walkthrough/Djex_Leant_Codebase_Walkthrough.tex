...
\filepath{src/Leant/Synth/Verification.hs} & Lazy callback verification parameterized by a successful-group quota, with opaque nominal acceptance receipts; unchanged by lane-local refill. \\
...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, per-command rank/filter dispatch, caller-owned 12/24/6 group bounds, rank-five versus complete-bounded-filter verification quotas, post-selection five-survivor presentation, survivor-only \code{itN} binding, complete bounded rejection rows, handled all-rejected state clearing, four-way descriptor-launch notices, and sanitized mode-neutral preserve-all warning. \\
\bottomrule
\end{longtable}
...
Ranking keeps every verified occurrence and uses the established positive/neutral/counterexample order. It retains the historical five-success verification frontier. Filtering instead constructs one total scalar or nominal pair partition after verifying one already bounded lane: at most 12 groups for a standalone engine, 24 for \code{both}, or 6 for the excluded-middle route. It retains preparation refusals, unassessed candidates, every raw solver status, bounded-positive candidates, and applicable-domain establishments. Only an independently replayed counterexample becomes rejected. The generic partition seal restores both subsequences to original callback order and supplies association, not evidence. Any selection failure preserves the literal original batch and presents no rejection; Main still caps that preserve-all presentation at five. On success Main takes at most five survivor presentations for binding but prints every rejection from the bounded assessed batch with the exact established counterexample or simplification note. It treats an all-rejected batch as handled while clearing its previous synthesis-splice cache and does not continue into a provider or classical lane. The four-entry input-vector MRU is fresh per assessment batch; every later hit is independently replayed against that later query. No bank or candidate crosses a lane or command, and no engine, verification module, scheduler-result type, or \code{ReplState} change implements the proposal's full Level-2 CEGIS loop.
...
An accepted all-rejected behavioral partition is still handled output. Leant
prints the rejection rows, clears the earlier synthesis-splice cache, creates
no new binding, and returns without entering another provider or classical
lane. Preserve-all assessment failure keeps its complete internal batch but
presents at most five original candidates. These outcomes do not signal Lean
