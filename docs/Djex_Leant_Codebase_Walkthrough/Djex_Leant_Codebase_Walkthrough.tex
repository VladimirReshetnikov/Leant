...
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/SMTLib/Z3/Process.hs} & Opaque raw Z3 owner with the established pathname-snapshot launcher and the additive Linux sealed-main-image branch: bounded one-pass source hash/copy, optional pin admission, verified anonymous-file seals, descriptor-only execution, signal and process-group ownership, and fail-closed cleanup without pathname retry. \\
...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque reusable execution/replay policy with independent optional origin-probe, finite input-box, complete applicable-domain, and non-vacuous positive-ordering policies; total assembly from validated Djex authorities; sealed digest-expectation and executable-launch classifiers; legacy and descriptor-bound constructors; and policy-plus-request-contract ranking/assessment entrances for both the scalar and SpinePair domains. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for startup versions 1--18: literal v1--v16 dispatch plus scalar v17 and binary-product v18 descriptor-bound launch roots. The later pairs select literal-ceiling, runtime-unscoped usable-work, relational, scoped usable-work, strict-relational, or sealed-main-image launch policy without revising older schemas. Its disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, and post-callback integration; default identity, configured sealed ordering, descriptor-launch selection notice without a premature pin-match claim, same-receipt counterexample and bounded-positive notes, and sanitized warning-on-fallback presentation. \\
\bottomrule
\end{longtable}
...
The current startup configuration extends through version 18. Versions 7 and 8 introduce the advanced scalar/product root and literal-ceiling positive-affine validation; v9/v10 add the runtime-unscoped v1 shared usable-work object; v11/v12 instead select relational positive-affine validation without that object; v13/v14 combine the relational rule with the owner-thread-affine, cooperatively checkpointed scoped-v2 budget; and v15/v16 preserve the complete v13/v14 root while replacing only the applicable-domain strategy with \code{strict-relational-positive-affine-v1}. The strict rule accepts every ordinary relational clause plus exactly one immediate normalized top-level natural complement, \code{not (L <= R)}, as \code{R + 1 <= L}; it is neither general negation normalization nor a claim about source evaluation strictness. V17/v18 preserve the complete v15/v16 behavioral and scoped-budget profile while adding only the required descriptor-bound executable-launch selector: the optional digest is checked against the sealed staged main-image bytes installed through descriptor execution, with no pathname retry and no authority over loaders, libraries, or solver semantics. Each later decoder is reached only after the complete older chain returns its closed unsupported-version sentinel. Versions 1--6 retain their unchanged eager paths; v7--v18 retain deferred opening, so an all-pure batch opens none and the first live miss opens at most one fresh lexical scope. V13--v18 place preparation, pure ranking, live work, and Leant-owned result forcing beneath one absolute scoped deadline with cooperative phase checkpoints. Applicable-domain notes expose derived maxima, checked/applicable counts, basis, and explicit vacuity; strict simplification metadata stays attached to the exact occurrence; session strategy and deadline observations themselves add no evidence note.
