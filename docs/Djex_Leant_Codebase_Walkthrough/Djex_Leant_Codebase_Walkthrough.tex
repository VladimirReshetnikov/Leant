...
\begin{enumerate}
  ...
  After the occurrence seal, \filepath{src/Leant/Synth/Length/Presentation.hs} traverses each whole materialized ranked receipt and derives one opaque candidate-text-plus-note value. Main uses that one ordered list for Lean bindings, splices, and \code{itN} output; it never zips a detached evidence list back to candidates by spelling or index. Only \code{Counterexample} produces a subordinate note. The note calls it a replayed, model-relative finite-list-spine Length counterexample, renders a bounded summary of observed input and result spine lengths, and reduces a provider-backed basis to the number of assumed laws. It never projects the receipt's private provider-name list or makes a Lean, proof, correctness, or Z3-attestation claim.
...
\filepath{src/Leant/Synth/Length/Handoff.hs} & Verified exact-origin ordinal/text eligibility, exact family/provider resolution, and legacy, role-aware, or exact-case candidate-specific Djex Length problem sealing. \\
...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, and post-callback integration; default identity, configured sealed ordering, same-receipt counterexample notes, and sanitized warning-on-fallback presentation. \\
\bottomrule
\end{longtable}
...
With that policy activated, \clicode{:synth --length-contract ABSOLUTE-PATH -- TYPE} admits and reads a separate contract-only version 1, 2, 3, 4, or 5 document with format \code{leant-finite-list-spine-length-contract} once before goal translation. The mandatory standalone delimiter leaves \code{TYPE} opaque. Version 1 reuses the exact bounded compatibility grammar; version 2 adds positive-literal Natural modulo while retaining the same limits and root. Version 3 retains modulo and requires the ordered target-role vector described above; versions 1 and 2 reject that field and preserve legacy all-observed semantics. Version 4 retains roles and modulo and requires the exact zero/step candidate-case policy; versions 1--3 remain case-rejecting. Version 5 requires roles and an explicit choice between case rejection and that exact policy, and alone adds positive-literal Natural quotient; startup and versions 1--4 reject quotient. None contains execution or evaluation fields, and startup configuration remains fixed at version 1. The decoded contract is associated with the already activated policy in one opaque request and reused by every verification batch and retry lane for that command. It is never written to \code{ReplState}, \code{ParsedGoal}, history, snapshots, or a cache; the next command returns to the startup-fixed contract unless it names another file.
