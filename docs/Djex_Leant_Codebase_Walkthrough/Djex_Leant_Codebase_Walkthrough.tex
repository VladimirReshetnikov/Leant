...
\begin{enumerate}
  ...
  For a surviving positive-literal modulo node, Djex never emits SMT-LIB \code{mod}. It allocates deterministic private quotient/remainder witnesses, hoists only linear equations and Natural bounds into QF\_LIA, and keeps those witnesses out of \code{get-value} and evidence presentation; replay recomputes Natural modulo from original inputs.
...
\filepath{src/Leant/Synth/Length/Contract/File.hs} & Exact contract-only versioned root: unchanged v1, v2 positive-literal Natural modulo, v3 required ordered target roles, and v4 explicit exact zero/step cases, with shared limits and no execution-policy fields. \\
