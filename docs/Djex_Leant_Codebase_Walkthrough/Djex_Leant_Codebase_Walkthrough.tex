...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, per-command rank/filter dispatch, survivor-only \code{itN} binding, separate rejection rows, handled all-rejected state clearing, four-way descriptor-launch notices, and sanitized mode-neutral preserve-all warning. \\
\bottomrule
\end{longtable}
...
Ranking keeps every verified occurrence and uses the established positive/neutral/counterexample order. Filtering instead constructs one total scalar or nominal pair partition. It retains preparation refusals, unassessed candidates, every raw solver status, bounded-positive candidates, and applicable-domain establishments. Only an independently replayed counterexample becomes rejected. The generic partition seal restores both subsequences to original callback order and supplies association, not evidence. Any selection failure preserves the literal original batch and presents no rejection. Main binds only survivors, prints each rejection with the exact established counterexample or simplification note, and treats an all-rejected batch as handled while clearing its previous synthesis-splice cache. The four-entry input-vector MRU is fresh per assessment batch; every later hit is independently replayed against that later query and no bank crosses a command.
