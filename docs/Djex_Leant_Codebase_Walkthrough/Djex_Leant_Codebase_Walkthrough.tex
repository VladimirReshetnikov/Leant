...
Applicable-domain validation is the second positive axis beside the finite input box: an independent, query-owned exhaustive traversal that establishes the precondition-applicable input domain, sealed in Djex under the schema tag \code{finite-binary-product-spine-lengths/}\allowbreak\code{finite-precondition-domain-establishment/v1} (with the corresponding \code{finite-list-spine-length} tag on the scalar side). Its receipts feed the API-only non-vacuous applicable-domain preference described above; like the input box, the external solver status itself never becomes the evidence.
...
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded JSON decoder for startup versions 1--6 whose disabled owner retains one strict validated policy beside one lazy fixed contract, with no copied source-derived pin flag; activation checks only the policy and releases that pair. \\
