...
\begin{enumerate}
  \item \textbf{Exference retains a typed occurrence.} The typed Exference runners return an opaque \code{TypedCandidate}: the historical compatibility candidate remains paired with either its exact bounded \code{TermGraph} or an explicit engine-owned absence reason. When present, the graph's erasure was checked against that compatibility candidate. \filepath{synthesis/src/Language/Haskell/Synthesis/TypedGenerated.hs} seals graph structure and typing relationships, while \filepath{synthesis/src/Language/Haskell/Synthesis/TypedCandidate.hs} prevents a graph from being detached and reassociated with another compatibility candidate. Both graph parameters---the neutral type and local-identity domains---are nominal. The sealed data constructor is private and has no \code{Generic} reconstruction path; public callers can submit a raw \code{TermGraphSource} only through the bounded validating \code{sealTermGraph} entrance.

  Leant's \filepath{src/Leant/Synth/Engine.hs} retains that candidate in a \code{TypedCandidateSemanticSidecar} together with the exact Exference inventory/session, prepared translation origin, source-name table, converted source goal, session policy, request, provider assignments, and a lazy structural graph fingerprint attempt. Candidate grouping and renderer filtering preserve the sidecar only while the group still denotes the same engine candidate. A wrapper such as the classical prove-mode transformation explicitly drops it.
  ...
  \item \textbf{The Length handoff rechecks correspondence.} \code{prepareCheckedLengthHandoff} in \filepath{src/Leant/Synth/Engine.hs} accepts the exact \code{Verified DetailedVerificationVariant}, not detached text. It fails closed unless the route is the direct typed-candidate route; the semantic sidecar is present; engine and fitting fragments agree; no premise or request context retargets the problem; the search, request, and converted source goals still agree; the retained graph has not been lost; and that graph has exactly one direct Lean rendering at original ordinal zero equal to the callback-accepted text.
...
The only candidate batch eligible for behavioral reordering is the exact callback-accepted batch. The only current reason to move an occurrence is an independently replayed \code{finite-list-spine-length/v1} counterexample for that exact retained Exference graph and handoff. No solver status removes a candidate, and a complete bounded permutation is sealed before presentation.
\end{invariantbox}

\section*{Djinn uses the envelope without inventing graph authority}

Djinn now exposes \code{DjinnTypedCandidate} and \code{DjinnTypedResult}, with typed counterparts of its ordinary, provider-candidate, assignment, and kinded-assignment runners in \filepath{djinn/src-core/Language/Haskell/Djex/Djinn.hs} and \filepath{djinn/src-core/Djinn/Core.hs}. The legacy runners are one-way compatibility projections of those typed paths. Final candidate keys are allocated only after proof checking, deduplication, and the configured final ordering step, so future typed occurrence identities need not depend on discarded proof-plan ordinals.
...
\filepath{synthesis/src/Language/Haskell/Synthesis/TypedCandidate.hs} & Opaque association of compatibility candidate with graph availability or an explicit engine-specific absence. \\
...
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/Problem.hs} & Atomic checked session, provider inventory, candidate semantics, and complete Length problem. \\
...
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/SMTLib.hs} & Canonical bounded QF\_LIA query, query fingerprint, and model-binding validation. \\
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/SMTLib/Execution.hs} & Pure explicit execution policy and response/resource admission. \\
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/SMTLib/Live.hs} & Sanitized rank-2 live-session facade and replayed-evidence observation. \\
...
\midrule
\endhead
\filepath{src/Leant/Synth/Engine.hs} & Exference run authority, typed semantic sidecar, renderer/verification provenance, and checked Length handoff. \\
...
\filepath{src/Leant/Synth/Length/Adapter.hs} & Pure conversion from one checked handoff to one canonical Length query. \\
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque explicit execution/replay policy; contract remains request-owned. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded version-1 JSON decoder returning a disabled configuration value. \\
...
\filepath{src/Leant/Synth/Length/Ranking.hs} & Complete preparation, one scoped serial pass, evidence replay, no-prune stable ordering, sanitized refusals, and atomic fallback. \\
...
\filepath{src/Leant/Synth/Length/PostVerification.hs} & Length-specific association of assessments with occurrence handles through final sealing. \\
