...
\filepath{src/Leant/Synth/Length/Contract.hs} & Solver-neutral caller-supplied spine, condition, provider-law, and optional exact target-role vocabulary. \\
\filepath{src/Leant/Synth/Length/Handoff.hs} & Verified exact-origin singleton/ordinal/text eligibility, exact family/provider resolution, and legacy or role-aware candidate-specific Djex Length problem sealing. \\
...
\filepath{src/Leant/Synth/Length/Contract/File.hs} & Exact contract-only versioned root: unchanged v1, v2 positive-literal Natural modulo, and v3 required ordered target roles, with shared limits and no execution-policy fields. \\
