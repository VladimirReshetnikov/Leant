...
The headline is intentionally modest: the repositories now have a checked insertion seam for one finite-list-spine Length model, a scoped Z3 execution boundary, and an explicit command-line opt-in in \filepath{src/Main.hs}. Without that opt-in the ordinary command and REPL behavior remains the exact non-strict identity path. There is no claim here of general behavioral constraints, general program equivalence, a Lean-to-SMT semantics, solver-certified proofs, or automatic Z3 integration.
...
\begin{enumerate}
  ...
  Leant's \filepath{src/Leant/Synth/Engine.hs} first builds one prepared semantic-origin record. Search, renderer inputs, retained Exference authority, and later exact-origin rerendering project that same record; no post-search field-for-field preparation copy exists. Each provider binding is the sole prepared owner of its ordered instantiation assignments; Djinn and Exference derive the aggregate list in binding order, while inspection projects the same compatibility view. The prepared semantic origin does not cache that aggregate beside the bindings. The engine retains the typed candidate in a \code{TypedCandidateSemanticSidecar} together with the exact Exference inventory/session, prepared origin, source-name table, converted source goal, session policy, request, and converted Exference assignments. It keeps no parallel graph-key cache: the exact graph remains owned by the opaque checked candidate, and Djex fingerprints it only while sealing an actual behavioral problem. Candidate grouping and renderer filtering preserve a group sidecar only while the group still denotes the same engine candidate. During combined-engine exact-text deduplication, an earlier compatibility-only spelling may instead retain a private variant-scoped witness to the first bounded later Exference occurrence with identical bytes. That witness keeps the Exference sidecar and source renderer ordinal separate from the display group's route, display ordinal, ordering, sidecar, and sibling variants. At the verification boundary, the display route, ordinal, and text are flattened into the callback candidate, which retains that spelling's exact typed origin once rather than keeping the scheduling-only variant and a parallel origin field. A wrapper such as the classical prove-mode transformation explicitly drops both direct and recovered authority.
  ...
  \item \textbf{The Length handoff rechecks correspondence.} \code{prepareCheckedLengthProblem} in \filepath{src/Leant/Synth/Length/Handoff.hs} accepts the exact \code{Verified DetailedVerificationVariant}, not detached text. It fails closed unless that exact spelling has a direct or recovered typed Exference origin; engine and fitting fragments agree; no premise or request context retargets the problem; the search, request, and converted source goals still agree; the retained graph has not been lost; the origin's original renderer ordinal is zero; and that graph has exactly one direct Lean rendering equal to the callback-accepted text. A recovered origin therefore authorizes assessment of the identical accepted spelling, not the Djinn group or another textual variant.

  Only after those checks does the handoff resolve the declared unary spine and named providers through retained translation provenance. Djex seals the exact inventory, spine model, and provider summaries once into an opaque session. Candidate sealing consumes that session-owned provider inventory directly, revalidates the separately supplied contract through the session context, and then seals the typed candidate and complete behavioral-problem identity. The exact checked provider summaries reached by symbolic interpretation flow directly into the canonical used-law identity; an independent ascending names-only list is retained only for the candidate receipt. Raw text, candidate equality, private-name spelling conventions, and a graph recovered from a rendered term are not association mechanisms.
  ...
  \item \textbf{Status and evidence stay separate.} \code{sat}, \code{unsat}, and \code{unknown} are all retained only as \code{HeuristicRankingOnly} solver observations. The generic association retains the raw observation once, and status plus conservative strength are derived from its closed family and status constructor without inspecting artifact payloads or copying either fact into parallel fields. In particular, \code{unsat} is relative to the bounded encoding and is neither a solver certificate nor pruning authority; status-only \code{sat} has no model evidence. Under the input-values policy, a satisfiable response can yield evidence only after exact symbol decoding and an independent concrete natural-number replay against the checked Length problem. Djex's \code{replayLengthSMTLibLiveQueryObservation} checks the returned query fingerprint before inspecting optional evidence, then replays that opaque evidence once more against the exact behavioral problem retained by the sealed query. Leant accepts a \code{ValidatedLengthCounterexample} only through that gate; the heavier handoff is transient after query sealing.
  ...
  \item \textbf{Assessment may reorder but never prune.} The package-private implementation in \filepath{src/Leant/Synth/Length/Ranking/Internal.hs} first productively admits at most 64 candidates and completes all pure candidate/query preparation before opening a worker. In the successful order, \code{Unassessed} candidates and candidates carrying only heuristic statuses retain their relative order; candidates with independently replayed counterexamples follow, also stably. Every original callback receipt remains present. Any returned session, query, fingerprint-association, or evidence-replay failure discards partial live assessments and restores the original order with all assessments \code{Unassessed}.
...
\filepath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/SMTLib/Live.hs} & Sanitized rank-2 live-session facade, replayed-evidence observation, and exact query-first evidence-consumption gate. \\
...
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/Semantic/Length/SMTLib/Protocol.hs} & Pure positional Length query protocol whose sealed plan retains exact artifact/response policy, query, cumulative cursor policy, barriers, and complete key rather than the full structured execution configuration. \\
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/Semantic/Length/SMTLib/Session/Process.hs} & Length compatibility and identity facade over the shared runtime, with exhaustive sanitized mapping and unchanged raw-process v2/limits-v1 sealing. \\
...
\filepath{synthesis/internal/Language/Haskell/Synthesis/Internal/Semantic/Length/SMTLib/Session.hs} & V4 ready-worker identity with one direct complete Length-policy occurrence, a strict post-probe query-only policy, process-owned limit projection, plan-owned replay inputs, completed-run status/evidence/key/accounting without a decoded-binding cache, scoped serial ordinals, poisoning, and cleanup. \\
...
\midrule
\endhead
\filepath{src/Leant/Synth/Engine.hs} & One prepared semantic origin shared by search and rendering, provider-binding-owned assignment order, Exference run authority, typed semantic sidecars, opaque verification provenance, and exact-origin rerender mechanics. \\
\filepath{src/Leant/Synth/Verification.hs} & Lazy bounded callback verification and opaque nominal acceptance receipts. \\
\filepath{src/Leant/Synth/Length/Contract.hs} & Solver-neutral caller-supplied spine, condition, and provider-law vocabulary. \\
\filepath{src/Leant/Synth/Length/Handoff.hs} & Verified exact-origin singleton/ordinal/text eligibility, exact family/provider resolution, and candidate-specific Djex Length problem sealing. \\
...
\filepath{src/Leant/Synth/Length/Configuration.hs} & Opaque explicit execution/replay policy, total assembly from validated Djex authorities, and sealed digest-expectation presence bridge; the reusable-policy entrance keeps the contract request-owned, while the compatibility entrance bundles one contract. \\
\filepath{src/Leant/Synth/Length/Configuration/File.hs} & Closed, bounded version-1 JSON decoder which retains its first sealed execution/evaluation results and returns a disabled configuration with no copied source-derived pin flag. \\
\filepath{src/Leant/Synth/Length/Configuration/File/Acquire.hs} & Explicit-path, bounded, sanitized POSIX descriptor acquisition; no discovery or activation. \\
\filepath{src/Leant/Synth/Length/Integration.hs} & Opaque disabled/configured mode, admission--load--activation setup, retained closed activation disposition for startup presentation, fixed-contract configured assessment, sanitized fallback, and exact disabled identity. \\
\filepath{src/Leant/Options.hs} & Pure CLI admission, pin-policy choice, and bounded configuration-load timeout ownership. \\
\filepath{src/Leant/Json/Bounded.hs} & Shared strict bounded JSON syntax, scalar decoding, and closed-object validation used by the configuration boundary. \\
...
\filepath{src/Leant/Synth/Length/PostVerification.hs} & Safe Length-specific facade exposing only configured or policy-plus-contract sealed assessment. \\
