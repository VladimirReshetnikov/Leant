...
\def\LeantBase{https://github.com/VladimirReshetnikov/Leant/blob/a9b917f5689ed71a8bf1d3aed038af8e34602a9e/}
...
\textbf{Leant snapshot} & \commitid{a9b917f5689ed71a8bf1d3aed038af8e34602a9e} \\
...
The headline is intentionally modest: the repositories now have a checked insertion seam for one finite-list-spine Length model, a scoped Z3 execution boundary, an explicit startup policy opt-in, an optional one-command contract choice, and an explicit per-command choice between stable ranking and replay-authorized hard filtering in \filepath{src/Main.hs}. Without the startup opt-in the ordinary no-option and explicit-rank commands remain the exact non-strict identity path, while a filter request or requested command contract is refused before file IO. Filtering can refill survivors inside each already bounded lane. Main now also introduces one nominal rank-2 context before translation, reuses its filter bank across every same-command lane, and continues after either no verification or complete behavioral rejection. Djex supplies the candidate-independent bounded replay-input bank and exact query-owned fresh replay; established rank and direct Selection compatibility paths retain their raw four-vector MRU. The scheduler keeps one lazy reverse outcome accumulation and finalizes it once. The Engine module separately exports an opaque 60-group cursor over one lazy detailed outcome, but Main does not import or use it. This is bounded command-local reuse of the existing provider/classical lane plan plus an unused observation foundation, not persistent state, cross-lane five-survivor quota filling, progressive runtime batching, counterexample-directed engine search, or the proposal's complete CEGIS loop. There is no claim here of general behavioral constraints, general program equivalence, a Lean-to-SMT semantics, solver-certified proofs, typed-sketch synthesis, or automatic Z3 integration.
...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, one pre-translation rank-2 assessment context, per-command rank/filter dispatch, caller-owned 12/24/6 group bounds, rank-five versus complete-bounded-filter verification quotas, distinct complete spelling-frontier and exact callback-attempt histories, private four-way lane classification and lazy reverse accumulation, no-verified/all-rejected continuation with full-frontier deduplication, first-survivor termination without quota filling, one plural result finalizer, survivor-only \code{itN} binding, complete accumulated rejection rows, structural/classical and abnormal diagnostic gates, four-way descriptor-launch notices, and sanitized mode-neutral preserve-all warning. \\
\bottomrule
\end{longtable}
...
Ranking keeps every verified occurrence and uses the established positive/neutral/counterexample order. It retains the historical five-success verification frontier and raw MRU. Filtering instead constructs one total scalar or nominal pair partition after verifying each already bounded lane: at most 12 groups for a standalone engine, 24 for \code{both}, or 6 for the excluded-middle route. It retains preparation refusals, unassessed candidates, every raw solver status, bounded-positive candidates, and applicable-domain establishments. Only an independently replayed counterexample becomes rejected. The generic partition seal restores both subsequences to original callback order and supplies association, not evidence. Any selection failure preserves the literal original batch and presents no rejection; Main still caps that terminal preserve-all presentation at five, while completed nominal-bank transitions remain committed. Main takes at most five survivor presentations from the first terminal lane but prints every rejection accumulated from earlier all-rejected lanes plus that terminal lane.

Main introduces one reusable rank-2 context before initial goal translation and carries it through universe retry and every baseline, provider, excluded-middle, and double-negation lane. Rank contexts remain bank-free; filter contexts reuse one nominal bank, and every retained sample still requires independent replay against the later query. Main represents each bounded result with a private lazy lane outcome: one field retains the complete bounded rendered-spelling frontier used for provider deduplication, while a separate trace contains only detailed variants recorded immediately before actual Lean callbacks. Verification and assessment perform no presentation or splice effects. A pure classifier distinguishes no verified receipt, survivors, all behavioral rejections, and preserve-all assessment failure. Main prepends actual outcomes to a lazy reverse accumulation; no-verified and all-rejected continue and both union the complete frontier, while survivor and preserve-all stop. The first survivor is terminal even below five. One plural finalizer owns chronological metrics, effective-prefix warnings, one final binding/cache phase, terminal candidate rows, accumulated rejections, and handled notes. No context or bank crosses the command, and no engine, verification module, or \code{ReplState} change implements quota filling, counterexample-directed engine search, persistence, or the proposal's complete Level-2 CEGIS loop.
...
The module sets bounded constants for backend candidate windows, combined verification frontiers, attempted groups, and shown successes. At this snapshot, ranking stops verification after five accepted groups. Filtering first takes each caller's finite lane prefix, may verify all 12 standalone, 24 combined, or 6 excluded-middle groups, assesses that frontier once through the command context, and only after a terminal outcome takes five survivor presentations. The larger internal window lets Lean failures and early behavioral rejections yield to later same-lane semantic candidates without widening visible output. A complete all-rejected lane may enter the next already scheduled lane, but this is not a cross-lane survivor quota: the first lane with one survivor is terminal. The consumer-side prefix remains separate from the successful-group quota because a failed group does not consume that quota; this is what keeps a lazy or cyclic tail outside the lane. The constants are named and centralized; code should not introduce ad hoc \code{take 5} operations that lose progress accounting.
...
deciding to present those notes once at lane exit belongs to a future
scheduler, not to this cursor.
...
The executable does not yet consume this surface. \filepath{src/Main.hs}
still imports \nolinkurl{forceDetailedOutcome} rather than any new cursor
symbol, forces one 12-, 24-, or excluded-middle 6-group lane, verifies and
assesses that whole finite prefix once, and stops on the first survivor or
preserve-all result. The cursor has no
assessment context, counterexample bank, provider-deduplication policy,
survivor quota, persistent state, or counterexample-directed engine input.
Progressive same-run verification and assessment therefore remain a separate
...
\begin{enumerate}
  ...
  \item verify and assess the bounded lane; stop on survivor or preserve-all, but retain and continue after no-verification or all-rejection;
...
The newer cursor does not relax that rule. Its step-specific force target lets a
future caller put each selected batch under the same command deadline while
...
When enabled after a retained sound refutation and the Glivenko split admits the fragment, Leant first offers bounded excluded-middle premises, then applies a double-negation transformation when excluded middle has no verified candidate or is entirely behaviorally rejected. Both routes use the command's assessment context. A survivor or preserve-all result stops; every actual lane outcome is accumulated, but neither route contributes negative evidence because:
...
Main takes the exact caller-owned attempted-group prefix before choosing a
successful-group quota. Ranking stops after five acceptances. Filtering may
traverse each complete finite lane prefix---12 groups for a standalone engine,
24 for the combined engine, or 6 for the excluded-middle classical route---and
passes all callback acceptances to one behavioral assessment under the
command's context. Failures can be retained for diagnostics but are not shown
as candidates. After the first terminal filter outcome, at most five survivors
are displayed and bound, while every rejection accumulated from earlier
all-rejected lanes and the terminal lane is reported. This is not cross-lane
five-survivor quota filling: one survivor stops the scheduler.

Each lane outcome retains two histories with different authority. The checked
frontier contains every rendered spelling in the complete bounded group
prefix, even when callback laziness skips it, and remains the provider-stage
deduplication input. The exact callback trace records a detailed variant
...
narrow trace for the complete frontier. Both no-verification and all-rejection
union the complete frontier before another provider stage.

Verification and optional Length assessment return a private lazy
...
every actual outcome to a lazy \code{SynthLaneAccumulation} in reverse attempt
order. No-verification and all-rejection continue; survivor and preserve-all
are terminal.
...
rejection rows; notes are prior-all-rejected then terminal, while
no-verification notes stay withheld. Final all-rejection clears once; terminal
...
\begin{tikzpicture}[node distance=7mm and 7mm]
  ...
  \node[goldbox,below=of accepted,text width=34mm] (assess) {Optional Length\\one-batch assessment};
...
\end{tikzpicture}}
\caption{The candidate funnel. The exact Lean target remains the acceptance gate; optional behavioral filtering partitions one bounded lane before the five-survivor presentation cap.}
...
effect, retains its rejection rows, and may enter another provider or classical
lane. Only final aggregate all-rejection prints every accumulated rejection and
clears the earlier synthesis-splice cache. Preserve-all assessment failure has
...
\lpath{test-unit/Spec.hs} is a broad unit suite covering JSON, classifiers, snapshot metadata and publication, replay decisions, fragment parsing and analysis, provider queries and cache behavior, Djex translations, assignment fitting, rendering, candidate grouping, and many regressions around rank-N providers and recursive families. At the bounded detailed-cursor characterization checkpoint the suite has 23{,}248 lines and 452 exact \code{testCase} tokens, 451 of which register and execute as suite cases. The new ten-case cursor group covers ordered nonempty batching, unchanged notes, terminal taxonomy, request admission before cursor demand, start and outer-result laziness, natural exhaustion versus the exact 60-group cap, poison and cyclic tails, legacy force parity, selected-payload forcing, opaque representation, and absence from Main. The suite's single largest area remains the Length/SpinePair behavioral-assessment layer: the current versionless scalar/product startup decoder, exact \code{:synth} behavior-mode and contract grammar, disabled-before-I/O authorization, guarded recursive piecewise-affine applicable-domain behavior, the checked handoff, ranking, replay-authorized selection, post-verification and total-partition sealing, association-safe presentation, scalar/product dispatch, bounded same-lane refill, command-local nominal bank contexts, cross-lane all-rejected continuation, and live execution against the \code{djex-fake-z3} fixture. Rank and direct Selection compatibility tests retain the raw MRU.

The lane-outcome characterization still separates the complete bounded spelling frontier from a deliberately shorter callback trace, pins every private lane-outcome field and four-way pure disposition, and rejects presentation or splice effects in the verification seam. Six Main characterizations now pin one pre-translation command context, the lane/accumulation boundary, chronological first-terminal folding, exactly-once plural finalization, provider and EM-to-NN continuation after both nonterminal dispositions, and the normal/structural/abnormal diagnostic gates. They also freeze a lazy positional accumulation without \code{Eq}, \code{Show}, or a record selector; full-frontier deduplication; finite bounds; and no quota fill or \code{ReplState} ownership. The package-private bank runner group retains its seven scalar/product route, cap, sealing, failure, Integration-lifetime, and architecture cases.

The focused bounded cursor surface passes 10/10, the strict bank runner surface
passes 7/7, and explicit Length assessment integration passes 21/21. The
complete strict Leant unit suite passes 451/451. Strict warning-as-error builds
...
golden transcripts pass byte-exact for this cursor checkpoint with exit status
zero, no update mode, and no fixture drift. Main, the 26 inputs, the 26 golden
files, and the runner are unchanged.
...
Leant combined one-lane group prefix & 24 & Bounds cross-engine semantic groups; filter mode may verify this complete prefix once. \\
Leant standalone one-lane group prefix & 12 & Bounds Djinn- or Exference-only groups; filter mode may verify this complete prefix once. \\
Leant excluded-middle group prefix & 6 & Keeps the first classical route cheap before its double-negation successor. \\
...
\begin{itemize}
  ...
  \item Leant pinned tree: \href{https://github.com/VladimirReshetnikov/Leant/tree/a9b917f5689ed71a8bf1d3aed038af8e34602a9e}{\nolinkurl{github.com/VladimirReshetnikov/Leant/tree/a9b917f...}}
