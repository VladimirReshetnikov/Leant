...
\def\LeantBase{https://github.com/VladimirReshetnikov/Leant/blob/0611e2c9685590508bf76cda165147c29b69d1fb/}
...
\textbf{Leant snapshot} & \commitid{0611e2c9685590508bf76cda165147c29b69d1fb} \\
...
The headline is intentionally modest: the repositories now have a checked insertion seam for one finite-list-spine Length model, a scoped Z3 execution boundary, an explicit startup policy opt-in, an optional one-command contract choice, and an explicit per-command choice between stable ranking and replay-authorized hard filtering in \filepath{src/Main.hs}. Without the startup opt-in the ordinary no-option and explicit-rank commands remain the exact non-strict identity path, while a filter request or requested command contract is refused before file IO. The Level-1 filter can refill survivors from the remainder of one already bounded synthesis lane, and Main retains a private four-way outcome for that lane. Djex also exposes candidate-independent bounded replay-input banks and exact query-owned fresh replay. Leant's additive context-aware filter path now threads one nominal package-private bank through that assessment; established rank and direct Selection compatibility paths retain their raw four-vector MRU. Integration can introduce a reusable rank-2 filter context, but its compatibility wrapper creates a fresh context per call and Main does not carry it between lanes. This is not the proposal's persistent or cross-lane counterexample-guided loop. There is no claim here of general behavioral constraints, general program equivalence, a Lean-to-SMT semantics, solver-certified proofs, typed-sketch synthesis, or automatic Z3 integration.
...
\filepath{src/Main.hs} & Explicit startup opt-in, load-once command contract selection, per-command rank/filter dispatch, caller-owned 12/24/6 group bounds, rank-five versus complete-bounded-filter verification quotas, distinct complete spelling-frontier and exact callback-attempt histories, private four-way lane classification, one presentation/splice finalizer, post-selection five-survivor presentation, survivor-only \code{itN} binding, complete bounded rejection rows, handled all-rejected state clearing, four-way descriptor-launch notices, and sanitized mode-neutral preserve-all warning. \\
\bottomrule
\end{longtable}
...
Ranking keeps every verified occurrence and uses the established positive/neutral/counterexample order. It retains the historical five-success verification frontier and raw MRU. Filtering instead constructs one total scalar or nominal pair partition after verifying one already bounded lane: at most 12 groups for a standalone engine, 24 for \code{both}, or 6 for the excluded-middle route. It retains preparation refusals, unassessed candidates, every raw solver status, bounded-positive candidates, and applicable-domain establishments. Only an independently replayed counterexample becomes rejected. The generic partition seal restores both subsequences to original callback order and supplies association, not evidence. Any selection failure preserves the literal original batch and presents no rejection; Main still caps that preserve-all presentation at five, while completed nominal-bank transitions remain committed. On success Main takes at most five survivor presentations for binding but prints every rejection from the bounded assessed batch with the exact established counterexample or simplification note.

Main represents that bounded work with private lazy lane outcomes. One field retains the complete bounded rendered-spelling frontier used for provider deduplication; a separate trace contains only detailed variants recorded immediately before actual Lean callbacks. Verification and the one Length assessment perform no presentation or splice effects. A pure classifier distinguishes no verified receipt, survivors, all behavioral rejections, and preserve-all assessment failure, and one finalizer owns observations, warnings, bindings, cache mutation, rows, and handled-lane notes. Only the empty-verified-receipt disposition continues; all other outcomes remain terminal. Integration can introduce one reusable rank-2 context, but Main still calls its compatibility assessor once per lane; rank calls use the raw MRU and filter calls create a fresh nominal context. Every later sample is independently replayed against that later query. The outcome remains private rather than a scheduler API. No context, bank, or candidate crosses a Main lane or command, and no engine, verification module, or \code{ReplState} change implements the proposal's full Level-2 CEGIS loop.
...
Complete replay releases \code{ApplicableDomainEstablished} or its nominal \nolinkurl{LengthSpinePairApplicableDomainEstablished} sibling, carrying \code{ValidatedLengthApplicableDomain} or \nolinkurl{ValidatedLengthSpinePairApplicableDomain}. \nolinkurl{renderLengthApplicableDomainValidationNote} and \nolinkurl{renderLengthSpinePairApplicableDomainValidationNote} report model/provider-relative basis, boxes, raw visits, unique and applicable assignments, explicit vacuity, and a bounded maxima prefix. They grant no hull, solver proof, global theorem, or pruning authority. The current file-schema resets are recorded in \filepath{docs/reports/2026-08-15-versionless-length-ranking-configuration.md} and \filepath{docs/reports/2026-08-15-versionless-length-contract.md}; the current guarded behavior is in \filepath{docs/reports/2026-08-15-guarded-conditional-length-ranking.md}, the connected selection path is recorded in \filepath{docs/reports/2026-08-15-command-authorized-length-filtering.md}, its lane-local refill is recorded in \filepath{docs/reports/2026-08-15-lane-local-length-survivor-refill.md}, the private outcome seam is recorded in \filepath{docs/reports/2026-08-16-explicit-synthesis-lane-outcomes.md}, the historical pure bank-state foundation is recorded in \filepath{docs/reports/2026-08-16-nominal-length-counterexample-bank-state.md}, and its filter-only context-runner successor is recorded in \filepath{docs/reports/2026-08-16-filter-only-length-counterexample-bank-context-runner.md}, while the earlier recursive and structural-seal reports remain non-normative implementation history at their landing checkpoints. Djex's exact guarded semantics and private identity boundary are in \filepath{lib/Djex/docs/reports/2026-08-15-guarded-conditional-length-applicable-domain.md}; its candidate-independent store and fresh-replay bridge are in \filepath{lib/Djex/docs/reports/2026-08-16-nominal-length-counterexample-bank-foundation.md} and \filepath{lib/Djex/docs/reports/2026-08-16-length-counterexample-bank-query-replay-bridge.md}.
...
traverse the complete finite lane prefix---12 groups for a standalone engine,
24 for the combined engine, or 6 for the excluded-middle classical route---and
passes all callback acceptances to one behavioral assessment. Failures can be
retained for verbose diagnostics but are not shown as candidates. After
filtering, at most five survivors are displayed and bound, while every bounded
behavioral rejection is reported.

The lane outcome retains two histories with different authority. The checked
...
narrow trace for the complete frontier.
...
survivors, all behaviorally rejected, or assessment preserved. The first is
guarded directly by receipt-batch emptiness and is the only continuation
authority. One finalizer owns metrics, preserve-all warnings, reverse binding,
splice-cache replacement or clearing, survivor and complete rejection rows,
and handled-lane notes. A final no-verification report emits its error before
the stored engine notes; intermediate no-verification lanes withhold those
weaker notes.
...
An accepted all-rejected behavioral partition is the explicit terminal
all-behaviorally-rejected disposition. Leant prints the rejection rows, clears
the earlier synthesis-splice cache, creates no new binding, and returns without
entering another provider or classical lane. Preserve-all assessment failure
has its own terminal disposition, keeps its complete internal batch, and
presents at most five original candidates. Neither outcome signals Lean
verification failure or command-local CEGIS continuation.
...
\lpath{test-unit/Spec.hs} is a broad unit suite covering JSON, classifiers, snapshot metadata and publication, replay decisions, fragment parsing and analysis, provider queries and cache behavior, Djex translations, assignment fitting, rendering, candidate grouping, and many regressions around rank-N providers and recursive families. At the filter-only bank-runner characterization checkpoint the suite has 22{,}240 lines and 439 exact \code{testCase} tokens, 438 of which register and execute as suite cases; its single largest area is the Length/SpinePair behavioral-assessment layer: the current versionless scalar/product startup decoder, exact \code{:synth} behavior-mode and contract grammar, disabled-before-I/O authorization, guarded recursive piecewise-affine applicable-domain behavior, the checked handoff, ranking, replay-authorized selection, post-verification and total-partition sealing, association-safe presentation, scalar/product dispatch, all-rejected control flow, bounded same-lane survivor refill, nominal bank contexts, and live execution against the \code{djex-fake-z3} fixture. The refill case rejects five early occurrences, retains seven later survivors in original order, and shows that one context freshly created by Integration's compatibility assessor for a Main lane call replays its first counterexample against later same-lane candidates; rank and direct Selection compatibility tests retain the raw MRU.

The lane-outcome characterization still separates the complete bounded spelling frontier from a deliberately shorter callback trace, pins every private lane-outcome field and the four-way pure disposition, rejects presentation or splice effects in the verification seam, and assigns metrics, warnings, reverse binding, cache mutation, rows, and handled notes to one finalizer. Scheduler checks prove that only \code{SynthLaneNoVerified} reaches another provider or classical lane and that provider deduplication never consumes the callback trace. The newest focused surface contains 14 package-private adapter cases and seven runner cases. It checks scalar/product synchronous and asynchronous exception restoration; immutable snapshots; every eager/deferred, unbudgeted, v1, and scoped-v2 route; source origins and open order; attempt and insertion caps; selection sealing and preserve-all continuity; live-session failure without cache rollback; reusable Integration contexts beside fresh wrappers and bank-free rank calls; and nominal, package-private, fatal-before-commit boundaries.

The focused strict bank adapter and runner surface passes 21/21, and the
complete strict Leant unit suite passes 438/438. Strict warning-as-error builds
of the test target and complete project are green; \code{cabal check} and
\code{git diff --check} are clean. The production and characterization changed
no golden fixture. The earlier golden-runner environment note remains
unchanged: no Lean REPL backend executable was installed there, so it was
...
\begin{itemize}
  ...
  \item Leant pinned tree: \href{https://github.com/VladimirReshetnikov/Leant/tree/0611e2c9685590508bf76cda165147c29b69d1fb}{\nolinkurl{github.com/VladimirReshetnikov/Leant/tree/0611e2c...}}
