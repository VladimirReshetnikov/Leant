...
\newcommand{\cmdrowbreak}[3]{%
  \parbox[t]{11.5em}{\code{#1}\\\code{#2}}\hspace{0.5em}%
  \parbox[t]{\dimexpr\linewidth-12em}{#3}\par}
...
\cmdrow{contract option}{\code{-{}-length-contract PATH} supplies one passive
  ...
  (\S\ref{sec:engines})}
\cmdrow{inline Length constraint}{\code{-{}-behavior-mode filter
  -{}-length-model MODEL -{}-length-inputs arg0[,argN...]
  -{}-where CLAUSE -{}- TYPE} selects one fixed exact-case list profile and
  one unquoted bounded postcondition; it is exclusive with a contract file and
  requires the startup policy (\S\ref{sec:engines})}
...
\cmdrow{:set synth-shown N}{accepted candidate groups shown, and the
  ranking stop (default 5)}
\cmdrow{:set synth-verify N}{fresh candidate groups Lean checks per lane
  (default 12; \code{both} sends twice that)}
\cmdrow{:set synth-window N}{candidate groups one lane may observe
  (default 60)}
\cmdrow{:set synth-budget N|off}{Djinn choice-point budget of the ordinary
  and provider lanes (default off)}
\cmdrow{:set synth-queue N}{Exference queue bound (default 1024)}
\cmdrow{:set synth-providers B}{discover live providers after the
  structural lane (\code{on}$\mid$\code{off}, default on)}
\cmdrowbreak{:set synth-provider-}{cap N}{providers one discovery may serialize
  (default 80)}
\cmdrowbreak{:set synth-library-}{premises N}{rated library offers kept per goal
  (default 8); \code{:set synth-ratings} lists the rated inventory}
\cmdrow{:set synth-debug B}{fragment, provider, lane-outcome, and metric
  diagnostics (\code{on}$\mid$\code{off}; \code{LEANT\_SYNTH\_DEBUG} at
  startup)}
\cmdrow{:set backend-timeout N}{seconds per Lean request (default 300,
  \code{--timeout} at startup; \code{0} none)}
\cmdrow{:set}{print every synthesis setting with its current value}
\end{cmdtable}
...
\small\code{:synth -{}-behavior-mode filter -{}-length-model
...
most 12 fresh candidate groups to Lean (\code{:set synth-verify N}). A
combined batch gets 24 and
...
collection window (\code{:set synth-window N}). The first spelling remains authoritative, while repeated
...
observation stops at the configured 60-group hard cap. Natural
...
groups (\code{:set synth-shown N}) and consumes one batch. An explicit behavioral filter verifies and
...
third tail probe. In either operation at most five candidates
(\code{:set synth-shown N}) are displayed and bound. Exference runs under
explicit budgets
...
verified occurrence, Lean discovers at most 80 live term providers
(\code{:set synth-provider-cap N}) under the
...
finite-list-spine \emph{Length} behavioral assessment: a passive JSON contract
or the fixed inline \code{-{}-where} surface states a law over the lengths of list-spine arguments and
...
candidate cutoff of 60. Established startup/file requests introduce one
rank-2 context before goal translation. An inline request creates its context
only after translation and physical-arrow resolution. Both carry that one
context through every ordinary, provider, and classical route. Each verified frontier enters exactly
...
The established startup/file option-bearing grammar is
...
passive scalar or binary-product contract, never startup execution policy.

The separate inline grammar is exactly
\code{:synth -{}-behavior-mode filter -{}-length-model
list-scalar-exact-cases|list-binary-product-exact-cases
-{}-length-inputs arg0[,argN...] -{}-where CLAUSE -{}- TYPE}. The bar presents
two alternative model tokens. The clause is unquoted; quotes are rejected
clause characters, and the standalone delimiter ends it. Inputs are a nonempty
strictly increasing subset of physical \code{arg0} through \code{arg7}; gaps
and unused declarations are allowed. Only translated source-arrow slots count,
not universal, instance, or contextual binders. Listed arrows become observed
roles in source order and omitted arrows become unobserved. The fixed profile
uses \code{List}/\code{List.nil}/\code{List.cons}, exact zero/step cases, true
precondition, and no provider laws; neither clause nor target infers authority
or semantic roles.

Main validates inline structure, selects the explicit or current goal,
authorizes literal filter mode, and enters Djex's bounded ASCII clause parser
before emitting the established inaccessible-hypothesis and premise-scope
report and starting Lean translation. Translation and its universe retry
supply the physical arrow arity; resolution precedes exactly one nominal
context shared by all lanes. A refusal before resolution creates no bank.
Provider-dependent candidates unsupported by the empty provider-law set are
retained as preparation refusals, never rejected merely for that dependency.

The startup policy fixes one guarded recursive piecewise-affine applicable-domain
...
bank, while rank contexts contain no bank. Main opens an established request's
context inside \code{synthRun} before initial translation; the inline entrance
opens its one context only after successful translation and resolution. Each
supplies its context to every cursor batch and later route. The
