...
\def\LeantCommit{49c8f9f100a644b5a32b3c942bc6565a3a3ba6a0}
...
``Linear'' means variables are not multiplied by one another and do not occur with variable exponents. Constant scaling such as $7x$ is linear; $xy$ and $x^2$ are nonlinear. Related names include \textsf{QF\_LRA} for linear real arithmetic, \QFBV{} for quantifier-free bit-vectors, and \textsf{QF\_AUFLIA} for arrays plus uninterpreted functions plus linear integer arithmetic.
...
\code{(push 1)} / \code{(pop 1)} & Save and restore assertion-stack scopes. \\
\code{(reset)} & Reset declarations, assertions, and much solver state. \\
...
SMT-LIB \code{Real} denotes exact mathematical reals, represented symbolically by rational and algebraic values. It is not IEEE floating point. Mixed integer/real formulas require explicit or theory-defined coercions such as \code{to_real}; one should not assume programming-language implicit conversions.
...
In the pure SMT-LIB Horn style, a clause with no positive relation in the head represents a safety query. The fixedpoint API also supports explicit \code{rule} and \code{query} commands.
...
\begin{lstlisting}[language=HaskellX,caption={Schematic typed QF-LIA plan; names are simplified.}]
data IntExpr
  = IVar Symbol
  | INumeral Natural
  | IAdd [IntExpr]
...
\item defines natural monus;
\item defines integer minimum;
\item defines integer maximum.
\end{enumerate}
...
\begin{itemize}
\item acceptance of required startup options;
\item \QFLIA{} satisfiability status;
\item model/value retrieval;
\item reset behavior;
\item response framing and whitespace rules;
\item transport liveness;
\item resource-limit behavior expected by the protocol.
\end{itemize}
...
\begin{lstlisting}[language=SMTLib,caption={Conceptual shape of the current fixed preamble.}]
(set-option :produce-models true)
(set-option :random-seed 1)
(set-logic QF_LIA)
...
The exact private symbols are allocated by the typed renderer and should not be treated as a stable public API. After the preamble, the query declares each input, declares any private Euclidean witnesses, asserts input nonnegativity and witness constraints, asserts the bad state, and calls \code{check-sat}.

A pedagogical query for the constant-empty candidate under unconditional length preservation is:

\begin{lstlisting}[language=SMTLib,caption={Pedagogical scalar Length counterexample query.}]
(set-option :produce-models true)
(set-option :random-seed 1)
(set-logic QF_LIA)
...
\item host deadline;
\item artifact-retention policy;
...
\begin{itemize}
\item QF-LIA acceptance;
\item reset behavior;
\item \sat/\unsat/\unknown{} status framing;
\item requested input-value responses;
\item command/response causality under chunked streams;
\item bounded parser compatibility;
\item transport and cleanup behavior.
\end{itemize}
...
The default load timeout is $5000$ milliseconds. The current acquisition entrance is POSIX-oriented and requires an admitted absolute path. Unless \code{--length-ranking-allow-unpinned} is present, activation rejects a policy with no expected executable digest.

The startup file is bounded JSON with a versionless current grammar. It has a fixed \code{format} literal and a required \code{rankingDomain} selecting \code{scalar} or \code{binary-product}. Its sections describe:

\begin{itemize}
\item execution admission and launch strategy;
\item executable path and optional expected SHA-256;
\item solver timeout, resource limit, and host deadline;
\item response byte/tree/token/integer bounds;
\item concrete evaluation limits;
\item optional origin probe;
\item optional finite input-box validation;
\item optional complete applicable-domain validation;
\item optional counterexample simplification;
\item optional nonvacuous-positive ranking preferences;
\item eager or deferred live-session opening;
\item optional shared usable-work budget;
\item one fixed scalar or binary-product contract.
\end{itemize}
...
\item[\code{Heuristic status}.] Z3 reported \sat, \unsat, or \unknown{}, but no independently replayed counterexample or positive finite receipt was attached.
...
\begin{enumerate}
\item Djex enumerates a bounded candidate prefix;
\item Lean verifies a batch of occurrences;
\item Length filtering rejects replayed violators and records useful inputs;
\item synthesis advances to another batch if more survivors are needed;
\item the new batch first replays retained inputs;
\item Z3 is opened only for candidates that survive pure replay and other enabled checks.
\end{enumerate}
...
\begin{enumerate}
\item \lpath{src/Leant/Synth/Length/Ranking/Internal.hs} or the selection sibling decides to open/use a worker under the configured policy.
\item \dpath{synthesis/src/Language/Haskell/Synthesis/Semantic/Length/SMTLib/Live.hs} exposes the public scoped capability.
\item private \filepath{Length/SMTLib/Session/} modules open, probe, reset, transact, and close the worker;
\item private \dpath{synthesis/internal/Language/Haskell/Synthesis/Internal/SMTLib/Causal/Driver.hs} and stream/response modules maintain framing;
\item private \dpath{synthesis/internal/Language/Haskell/Synthesis/Internal/SMTLib/Z3/Process.hs} owns process launch, transport, cancellation, and cleanup;
\item public replay gates return a status plus optional independently validated evidence.
\end{enumerate}
...
The existing filter can learn an input from candidate $C_1$ and replay it against later candidates $C_2,C_3,\ldots$. The next step is to expose the sample set to Djex's search scheduler before candidates reach Lean.
...
\begin{itemize}
\item newest-first MRU, as the current ranking layer uses for a small seed set;
...
A production assumption literal can imply the corresponding choice. During a candidate query, \code{check-sat-assuming} can activate a partial assignment and expose an unsat core over those assumptions.
...
\begin{lstlisting}[language=SMTLib,caption={Tracked contract clauses.}]
...
A fixedpoint engine such as Spacer can establish that \code{Bad} is unreachable by inferring an invariant equivalent to $r=n$. A faulty step rule might make \code{Bad} reachable and produce a derivation or trace.
...
When \code{Bad} is unreachable, Spacer may expose an inductive interpretation for relations. An invariant candidate $I(x,y)$ is useful only after checking:
...
\begin{enumerate}
\item raw fixedpoint \unsat{} and answer expression: heuristic solver observation;
...
\begin{description}
\item[Source input.] Should have a stable generated position and appear in the exact input-value request.
\item[Selector.] Future sketch choice; must have a checked finite range.
\item[Private witness.] Quotient/remainder, abstraction, or auxiliary value; should not be exposed as a source counterexample without a decoder.
...
\begin{itemize}
\item assessment is disabled;
\item the command used ordinary ranking with no enabled context;
\item configuration or activation failed before request admission;
\item the candidate route lacks exact typed authority;
\item handoff, contract sealing, candidate interpretation, or query construction refused;
\item a retained bank sample already produced a counterexample;
\item the origin probe or finite validator completed the assessment;
\item deferred opening was selected and no candidate reached a live miss;
...
\code{(reset)} & Reset declarations and assertions to a fresh solver state. \\
\code{(reset-assertions)} & Remove assertions while retaining declarations/options where supported. \\
...
In \QFLIA, multiplication must remain linear: at most one operand contains variables. The current Length translator lowers its admitted natural quotient/modulo operations to Euclidean witnesses rather than emitting \code{div}/\code{mod}.
...
\bottomrule
\endfoot
\lpath{docs/Leant.tex} & User-facing Leant manual and REPL orientation & Explains Leant broadly. This introduction supplies the logic/solver prerequisites and behavioral-assessment internals. \\
...
\item \lpath{src/Leant/Synth/Length/CounterexampleBank.hs}: nominal bank facade.
