...
Since this document's pinned August 16 snapshots, Leant has also activated the
fixed-profile inline Length where-clause frontend. Current-tree additions are
identified explicitly below; the immutable source links and their
point-in-time claims remain pinned.
\end{currentstate}
...
At the inspected snapshot, the command parser recognized two request-scoped
...
The post-snapshot current tree adds this exact fixed-order form:

\begin{lstlisting}[language={},caption={Current inline Length constraint form.}]
...
The bar presents two alternative literal model tokens. The clause is unquoted
and ends at the standalone delimiter; quotes are rejected clause characters.
The input token is a nonempty strictly increasing subset of physical
\code{arg0} through \code{arg7}. Gaps and unused declarations are valid.
Only translated source-arrow slots count, not universal, instance, or
contextual binders. Listed arrows become compact observed inputs in source
order; omitted arrows are unobserved.

The two fixed profiles supply \code{List}/\code{List.nil}/\code{List.cons},
exact zero/step cases, true precondition, scalar or canonical-product result,
and no provider laws. Neither clause nor target infers those choices. The
inline form requires the activated startup policy and literal filter mode and
is mutually exclusive with \code{--length-contract}. Main validates structure,
selects the goal, authorizes filter mode, parses the bounded ASCII clause,
reports inaccessible hypotheses and premise scope, translates/retries, counts
physical arrow slots, resolves the profile, and only then introduces exactly
one nominal assessment context for every lane. A provider preparation refusal
under the empty law set remains retained.
...
Fixed inline Length constraint & Implemented since the pinned snapshot & Unquoted bounded ASCII clause, explicit list model and physical inputs, literal filter authority, no provider-law inference. \\
...
\begin{sourcebox}
Request parser at the pin: \lpath{src/Leant/Synth/Length/Command.hs}. The
post-snapshot current-tree profile resolver is
\filepath{src/Leant/Synth/Length/Where.hs}. Startup options:
\lpath{src/Leant/Options.hs}. Configuration decoding and activation:
\lpath{src/Leant/Synth/Length/Configuration/File.hs} and
\lpath{src/Leant/Synth/Length/Integration.hs}. Ranking:
\lpath{src/Leant/Synth/Length/Ranking.hs}. Generic occurrence-sealed selection:
\lpath{src/Leant/Synth/BehavioralSelection.hs}. Scalar filter policy:
\lpath{src/Leant/Synth/Length/Selection.hs}; the binary-product sibling is
under \filepath{SpinePair/Selection.hs}. Main's progressive scheduler is in
\lpath{src/Main.hs}; its later inline activation is a current-tree addition to
that same path.
\end{sourcebox}
...
For a post-snapshot inline request, refine steps 4--6: current-tree
\filepath{src/Leant/Synth/Length/Command.hs} validates the fixed inline
structure; Main resolves the goal and asks Integration for literal filter
permission; current-tree \filepath{src/Leant/Synth/Length/Where.hs} enters the
bounded parser; Main reports inaccessible hypotheses and premise scope,
translates/retries, and counts only \code{SlotArrow} entries; Where resolves
the explicit role vector and fixed profile; only then does Main introduce one
context and enter the common scheduler. Established startup/file requests keep
their context-before-translation lifetime.
...
Inline syntax (post-snapshot) & current-tree \filepath{src/Leant/Synth/Length/Command.hs} and \filepath{src/Main.hs} & \code{inline finite-spine Length command rejected:} plus closed \code{LengthSynthInlineCommandError}: literal filter/model/inputs/where/order/delimiter/file-exclusion refusal & command refused \\
...
Inline clause/target (post-snapshot) & current-tree \filepath{src/Leant/Synth/Length/Where.hs} and \filepath{src/Main.hs} & sanitized \code{inline finite-spine Length where clause rejected:} or \code{... where target rejected:}; byte/ASCII/syntax/limit, physical-arity/role, or result-domain refusal, with no clause echo & command refused before context creation \\
...
\code{inline finite-spine Length command rejected:} \ldots{} & Inline option absent, repeated, out of order, combined with \bcode{--length-contract}, not preceded by literal filter, or missing its standalone delimiter & current-tree \filepath{src/Leant/Synth/Length/Command.hs} & Use exactly \bcode{--behavior-mode filter --length-model MODEL --length-inputs arg0[,argN...] --where CLAUSE -- TYPE}; do not quote the clause. \\
\code{finite-spine Length request rejected before IO:} \bcode{LengthAssessmentFilteringRequiresActivatedPolicy} or \bcode{LengthAssessmentExplicitContractRequiresActivatedPolicy} & Filter mode, an inline clause, or a contract file requested while assessment is disabled & \lpath{src/Leant/Synth/Length/Integration.hs}, \bcode{startupLengthAssessmentRequest} & Start Leant with \bcode{--length-ranking-config}; neither a contract file nor inline clause can activate Z3. \\
\code{inline finite-spine Length where clause rejected:} or \code{... where target rejected:} & Non-ASCII/quoted/malformed or over-limit relation; undeclared/out-of-range \bcode{argN}; wrong scalar/product result reference; or more than eight physical arrows & current-tree \filepath{src/Leant/Synth/Length/Where.hs}, \filepath{src/Main.hs} & Use an unquoted bounded relation, declare a strictly increasing subset of \bcode{arg0} through \bcode{arg7}, and match result references to the explicit model. \\
...
Every refusal before a worker exists is a pure decision printed with its constructor name; there is no hidden state to inspect. Several ordered places can refuse.
...
\item[The \code{:synth} line.] At the pin, \lpath{src/Leant/Synth/Length/Command.hs} parses only two exact tokens. \bcode{--behavior-mode} without a following word is \code{LengthSynthCommandBehaviorModeMissing}; a word other than \code{rank}/\code{filter} is \code{...BehaviorModeInvalid}; either option without a later standalone \bcode{--} is \code{...DelimiterMissing}; \bcode{--length-contract} directly followed by \bcode{--} is \code{...ContractPathMissing}; and a mode token after the contract option is \code{...BehaviorModeMustPrecedeContract}. Main prints the single established \code{usage:} line for all five. Longer look-alikes such as \bcode{--behavior-model} are ordinary goal text and are handed to Lean. The post-snapshot current tree first recognizes the separate fixed-order inline form. Missing, repeated, reordered, non-filter, or file-mixed inline options produce \code{inline finite-spine Length command rejected:} plus one closed \code{LengthSynthInlineCommandError} and the exact inline usage line; no retained clause is shown.

\item[Request authorization.] Before any contract path is admitted, \lpath{src/Leant/Synth/Length/Integration.hs} asks the activated mode for permission. With assessment disabled, \bcode{--behavior-mode filter} yields \code{LengthAssessmentFilteringRequiresActivatedPolicy} and \bcode{--length-contract} yields \code{LengthAssessmentExplicitContractRequiresActivatedPolicy}; both are printed after \code{finite-spine Length request rejected before IO:}. The post-snapshot inline path resolves its goal source, then asks for the same literal-filter permission before parsing the clause. Only after permission is a named contract file admitted (\code{one-shot finite-spine Length contract admission failed:}) and read (\code{... load failed:}), or an inline clause entered through the bounded parser. A file decoder can never supply execution authority.

\item[Inline clause and target.] Current-tree \filepath{src/Leant/Synth/Length/Where.hs} parses the authorized clause into source-free bounded data. Main then emits the established inaccessible-hypothesis and premise-scope report and translates/retries before the resolver receives the \code{SlotArrow}-only arity. Clause failures use \code{inline finite-spine Length where clause rejected:}; physical-input, role, or scalar/product result-association failures use \code{... where target rejected:}. Both are sanitized and occur before the one nominal context exists.
\end{description}
...
The proposal was tied to an August 15 snapshot at which Length was described as ranking-only. The Leant snapshot inspected for this document has already implemented the narrow replay-authorized filter path. Since this document's August 16 pin, the fixed-profile inline Length constraint has also shipped through the same filter authority and checked handoff. Full solver-guided sketches, general CEGIS, richer domains, CHCs, and proof artifacts remain planned.
...
Post-snapshot current-tree additions, intentionally not linked through the
pinned base, are \filepath{src/Leant/Synth/Length/Command.hs} (the separate
inline structural parser), \filepath{src/Leant/Synth/Length/Where.hs} (fixed
profiles, bounded clause boundary, and physical-role resolver), and
\filepath{src/Main.hs} (authorization, scope reporting, translation/retry,
\code{SlotArrow} arity, resolution, and one-context activation).
