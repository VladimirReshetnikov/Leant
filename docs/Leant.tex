...
\code{Nat} brings a curated handful of library functions with it ---
\code{List.map}, \code{List.foldr}, \code{List.append},
\code{List.flatten}, \code{Nat.add} --- instantiated at the goal's
own types and offered to the engine as extra premises. Asking for a
function \emph{by its type} then works for types whose one sensible
inhabitant is recursive:
...
library search runs beside the constructor search of the previous
