...
outer 12- and 24-group verification frontiers then apply.  Whatever the
frontier verifies, at most five candidates are ever displayed and bound;
the rest of the frontier exists to survive Lean's rejections.  Exference
runs under
explicit budgets
...
verified batch and exposes no partial rejection. A
...
cache, and does not call them Lean-verification failures. Behavior mode,
selection results, and the MRU are never persistent REPL state. The
current reference is \code{docs/length-ranking.md}; the connected checkpoint
is recorded in
\code{docs/reports/2026-08-15-command-authorized-length-filtering.md}. Dated
