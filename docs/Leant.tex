...
Two implementations live side by side in the repository: the Haskell
implementation at the repository root — the primary one, where
development happens, speaking the backend protocol directly — and
\code{LeantPy/}, a lighter Python edition built on LeanInteract that
is kept up to date with it. They share the command set and the same session semantics;
...
afterwards. Proof-state identifiers are local to one backend process; if
it stops, Leant leaves prove mode and prints the accumulated script rather
than submitting a stale identifier after session reconstruction.
...
never invents a polytype the query did not supply. Djinn's current
context-free hypothesis rule reaches four leading binders without
raising its fixed search caps; chains with five or more stay opaque.
Its singleton, pairwise, and triple open/opaque frontiers grow
cubically and cover every choice through seven independent quantified
sites without enumerating a power set. That scope is still being
...
A polymorphic hypothesis can be instantiated at types the goal itself
supplies. Context-free chains now reach four leading binders, and Leant
inserts the inferred type arguments which Djinn leaves implicit. Lean
then verifies each rendered candidate against the original type:
...
Vacuous local providers can retain the same query-supplied choice even
when their result is an opaque Lean type. Here \code{Demo.Token} becomes
a fixed ambient query rigid, not an unresolved inference variable:
...
context-free leading provider chain, and only selects a closed proper type
already present in the checked query. Fixed ambient rigids may occur in the
provider result; a free flexible variable still rejects the route. A local
value has no global binder name, so Leant renders a positional \code{@}
application. For a quantified argument it tries binder domains \code{\_},
\code{Type \_}, and \code{Prop}, in that order, and backend verification keeps
the first spelling which elaborates. Named globals retain their
source-directed applications. Each domain lane retains at most twelve
site-and-style spellings, for a hard 36-variant bound only when the local
quantified argument is domain-sensitive; ordinary duplicate spellings collapse
to the historical smaller group.

Leant also retains applications whose arguments are proper types. A
bound first-order constructor remains a higher-kinded variable in Djex;
an opaque Lean family receives one rigid nominal head across its
occurrences. Both engines can therefore see the query-supplied
polytype without learning any hidden implementation:
...
only application arguments whose own type is a universe. Term-indexed
families such as \code{P 3} remain opaque. A retained nominal application
can support a positive, Lean-verified candidate but always poisons a
negative verdict because its constant hides structure.

The candidate vocabulary comes only from proven proper-type positions below
arrows and tuples in the sequent, including quantified subtrees already
present there; this is guarded impredicativity, not general second-order
search. A visible provider chain is capped at four binders and 32 argument
combinations. The separate explicit-instance-head route remains
monotype-only, and open or context-bearing specified arguments fail closed.
Chains with five or more leading binders remain a deliberate incomplete
boundary.
...
alternating tails. Exference runs under explicit budgets
...
provider-free lane and asks Lean to verify its candidates. After a
nonterminal miss, Lean discovers at most 80 live term providers under
the namespaces used by the target (plus exact session declarations).
...
improves. The current plan family is exhaustive through seven
independent quantified sites; an eight-site four-open/four-opaque goal
is the next deliberate occurrence-plan gap, while hypothesis
instantiation chains with five or more binders remain opaque. Such
...
candidate's term binders. Provider projection does not serialize Lean's
instance registry into Djex, so a polytype visible only inside an instance
head is not a candidate: it must already occur in the checked query. Extending
that boundary would require bounded, checked caller-supplied evidence rather
than an approximation through Haskell classes. The fragment is bounded on
purpose: inside it, answers are fast and verified. Refutations are final for
the complete provider-free structural calculus; the bounded, best-effort live-provider
inventory is
not an exhaustive theorem about every declaration or axiom in the Lean
environment.
...
Leant is a working prototype under active development. The Haskell
implementation at the repository root is primary; the Python edition
(\code{LeantPy/}) is kept up to date with it as a lighter alternative
without synthesis.
