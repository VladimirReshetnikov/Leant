...
\cmdrow{:synth TYPE}{synthesize verified terms of \code{TYPE};
  ...
  (\S\ref{sec:synth})}
\cmdrow{:synth --behavior-mode rank|filter -- TYPE}{explicitly choose the
  operation; disabled \code{rank} is identity, while filtering uses the
  activated startup Length contract and requires its policy}
\cmdrow{:synth --length-contract P -- TYPE}{use one passive Length contract
  file for this command only; an optional preceding behavior mode selects
  \code{rank} or \code{filter}, omitted mode means \code{rank}, the standalone
  delimiter is mandatory, and every contract form requires the startup policy
  opt-in
  (\S\ref{sec:engines})}
