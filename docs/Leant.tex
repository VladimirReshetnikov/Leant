...
containing depth truncation (\code{FDepth}) or a legacy raw instance marker
(\code{FInst}) is discarded in full. Dedicated exact-assignment serialization
may instead retain a bounded \code{FExactContext}: the exact nominal Lean class
head, every ordered argument's bounded ground-kind arity, and the contextual
body. Positive-arity arguments retain only a canonical bare or partially
applied nominal head with proper-type supplied arguments, serialized as
...
nominal-family arity claims. Provider-prefix fallback keeps metadata
attached to its exact declaration. The checked runners validate provider identity, arity,
...
passed in total. A closed contextual argument such as
...
ground-kinded arguments as \code{FExactContext}. Higher-kinded arguments such
as \code{List} retain their canonical nominal head and residual kind arity;
Djex checks the reconstructed class-parameter kind and rendering restores the
exact Lean class head. The same contextual path supports canonical
\code{Prod} and \code{Sum}; the direct path preserves their structural Djex
identities. Open, incomplete,
...
\code{FInst} slots and provider schemes continue to erase instance evidence;
neither path gains contextual search. This provider-local channel can expose a
