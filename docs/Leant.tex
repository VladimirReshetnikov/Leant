...
\code{LeantPy/} (the Python original, whose backend management is
delegated to LeanInteract) and the Haskell port at the repository
root, now the primary implementation, which speaks the backend
protocol directly. They share the command set and the same session semantics;
...
implementation at the repository root is primary; the Python original
(\code{LeantPy/}) remains as a lighter alternative without synthesis.
The synthesis design and its phased history are documented in
\code{SYNTHESIS\_PROPOSAL.md}; transcript-level golden tests live in
