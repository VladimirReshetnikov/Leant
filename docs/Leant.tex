...
singleton, pairwise, and triple open/opaque frontiers grow cubically and
cover every choice through seven independent quantified sites without
enumerating a power set. That scope is still being improved, and Leant
keeps the boundary to the engines narrow on purpose, so each
improvement arrives by version bump. The engines speak Haskell types;
...
is the next deliberate gap. Such higher-rank goals answer ``no term
