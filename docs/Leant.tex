...
Context-free instantiation chains extend to four leading binders. Leant
...
Discovery opens no more than four leading proper-type binders on one exact
provider while retaining its erased instance constraints. It inspects at most
32 active heads per provider in Lean resolver order, trying each under restored
metavariable state, and retains at most 16 distinct closed, context-free proper
types. The whole command passes no more than 32 provider/type associations to
Djex, and takes that aggregate prefix before a candidate affects family
planning, rigidity, or translation. Candidate metadata is optional in the
provider protocol, so historical
metadata-free and binder-only inventories still parse. A serialized candidate
containing depth truncation (\code{FDepth}) or an instance binder
(\code{FInst}) is discarded, including an instance binder revealed by reducing
an alias. Provider-prefix fallback keeps this metadata attached to its exact
declaration, and the checked Djinn and Exference runners validate the same
exact-provider association. Thus a later or alpha-identically typed provider
cannot donate its choice to an earlier one.
...
as \code{P 3} remain opaque. A retained nominal application can support a
...
quantified subtrees already present there; provider-local candidates may also
come from the bounded active-instance channel above. This is guarded,
evidence-directed impredicativity, not general second-order search. The
query-derived visible-provider route is capped at four binders and 32 argument
combinations; Djex's internal Haskell-instance route remains monotype-only. The
external Lean evidence route has the separate four-binder, 32-head,
16-candidate-per-provider, and 32-association limits. Open or context-bearing
specified arguments fail closed, and chains with five or more leading binders
remain a deliberate incomplete boundary. The original local-provider contract
and the active-instance extension are recorded in the
...
pass can attach bounded active-instance-head choices. Resolution follows Lean's
own instance order but isolates each attempted head's metavariable state.
Provider slicing retains the association only with its source declaration;
after context/depth filtering Leant takes the command-wide prefix before
planning or translation, then calls the checked Djinn or Exference evidence
runner. Lean still reconstructs every dictionary during final
elaboration---only the closed proper-type choice crosses the bridge.
...
heads only to obtain bounded proper-type choices for the exact provider whose
erased constraint they solve. It never serializes dictionaries or general
instance resolution into Djex. Up to 32 heads are inspected and 16 distinct
choices retained per provider, with at most 32 associations passed in total;
open, depth-truncated, and contextual choices fail closed. The reducible
...
on purpose: inside it, answers are fast and verified. Refutations are final for
