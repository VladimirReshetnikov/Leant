...
The refill boundary is deliberately finite and lane-local. A rank request asks
Lean for at most five successful groups. A filter request first bounds its
input, then may verify and assess up to all 12 groups in a standalone lane, all
24 in a combined lane, or all 6 in the cheaper excluded-middle classical
route. That verified filter frontier enters one assessment batch, allowing its
fresh nominal scalar or product context bank to replay an early
counterexample against a later occurrence. Existing ranking and direct
Selection compatibility paths keep the raw four-vector MRU unchanged. Only
after selection does Leant take at most five survivor
presentations for display and binding; every rejection from the bounded batch
is still printed.
...
Integration can introduce one reusable rank-2
\code{LengthAssessmentContext}; repeated context assessments reuse its filter
bank, while rank contexts contain no bank. Its compatibility request assessor
creates a fresh context per call. Main still calls that wrapper once for each
bounded lane and retains no context, so a later lane starts fresh.
...
verified occurrences are rejected, the result is handled: Leant prints the
rejections, creates no \code{itN} binding, clears its previous synthesis-splice
cache, does not call them Lean-verification failures, and does not continue
into a later provider or classical lane.

Main now records one bounded lane in private lazy outcome types rather than
collapsing these cases into a Boolean. The outcome keeps the complete bounded
rendered-spelling frontier used for provider deduplication separate from the
exact variants which reached Lean callbacks. Verification and its one Length
assessment perform no candidate presentation or synthesis-splice mutation. A
pure classifier distinguishes no verified receipt, survivors, all behavioral
rejections, and preserve-all assessment failure; one finalizer owns metrics,
warnings, binding and cache effects, rows, and handled-lane notes. Only the
empty-verified-receipt disposition can continue to another provider or
classical lane, so the refactor changes no scheduling or output behavior.

Behavior mode, assessment contexts, selection results, both kinds of
replay-input store, and the lane outcomes are never persistent REPL state. This
is not the proposal's full command-local CEGIS loop: Main carries no context or
candidate across a lane, and no engine, verification module, scheduler, or REPL
state changed. The current reference
is \code{docs/length-ranking.md}; the command-authority predecessor, current
