...
into a later provider or classical lane. Behavior mode, selection results, and
the MRU are never persistent REPL state. This is not the proposal's full
command-local CEGIS loop: no bank or candidate crosses a lane, and no engine,
verification module, scheduler-result type, or REPL state changed. The current
reference is \code{docs/length-ranking.md}; the command-authority predecessor
and current refill checkpoint are recorded in
\code{docs/reports/2026-08-15-command-authorized-length-filtering.md} and
\code{docs/reports/2026-08-15-lane-local-length-survivor-refill.md}. Dated
