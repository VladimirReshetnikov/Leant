...
Discovery opens no more than four leading type binders on one exact
...
\code{runExferenceQueryWithKindedInstantiationAssignments}. Metadata-free and
...
bounds, and the exact vacuous application under Djinn, Exference, and combined
mode. The live \code{synth-provider-higher-kind-assignment} transcript retains
the mixed higher-kinded/rank-N vector and requires
\code{Higher.vacuous («F» := Higher.Wrap)} in all three modes.
...
or Exference assignment runner. Lean still reconstructs every dictionary
