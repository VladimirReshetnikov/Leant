...
The candidate vocabulary comes only from the sequent, including
quantified subtrees already present there; this is guarded
impredicativity, not general second-order search. Chains with five or
more leading binders remain a deliberate incomplete boundary.
...
Before Exference searches, Lean discovers at most 80 live term providers
under the namespaces used by the target (plus exact session
declarations). Type constructors and type families, whose fully peeled
result is a sort, are excluded before search. Exact-result session and public declarations lead the
ranking; conventional implementation-worker names remain eligible but
follow public fallbacks, while session declarations bypass that naming
heuristic. The serializer derives a canonical query from the
...
opened. Opaque and bound-variable proper-type applications retain their
ordered arguments, but qualifying inductives still use the separate
constructor-expansion projection; sharing one parametric family across
distinct \code{Option}/\code{List} occurrences remains the next bridge
boundary. Quantifier handling beyond the propositional fragment follows
