...
four-entry MRU input bank to replay an early counterexample against a later
occurrence. Only after selection does Leant take at most five survivor
...
inside \code{applicableDomainValidation}. The MRU, domain, origin, deferred
opening, live query, post-\code{unsat} input-box, scoped-budget, and stable
preference boundaries remain unchanged. The four-vector MRU is local to one
verified batch; a later candidate must independently replay a vector against
its own checked problem, and the next batch starts empty.
...
query-owned fresh replay. Leant now has a package-private pure state adapter
over that bridge: each nominal domain retains validated limits and at most one
active scope, resets on exact scope drift, threads charged successors through
newest-first replay, and separates a replay hit from evaluation-free
promotion. No ranking, filtering, integration, command, or persistent-state
path calls the adapter. It is therefore a dormant runtime foundation, not a
command-local bank or a change to the active batch-local MRU.
...
displays at most five original candidates. A
...
Behavior mode, selection results, both kinds of replay-input store, and the
lane outcomes are never persistent REPL state. This is not the proposal's full
command-local CEGIS loop: no bank or candidate crosses a lane, and no public
API, engine, verification module, or REPL state changed. The current reference
...
\code{docs/reports/2026-08-16-explicit-synthesis-lane-outcomes.md}. The dormant
nominal state foundation is recorded in
\code{docs/reports/2026-08-16-nominal-length-counterexample-bank-state.md}.
Dated
