...
its constant hides structure. Candidate types come only from proven proper-type
positions below arrows and tuples in the sequent, including quantified subtrees
already present there: this is guarded impredicativity, not general
second-order search. A visible provider chain is capped at four binders and 32
argument combinations. The separate explicit-instance-head route remains
monotype-only; open or context-bearing specified arguments fail closed, and
chains with five or more leading binders remain a deliberate incomplete
boundary. The implementation contract and Lean~4.31 transcript are recorded in
the \href{reports/2026-08-01-scoped-quantified-local-providers.md}{scoped
...
proofs that use the goal's own arguments, so asking for a function
\emph{by its type} puts the library answer first:
