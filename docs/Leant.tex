...
disjunction goal is probed with \code{left}/\code{right}, and a
hypothesis whose type a single step can take apart is probed with
\code{cases} or \code{obtain}.  The search prefers a candidate that
closes the goal outright and annotates the suggestion accordingly
(\code{closes the goal}, \code{splits into 2 goals}); when no single
tactic closes it, a second phase chains quick finishers onto the best
progressing candidates (\code{constructor <;> omega}, or an
\code{obtain} step chained with the \code{exact} it enables), so even
the opening suggestion is often a complete checked proof.
