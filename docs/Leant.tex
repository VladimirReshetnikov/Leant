...
\code{-{}-length-ranking} options opt into finite-list-spine Length
counterexample ranking (\S\ref{sec:engines}): the configuration file is
...
\cmdrow{:synth TYPE}{synthesize verified terms of \code{TYPE};
  ...
  (\S\ref{sec:synth})}
\cmdrow{:synth --length-contract P -- TYPE}{use one passive Length
  contract file for this command only; the standalone \code{-{}-} is
  mandatory and requires the startup ranking opt-in
  (\S\ref{sec:engines})}
...
\code{-{}-length-ranking-config} at startup (\S\ref{sec:start}) opts
into finite-list-spine \emph{Length} counterexample ranking: a passive
...
\code{QF\_LIA} queries, and a scoped Z3 worker is consulted once per
batch.  Only a counterexample that Leant's vendored Djex engine has
independently replayed against the exact checked problem can move a
candidate --- such candidates are stably demoted, never pruned --- and a
raw \code{sat}, \code{unsat}, or \code{unknown} has no authority at
all.  A per-command variant, \code{:synth -{}-length-contract PATH
-{}- TYPE}, swaps in one contract file for a single command.  The
...
preference boundaries remain unchanged. The
current reference is \code{docs/length-ranking.md}; dated
reports under \code{docs/reports/} are historical engineering records rather
than current file contracts. Everything in this paragraph
is off by default and leaves no trace when disabled.
