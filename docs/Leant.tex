...
other candidate. An explicit behavioral filter uses the same command-local
bank in both routes: either no verification or complete all-rejection in the
excluded-middle lane enters double negation, while any survivor or
preserve-all result stops. The sound-refutation caller accumulates both attempts and
finalizes them only once. The whole behavior sits behind a switch:
...
outer 12- and 24-group verification frontiers then apply.

Independently of the executable scheduler, \code{Leant.Synth.Engine} now
exports an opaque \code{DetailedSynthCursor} over one lazy detailed outcome.
...
deadline. Main imports none of this new surface and still uses the one-window
behavior described here; the cursor is a foundation for a later progressive
scheduler, not a new command behavior.

Ordinary ranking retains its historical verifier stop after five accepted
groups.  An explicit
behavioral filter may instead verify the rest of that same bounded lane, assess
the verified occurrences once, and refill early behavioral rejections from
later same-lane survivors.  In either operation at most five candidates are
displayed and bound.  Exference runs under
explicit budgets
...
Every assessment remains deliberately lane-bounded. A rank request asks Lean
for at most five successful groups. A filter request first bounds its input,
then may verify and assess up to all 12 groups in a standalone lane, all 24 in
a combined lane, or all 6 in the cheaper excluded-middle classical route. Main
introduces one rank-2 context before goal translation and carries it through
universe retries and every ordinary, provider, and classical lane. Each
verified frontier enters one assessment batch, while the filter context's one
nominal scalar or product bank can replay an earlier counterexample against a
later occurrence or same-scope lane. Existing ranking and direct Selection
...
\code{synthRun} before initial translation and supplies it to every lane. Its
...
verified occurrences are rejected, the lane is handled but not yet finalized:
like a no-verified lane, it may enter the next provider or classical route with
the same bank and its complete spelling frontier in the deduplication set.
Survivor and preserve-all results remain terminal.

Main prepends each actual lane outcome to one private lazy accumulation in
reverse attempt order. Each outcome keeps the complete bounded
rendered-spelling frontier separate from the exact variants which reached Lean
callbacks. Verification and assessment perform no candidate presentation or
synthesis-splice mutation. A pure fold distinguishes no verified receipt,
survivors, aggregate all-rejection, and preserve-all failure, stopping at the
first terminal result. One deferred finalizer owns metrics, warnings, binding
and cache effects, rows, and handled-lane notes. Terminal candidate rows print
first; rejection rows are every earlier all-rejected projection followed by
the terminal lane's rejections; handled notes retain that chronological order.
No-verification contributes only its debug metrics. Final all-rejection clears
the old splice cache once; survivor or preserve-all output replaces it once.

Continuation does not fill a five-survivor quota across lanes. The first lane
with even one survivor stops scheduling, ranking retains its historical
five-success quota, and filtering retains each 12/24/6 bound. On ordinary
...
refutation, timeout or engine error finalizes completed outcomes first and then
prints its unchanged abnormal diagnostic.
...
context survives the command. Main's engine invocation, verification module,
and REPL state contract remain unchanged. The separate additive Engine cursor
does not enter this path. The current reference
