...
-- the binary-product finite-spine domain.  This is the nominal sibling of
...
-- "Leant.Synth.Length.Ranking.Generic" and is shared with the scalar domain.
-- This module owns the product instance of that generic core and the nominal
-- public vocabulary that instance produces: the pair assessment strengths, the
-- sanitized failure classes, the four assessment policies, the opaque
-- ranking, candidate, and post-verification receipt types, and the three
-- entrances which replay and record through a command-local pair
-- counterexample bank instead of the batch-local MRU.  Every exported name
-- and signature is the established one; the values wrap the shared structure
-- in product newtypes so a pair receipt can never be confused with a scalar
-- one.  The shared vocabulary the two domains export together
-- ('LengthRankingInputError', 'LengthPreparationRefusalClass',
-- 'LengthLiveSessionOpeningPolicy') is exported by the scalar module only.
...
import Leant.Synth.Length.Handoff (LengthSpinePairHandoffRefusal (..))
import Leant.Synth.Length.Ranking.Generic
...
-- | The binary-product finite-spine ranking domain.  This tag selects the
...
-- | Pair-domain assessment strengths.  Heuristic status, finite-box success,
...
--
-- 'Nothing' with 'LengthSpinePairUnassessed' means that preparation succeeded
-- but an operational batch failure atomically reset the assessment; callers
-- should
...
lengthSpinePairRankingCandidates
  ...
  -> [RankedLengthSpinePairCandidate]
lengthSpinePairRankingCandidates
    (LengthSpinePairRanking (Ranking candidates _)) =
  map RankedLengthSpinePairCandidate candidates

-- | The batch-wide sanitized failure behind an all-'LengthSpinePairUnassessed'
-- fallback.
lengthSpinePairRankingFailure
  :: LengthSpinePairRanking
  -> Maybe LengthSpinePairRankingFailure
lengthSpinePairRankingFailure
    (LengthSpinePairRanking (Ranking _ failure)) = failure
...
postVerificationLengthSpinePairRankingBatch
    (PostVerificationLengthSpinePairRanking
      (PostVerificationRanking batch _ _)) = batch

-- | The batch-wide sanitized failure retained from the sealed associated
-- ranking, when its candidates were left 'LengthSpinePairUnassessed'.
postVerificationLengthSpinePairRankingFailure
  :: PostVerificationLengthSpinePairRanking
  -> Maybe LengthSpinePairRankingFailure
postVerificationLengthSpinePairRankingFailure
    (PostVerificationLengthSpinePairRanking
      (PostVerificationRanking _ _ failure)) =
  failure
...
-- | Counterexample-bank filter entrance.  It is the occurrence-associated
-- complete-policy runner whose counterexample replay and recording go through
...
rankPostVerificationLengthSpinePairCandidatesWithRankingPoliciesAndCounterexampleBankContextAndLiveSessionOpening
    ...
    simplificationPolicy openingPolicy execution evaluation contract =
  fmap (fmap AssociatedLengthSpinePairRanking)
    . rankAssociatedCandidatesWithLiveSessionOpeningAndCursor @PairLength
...
rankPostVerificationLengthSpinePairCandidatesWithRankingPoliciesAndCounterexampleBankContextAndUsableWorkBudget
    ...
    contract =
  rankAssociatedCandidatesWithUsableWorkBudgetAndCursor @PairLength
    (CommandLocalCounterexampleBank context) budget
    (finalize . AssociatedLengthSpinePairRanking)
    forceAssociatedLengthSpinePairRankingOwnedResult
    (rankingPolicies inputBoxPolicy applicableDomainPolicy originProbePolicy
      simplificationPolicy)
...
rankPostVerificationLengthSpinePairCandidatesWithRankingPoliciesAndCounterexampleBankContextAndScopedUsableWorkBudget
    ...
    originProbePolicy simplificationPolicy openingPolicy execution evaluation
    contract =
  rankAssociatedCandidatesWithScopedUsableWorkBudgetAndCursor @PairLength
    (CommandLocalCounterexampleBank context) budget
    (finalize . AssociatedLengthSpinePairRanking)
    forceAssociatedLengthSpinePairRankingOwnedResult
    (rankingPolicies inputBoxPolicy applicableDomainPolicy originProbePolicy
...
preferNonVacuousBoundedPositiveLengthSpinePairRanking
  ...
  -> LengthSpinePairRanking
preferNonVacuousBoundedPositiveLengthSpinePairRanking
    (LengthSpinePairRanking ranking) =
...
-- discipline.  The two domains share one bank shape because a seed is only an
-- input vector.
promoteLengthSpinePairCounterexampleSeed
  :: [Natural]
  -> [[Natural]]
...
instance LengthRankingDomain PairLength where
  ...
  prepareQuery contract verified =
    case prepareCheckedLengthSpinePairQuery contract verified of
      ...
      Right (Left refusal) ->
        Left $ lengthSpinePairQueryPreparationRefusalClass refusal
  ...
  simplifyCounterexample evaluation limits query receipt =
    case simplifyLengthSpinePairSMTLibQueryCounterexample
      ...
      Left (LengthSpinePairSMTLibCounterexampleSimplificationAssociationRejected
          _) ->
  ...
  buildFailureClass view = case view of
    ViewLiveSessionFailed nested ->
      LengthSpinePairRankingLiveSessionFailed nested
