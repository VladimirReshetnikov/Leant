...
class LengthRankingDomain d where
...
rankAssociatedCandidatesWithUsableWorkBudget budget finish forceResult
...
rankAssociatedCandidatesWithScopedUsableWorkBudget
  ...
  -> [association]
  -> IO (Either LengthRankingInputError result)
rankAssociatedCandidatesWithScopedUsableWorkBudget budget finish
...
runPreparedCandidatesUnderUsableWorkDeadline
  :: forall d association budget. LengthRankingDomain d
...
runPreparedCandidatesWithDeferredLiveSessionOpeningUnderDeadline deadline
  ...
  PreparedCandidatesFailed failure -> pure
    $ unassessedRanking prepared failure
  PreparedCandidatesNeedLive reversed seedBank index association query
      rest -> do
    scoped <- withLengthSMTLibLiveSessionUnderDeadline deadline execution
      $ \session -> do
...
runPreparedCandidatesWithDeferredLiveSessionOpeningUnderScopedDeadline
  ...
  case beforeLive of
    ...
    Right (PreparedCandidatesFailed failure) -> pure $ Right
      $ unassessedRanking prepared failure
    Right (PreparedCandidatesNeedLive reversed seedBank index
...
runPreparedCandidatesBeforeLiveUnderScopedUsableWorkDeadline
  ...
  -> IO
      (Either LengthSMTLibLiveSessionError
        (PreparedCandidatesBeforeLive d association))
  ...
  go reversed seedBank remaining = case remaining of
    ...
    PreparedCandidateUnassessed index association refusal : rest ->
      continue
        ...
        seedBank rest
    PreparedCandidateEligible index association query : rest -> case
...
runPreparedCandidatesFromUnderScopedUsableWorkDeadline
  :: forall d association budget epoch. LengthRankingDomain d
  => LengthSMTLibLiveScopedUsableWorkDeadline budget
  -> LengthEvaluationLimits
  -> RankingPolicies
  -> LengthSMTLibLiveSession epoch
...
rankAssociatedCandidates policies execution evaluation contract
    verifiedFor associations =
...
rankAssociatedCandidatesWithLiveSessionOpening
  ...
  -> [association]
  -> IO (Either LengthRankingInputError (AssociatedRanking d association))
rankAssociatedCandidatesWithLiveSessionOpening policies openingPolicy
...
data PreparedCandidatesBeforeLive d association
...
type role PreparedCandidatesBeforeLive nominal nominal
...
runPreparedCandidatesWithDeferredLiveSessionOpening execution evaluation
    policies prepared = case runPreparedCandidatesBeforeLive evaluation
        applicableDomainPolicy originProbePolicy simplificationPolicy prepared of
  PreparedCandidatesCompleted assessed -> pure
    $ AssociatedRanking (stableCounterexampleDemotion assessed) Nothing
  PreparedCandidatesFailed failure -> pure $ unassessedRanking prepared failure
  PreparedCandidatesNeedLive reversed seedBank index association query
      rest -> do
        ...
        Right observation -> case assessCandidate evaluation inputBoxPolicy
          ...
          Right assessed ->
            let nextSeedBank = case counterexampleSeed assessed of
                  ...
                  Just retained -> promoteCounterexampleSeed retained seedBank
            in nextSeedBank `seq` runPreparedCandidatesFrom evaluation
                policies session (assessed : reversed) nextSeedBank rest
    pure $ case scoped of
      Left failure -> unassessedRanking prepared
        $ sessionRankingFailure @d failure
      Right (Left failure) -> unassessedRanking prepared failure
...
runPreparedCandidatesBeforeLive evaluation applicableDomainPolicy
 ...
 where
  go reversed seedBank remaining = case remaining of
    [] -> PreparedCandidatesCompleted $ reverse reversed
    PreparedCandidateUnassessed index association refusal : rest ->
    ...
    PreparedCandidateEligible index association query : rest -> case
      ...
      Just (inputs, receipt) ->
        inputs `seq` case simplifyCounterexampleAssessment evaluation
            simplificationPolicy index association query receipt of
          Left failure -> PreparedCandidatesFailed failure
          Right assessed -> continueAssessed reversed seedBank rest assessed
      Nothing -> case assessApplicableDomainCandidate evaluation
          applicableDomainPolicy simplificationPolicy index association query of
        Left failure -> PreparedCandidatesFailed failure
        Right (Just assessed) -> continueAssessed reversed seedBank rest assessed
        Right Nothing -> case probeOriginCounterexample @d evaluation
            originProbePolicy query of
          ...
          Right (Just receipt) -> case simplifyCounterexampleAssessment
            ...
            Left failure -> PreparedCandidatesFailed failure
            Right assessed -> continueAssessed reversed seedBank rest assessed
          Right Nothing -> PreparedCandidatesNeedLive
            reversed seedBank index association query rest
...
runPreparedCandidates
  :: forall d association epoch. LengthRankingDomain d
  => LengthEvaluationLimits
  -> RankingPolicies
  -> LengthSMTLibLiveSession epoch
  -> [PreparedCandidate d association]
...
runPreparedCandidatesFrom evaluation policies session = go
 where
  ...
  go reversed seedBank remaining = case remaining of
    ...
    PreparedCandidateUnassessed index association refusal : rest ->
      go (AssociatedRankedCandidate
        index association (CandidatePreparationRefused refusal) : reversed)
        seedBank rest
    PreparedCandidateEligible index association query : rest -> case
          replayCounterexampleSeeds @d evaluation query seedBank of
      Just (inputs, receipt) ->
          ...
          Right Nothing -> do
            ...
            case observed of
              ...
              Right observation -> case
                  assessCandidate evaluation inputBoxPolicy simplificationPolicy
...
counterexampleSeed
  ...
  => AssociatedRankedCandidate d association
  -> Maybe [Natural]
counterexampleSeed (AssociatedRankedCandidate _ _ state) = case state of
  CandidateAssessed assessment _ -> case viewAssessment @d assessment of
    ViewCounterexample receipt -> Just $ counterexampleInputs @d receipt
    _ -> Nothing
  _ -> Nothing
...
assessCandidate evaluation inputBoxPolicy simplificationPolicy index association
  ...
  assessment <- case observation of
    ...
    LiveCounterexample receipt -> Right $ ViewCounterexample receipt
  case assessment of
    ViewCounterexample receipt -> simplifyCounterexampleAssessment evaluation
      simplificationPolicy index association query receipt
    _ -> pure $ AssociatedRankedCandidate index association
    ...
    (SolverUnsatisfiable, InputBoxEnabled limits maximums) ->
      case validateInputBox @d evaluation limits query maximums of
        ...
        Left BoxAssociationRejected ->
          Left $ localRankingFailure @d ViewEvidenceReplayMismatch index
        Right (BoxCounterexample receipt) -> Right $ ViewCounterexample receipt
        Right (BoxValidated receipt) -> Right $ ViewBoundedPositive receipt
    _ -> Right $ ViewHeuristic status
